...
/**
 * A GrGen Backend which generates C code for a libGr searchplan
 * graph model impl and a searchplan based graph matcher
 * @author Veit Batz
 * @version $Id$
 */
...
//		path.mkdirs();
	...
	protected void genActualActions(StringBuffer sb) {
		...
		for ( Iterator<Action> act_it = actionMap.keySet().iterator(); act_it.hasNext(); ) {
			...
			sb.append("static sp_acts_graph_t *negative_pattern_graphs_of_act_" + act_id + "[] = {\n  ");
			...
			/*			//create a C-array of ptrs to matcher ops representing the whole matcher program
			 if (n_matcher_ops > 0) {
			 sb.append(
			 "sp_matcher_op_t *matcher_program_of_action_" + act_id +
			 "[" + n_matcher_ops + "] = {\n");
			...
			for(int pattern_num = 0; pattern_num < n_negative_patterns[act_id] + 1; pattern_num++) {
				if (n_matcher_ops[pattern_num] > 0) {
					sb.append(
						"static sp_matcher_op_t *matcher_program_" + pattern_num + "_of_action_" + act_id +
					...
					sb.append("static sp_matcher_op_t *matcher_program_" + pattern_num + "_of_action_" + act_id + "[0] = {};\n");
				}
			}
			
			//gen the list of matcher progs
			sb.append("static sp_matcher_op_t **matcher_programs_of_action_" + act_id + "[" + (n_negative_patterns[act_id] + 1) + "] = {\n");
			...
			if (action instanceof Rule) {
				...
				if (n_new_edges > 0) {
					sb.append(
						"static sp_acts_edge_t *new_edges_of_action_" + act_id + "[" + n_new_edges + "] = { \n");
			...
			sb.append(
				"static sp_action_t action_" + act_id + " = {\n" +
			...
			sb.append( "  " + conditions.get(act_id).size() + ", \n");      /* #conditions */
			if(conditions.get(act_id).isEmpty())
				sb.append("  NULL,\n");
			else {
				sb.append("  { ");
				for(Expression expr : conditions.get(act_id)) {
					sb.append("condition_" + conditionNumbers.get(expr) + ", ");
				}
				sb.append("},\n");
			}
		...
		sb.append(
			"/* the array of all action structs */\n" +
				"static sp_action_t *sp_action[sp_N_ACTIONS] = {\n");
	 ...
	 * matcher ops are represented by C structs of type sp_matcher_op_t
	 *
	...
	private void genOp(Node node, Edge edge, MatchingAction action,
		...
		String kind = "";
		if (edge == null)
			kind = "sp_matcher_op_start_node";
		else if (nodeVisited.contains(node))
			kind = "sp_matcher_op_check";
		else {
			...
			if (homomorphicNodes.isEmpty())
				...
				kind = "sp_matcher_op_extend";
			else
				kind = "sp_matcher_op_check_or_extend";
		}
		...
		if (kind.equals("sp_matcher_op_check"))
			...
			//is chosen
			op_direction = "sp_dir_out";
		else if (! kind.equals("sp_matcher_op_start_node")) {
			//case 'extend' od 'check_or_extend'
			if (pattern.getTarget(edge) == node)
				op_direction = "sp_dir_out";
			else
				op_direction = "sp_dir_in";
		}
		...
		if (edge != null) {
			...
			sb.append(
				"sp_matcher_op_t mop_" + op_counter + "_of_pattern_" + pattern_num + "_of_action_" + act_id + " = {\n" +
					"  .kind = " + kind + ",\n" +
	...
	private int genConditionOps(Node node, Edge edge, MatchingAction action, int pattern_num, PatternGraph pattern, Collection<IR> nodeVisited, Collection<IR> edgeVisited, Collection<Entity> currentSubgraph, Collection<Expression> alreadyCheckedConds, Collection<Collection> alreadyCheckedTypeConds, StringBuffer sb) {
		...
		Iterator<Collection> type_cond_it = typeConditions[act_id].iterator();
		...
		if (evaluatableConditions.size() + evaluatableTypeConditions.size() + evaluatableGlobalConditions.size() > 0) {
			...
			for ( ; eval_type_cond_it.hasNext(); ) {
				...
				sb.append(
					"sp_matcher_op_t mop_" + op_counter + "_of_pattern_" + pattern_num + "_of_action_" + act_id + " = {\n" +
						"  .kind = sp_matcher_op_check_condition,\n" +
						"  .operand.cond = " + "type_condition_" + cond_num + "_of_action_" + act_id + ",\n" +
						"  .attrib.cond = sp_condition_local\n" +
						"};\n\n");
			...
			for ( ; eval_cond_it.hasNext(); ) {
				...
				sb.append(
					"sp_matcher_op_t mop_" + op_counter + "_of_pattern_" + pattern_num + "_of_action_" + act_id + " = {\n" +
						"  .kind = sp_matcher_op_check_condition,\n" +
						"  .operand.cond = " + "condition_" + cond_num + "_of_action_" + act_id + ",\n" +
						"  .attrib.cond = sp_condition_global\n" +
						"};\n\n");
	...
	private void genCheckNegativeOp( MatchingAction action, int neg_num, int op_counter, PatternGraph negative_pattern, StringBuffer sb ) {
		...
		sb.append(
			"sp_matcher_op_t mop_" + op_counter++ + "_of_pattern_0_of_action_" + act_id + " = {\n" +
				"  .kind = sp_matcher_op_check_negative,\n" +
		...
		for(Iterator<Collection> it = evalListMap.keySet().iterator(); it.hasNext(); ) {
			...
			for (Iterator<Node> node_it = involved_nodes.iterator(); node_it.hasNext(); ) {
				...
				if( node_num == null ) {
					if(current_node.isRetyped()) {
						...
						sb.append(
							"  sp_node_t *old_replacement_node_" + node_num + " = get_replacement_host_node(" + node_num + ");\n" +
						...
						sb.append(
							"  sp_node_t *pattern_node_" + node_num + " = get_host_node(" + node_num + ");\n" +
								"  int type_pattern_node_" + node_num + " = (int) pattern_node_" + node_num + "->type;\n");
					}
				}
				else {
					sb.append(
						"  sp_node_t *replacement_node_" + node_num + " = get_replacement_host_node(" + node_num + ");\n" +
			...
			for (Iterator<Edge> edge_it = involved_edges.iterator(); edge_it.hasNext(); ) {
				...
				if( edge_num == null ) {
					...
					sb.append(
						"  sp_edge_t *pattern_edge_" + edge_num + " = get_pattern_host_edge(" + edge_num + ");\n" +
							"  int type_pattern_edge_" + edge_num + " = (int) pattern_edge_" + edge_num + "->type;\n");
				}
				else {
					sb.append(
						"  sp_edge_t *replacement_edge_" + edge_num + " = get_replacement_host_edge(" + edge_num + ");\n" +
			...
			for (Iterator<Node> node_it = involved_nodes.iterator(); node_it.hasNext(); ) {
				...
				for (; attr_id_it.hasNext(); ) {
					...
					kindStr=kindStr.replace("fb_","sp_");
					
					sb.append(
						"  int index_a" + attr_id + "_" + pattern_or_repl +  "_node_" + node_num + " = " +
							"sp_node_attr_index[type_" + pattern_or_repl + "_node_"  + node_num + "][" + attr_id + "];\n" );
				}
			}
				...
				for (; attr_id_it.hasNext(); ) {
					...
					sb.append(
						"  int index_a" + attr_id + "_" + pattern_or_repl +  "_edge_" + edge_num + " = " +
							"sp_edge_attr_index[type_" + pattern_or_repl + "_edge_"  + edge_num + "][" + attr_id + "];\n");
				}
			}
	...
	private void genConditionsCode(StringBuffer sb) {
		...
		for(Iterator<Action> act_it = actionMap.keySet().iterator(); act_it.hasNext(); ) {
			...
			for (Iterator<Expression> it = conditions.get(act_id).iterator(); it.hasNext(); ) {
				...
				sb.append(
					"/* condition " + cond_num + " (belongs to action " + act_id + ") */\n" +
						"static int condition_" + cond_num + "_of_action_" + act_id + "(sp_acts_private_t *acts_p, unsigned int neg_pattern_num)\n"+
						"{\n" +
						"  int condition_holds;\n" +
						"  /* local variables representing the 'ptr to' and the 'actual type of' the\n" +
						"   * host graph images of pattern nodes/edges with the respective number */\n");
				...
				for (Iterator<Node> node_it = involved_nodes.iterator(); node_it.hasNext(); ) {
					...
					if( node_num == null ) {
						...
						sb.append(
							"  sp_node_t *negative_pattern_node_" + node_num + " = get_negative_host_node(" + node_num + ");\n" +
				...
				for (Iterator<Edge> edge_it = involved_edges.iterator(); edge_it.hasNext(); ) {
					...
					if( edge_num == null ) {
						...
						sb.append(
							"  sp_edge_t *negative_pattern_edge_" + edge_num + " = get_negative_host_edge(" + edge_num + ");\n" +
								"  int type_negative_pattern_edge_" + edge_num + " = (int) negative_pattern_edge_" + edge_num + "->type;\n");
					}
					else {
						sb.append(
							"  sp_edge_t *pattern_edge_" + edge_num + " = get_host_edge(" + edge_num + ");\n" +
								"  int type_pattern_edge_" + edge_num + " = (int) pattern_edge_" + edge_num + "->type;\n");
					}
				...
				for (Iterator<Node> node_it = involved_nodes.iterator(); node_it.hasNext(); ) {
					...
					Iterator<Integer> attr_id_it =
						((Collection) involvedPatternNodeAttrIds.get(current_cond).get( current_pattern_node )).iterator();
					...
					for (; attr_id_it.hasNext(); ) {
						...
						sb.append(
							"  int index_a" + attr_id + "_" + pattern_or_neg +  "_node_" + node_num + " = " +
								"sp_node_attr_index[type_" + pattern_or_neg + "_node_"  + node_num + "][" + attr_id + "];\n" +
				...
				for (Iterator<Edge> edge_it = involved_edges.iterator(); edge_it.hasNext(); ) {
					...
					Iterator<Integer> attr_id_it =
						((Collection) involvedPatternEdgeAttrIds.get(current_cond).get(current_pattern_edge)).iterator();
					for (; attr_id_it.hasNext(); ) {
						Integer attr_id = attr_id_it.next();
						String kindStr =
							AttrTypeDescriptor.kindToStr(edge_attr_info[attr_id.intValue()]);
						...
						sb.append(
							"  int index_a" + attr_id + "_" + pattern_or_neg +  "_edge_" + edge_num + " = " +
								"sp_edge_attr_index[type_" + pattern_or_neg + "_edge_" + edge_num + "][" + attr_id + "];\n" +
			...
			for (Iterator<Collection> it = typeConditions[act_id].iterator(); it.hasNext(); ) {
				...
				if (!involved_nodes.isEmpty() || !involved_edges.isEmpty())
					sb.append(
						"/* condition " + cond_num + " (is a type condition, " +
							...
							"static int type_condition_" + cond_num + "_of_action_" + act_id + "(sp_acts_private_t *acts_p, unsigned int neg_pattern_num)\n" +
							"{\n" +
							"  int condition_holds;\n" +
							"  /* local variables representing the 'ptr to' and the 'actual type of' the\n" +
				...
				if (!involved_nodes.isEmpty()) {
					...
					if( node_num == null ) {
						...
						sb.append("  sp_node_t *negative_pattern_node_" + node_num + " = get_negative_node(" + node_num + ");\n");
					}
					else {
						sb.append("  sp_node_t *pattern_node_" + node_num + " = get_host_node(" + node_num + ");\n");
					}
					...
					if( edge_num == null ) {
						...
						sb.append("  sp_edge_t *negative_pattern_edge_" + edge_num + " = get_negative_edge(" + edge_num + ");\n");
					}
					else {
						sb.append("  sp_edge_t *pattern_edge_" + edge_num + " = get_host_edge(" + edge_num + ");\n");
					}
...
//					"different operand types found in binary expression";
	...
	private void genActionsDescr(StringBuffer sb) {
		...
		for(Iterator<Action> it = actionMap.keySet().iterator(); it.hasNext(); ) {
			...
			for (Iterator<Object> cond_it = allConditions.iterator(); cond_it.hasNext(); ) {
				...
				String condition_function_name = "NULL";//"condition_" + cond_num + "_of_action_" + act_id; //TODO
				String condition_kind = "sp_condition_global"; //TODO
				
				//gen the actual condition descriptor
				sb.append(
					"sp_condition_descr_t condition_" + cond_num + " = {\n" +
						"  " + cond_num + ", (gr_id_t)" + act_id + ",\n" +
						"  " + condition_function_name + ", " + condition_kind + ",\n" +
	...
	private void genActionsInfo(StringBuffer sb) {
		...
		sb.append(
			"/* the overall number of graph actions */\n" +
				"#define sp_N_ACTIONS " + n_graph_actions + "\n\n" +
				"/* the overall number of pattern graph conditions */\n" +
				"#define sp_N_CONDITIONS " + n_conditions + "\n\n");
		
		sb.append(
			"/* the overall maximum number of nodes and edges of all\n" +
				" * pattern and replacement graphs respectively */\n" +
				"#define sp_MAX_N_PATTERN_NODES " + max_n_pattern_nodes + "\n" +
				"#define sp_MAX_N_PATTERN_EDGES " + max_n_pattern_edges + "\n" +
				"#define sp_MAX_N_REPLACEMENT_NODES " + max_n_replacement_nodes + "\n" +
				"#define sp_MAX_N_REPLACEMENT_EDGES " + max_n_replacement_edges + "\n" +
				"#define sp_MAX_N_NEGATIVE_PATTERNS " + max_n_negative_patterns + "\n" +
				"#define sp_MAX_N_NEGATIVE_NODES " + max_n_negative_nodes + "\n" +
				"#define sp_MAX_N_NEGATIVE_EDGES " + max_n_negative_edges + "\n" +
				"#define sp_MAX_N_SUBGRAPHS " + max_n_subgraphs + "\n" +
				"\n\n");
	}
	...
	private void genGraph(
		...
		if (graph.getEdges().size() > 0) {
			...
			for ( ; edge_it.hasNext(); ) {
				...
				//gen a struct which describes an sp_acts_edge_t
				sb.append(
					"static sp_acts_edge_t " + prefix + "edge_" + edge_num + postfix +
						" = {\n" +
		...
		if (graph.getNodes().size() > 0) {
			...
			for ( ; node_it.hasNext(); ) {
				...
				if ( n_out_edges > 0 ) {
					sb.append(
						"static sp_acts_edge_t *out_edges_of_node_"  + node_num + "_of_" +
					...
					//...and gen an array of ptrs of type *sp_acts_edge_t
					for ( ; edge_it.hasNext(); ) {
				...
				if ( n_in_edges > 0 ) {
					sb.append(
						"static sp_acts_edge_t *in_edges_of_node_" + node_num + "_of_" +
				...
				//gen node struct
				sb.append(
					"static sp_acts_node_t " + prefix + "node_" + node_num + postfix + " = {\n" +
		...
		if (graph.getNodes().size() > 0) {
			sb.append(
				"static sp_acts_node_t *all_nodes_of_" + graphName + "[" +
					graph.getNodes().size() + "] = {\n");
		...
		if (graph.getEdges().size() > 0) {
			sb.append(
				"static sp_acts_edge_t *all_edges_of_" + graphName + "[" +
					graph.getEdges().size() + "] = {\n");
		...
		sb.append(
			"/* the struct representing the hole graph */\n"  +
				"static sp_acts_graph_t " + graphName + " = {\n" +
	...
	protected String genAttrKindFromInt(int kind) {
		switch (kind) {
			case AttrTypeDescriptor.INTEGER:
				return "sp_kind_prim_int";
			case AttrTypeDescriptor.BOOLEAN:
				return "sp_kind_prim_boolean";
			case AttrTypeDescriptor.STRING:
				return "sp_kind_prim_string";
			case AttrTypeDescriptor.ENUM:
				return "sp_kind_enum";
		}
