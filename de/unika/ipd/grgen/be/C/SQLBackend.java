...
public abstract class SQLBackend extends CBackend {
  ...
  public void init(Unit unit, ErrorReporter reporter, String outputPath) {
	...
	protected void genAttrTableCmd() {
		...
		String valids[]    = new String[] { "GR_NODE_ATTR_VALID", "GR_EDGE_ATTR_VALID" };
		...
		// create valid defines
		for (int i = 0; i < maps.length; ++i) {
			sb.append("/** Checks, wheater a " + names[i] + " attribute is valid. */\n");
			sb.append("#define " + valids[i] + "(a) ((a) >= 0 && (a) < "+ maps[i].size() + ")\n\n");
		}			 		
