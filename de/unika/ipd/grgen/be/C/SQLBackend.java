...
public abstract class SQLBackend extends CBackend {
  ...
  public SQLBackend() {
	 ...
	 */
	protected abstract String genColumn(String name, String type, 
		boolean notNull, boolean unique, boolean auto_inc, boolean primary);
	...
	private void makeJoin(StringBuffer sb, Graph gr, Edge e1, Edge e2) {
		...
		String[] names = {
			getEdgeCol(e1, colEdgesSrcId), getEdgeCol(e1, colEdgesTgtId),
			getEdgeCol(e2, colEdgesSrcId), getEdgeCol(e2, colEdgesTgtId),
		};
		
		for(int i = 0; i < nodes.length; i++) 
			for(int j = i + 1; j < nodes.length; j++) 
				addTo(sb, "", " AND ", names[i] 
					+ (nodes[i].equals(nodes[j]) ? "=" : "<>") + names[j] + BREAK_LINE);
	}
	 ...
	 */
	protected String genMatchStatement(MatchingAction act, 
		...
		for(Iterator it = graphEdges.iterator(); it.hasNext(); i++) {
			...
			if(!processedNodes.contains(src)) {
				...
				addTo(nodeWhere, "", " AND ", getNodeCol(src, colNodesId)
					+ " = " + getEdgeCol(edge, colEdgesSrcId) + BREAK_LINE);
			...
			if(!processedNodes.contains(tgt)) {
				...
				addTo(nodeWhere, "", " AND ", getNodeCol(tgt, colNodesId)
					+ " = " + getEdgeCol(edge, colEdgesTgtId) + BREAK_LINE);
		...
		for(Iterator it = negatedEdges.iterator(); it.hasNext();) {
			...
			addTo(nodeWhere, "", " AND ", "NOT EXISTS (SELECT * FROM "
			  + tableEdges + " WHERE " + tableEdges + "." + colEdgesSrcId 
			  + "=" + mangleNode(src) + "." + colNodesId 
			  + " AND " + tableEdges + "." + colEdgesTgtId
			  + "=" + mangleNode(tgt) + "." + colNodesId + ")"  + BREAK_LINE);
		}
		...
		for(Iterator it = graphEdges.iterator(); it.hasNext();) {
			...
			addTo(edgeWhere, "", " AND ", edgeTypeIsAFunc 
				+ "(" + getEdgeCol(e, colEdgesTypeId) + ", " + typeId + ")" 
				+ BREAK_LINE);
		}
		...
		for(Iterator it = nodes.iterator(); it.hasNext();) {
			...
			addTo(nodeWhere, "", " AND ", nodeTypeIsAFunc 
				+ "(" + getNodeCol(n, colNodesTypeId) + ", " + typeId + ")" 
				+ BREAK_LINE);
		}

		return "SELECT " + join(nodeCols, edgeCols, ", ") + BREAK_LINE
			+ " FROM " + join(nodeTables, edgeTables, ", ") + BREAK_LINE 
			+ " WHERE " + join(nodeWhere, edgeWhere, " AND ");
	}
	...
	protected void genRuleFinish(StringBuffer sb, Rule r, int id, Match m) {
		...
		if(nodesToInsert.size() > 0) {
		   ...
		   * We need an array to save the IDs of the inserted nodes, since
			...
			for(Iterator it = r.getRedirections().iterator(); it.hasNext();) {
				...
				sb.append("  REDIR_" + dir + "(GET_MATCH_NODE(" + fromId + ")"
					+ ", inserted_nodes[" + toId + "], " 
		...
		for(Iterator it = w.iterator(); it.hasNext();) {
			Edge e = (Edge) it.next();
			if(!e.isNegated())
				sb.append("  DELETE_EDGE(GET_MATCH_EDGE(" 
					+ m.edgeIndexMap.get(e) + "));\n");
		}
		...
		for(Iterator it = w.iterator(); it.hasNext();) {
			...
			sb.append("  INSERT_EDGE(" + etid + ", " + leftNode
				+ ", " + rightNode + ");\n");
		}
	}
	...
	protected void genMatch(StringBuffer sb, MatchingAction a, int id) {
		...
		sb.append("  MATCH_GET_RES(" + nodes.size() + ", " + edges.size()
			+ ", " + nodeNamesIdent + ", " + edgeNamesIdent + ");\n");
		sb.append("}\n\n");
  ...
  protected void genExtra() {
  	...
  	Arrays.sort(matches, Match.comparator);	
		for(int i = 0; i < matches.length; i++) {
			...
			sb.append("  { " + m.matchIdent + ", " + m.finishIdent 
				+ ", &" + m.stmtIdent + " },\n");
		}
  	sb.append("};\n");
