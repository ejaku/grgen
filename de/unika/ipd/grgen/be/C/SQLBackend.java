...
public abstract class SQLBackend extends CBackend {
  ...
  public void init(Unit unit, ErrorReporter reporter, String outputPath) {
	...
	/**
	 * Creates the command for creating attribut tables
	 */
	protected void genAttrTableCmd() {
		...
		addDefine(sb, "GR_HAVE_NODE_ATTR", nodeAttrMap.size() > 0 ? "1" : "0");
		
		sb.append("\n/**\n * The SQL command for creating the node attribute table\n */\n");
		sb.append("static const char *cmd_create_node_attr = \n");
		sb.append("\"CREATE TABLE " + tableNodeAttrs + " (\" \\\n");
		sb.append("\"" + colNodesId + " " + getIdType() + " NOT NULL PRIMARY KEY\" \\\n");
		for(Iterator it = nodeAttrMap.keySet().iterator(); it.hasNext();) {
			Entity ent = (Entity) it.next();	
			Type ty = ent.getType();
				
			sb.append("\", " + ent.getIdent() + " " + getSQLType(ty) + " \" \\\n");
		}
		sb.append("\")\";\n\n");
		
		// now edge attributes
		addDefine(sb, "GR_HAVE_EDGE_ATTR", edgeAttrMap.size() > 0 ? "1" : "0");
		
		sb.append("\n/**\n * The SQL command for creating the edge attribute table\n */\n");
		sb.append("static const char *cmd_create_edge_attr = \n");
		sb.append("\"CREATE TABLE " + tableEdgeAttrs + " (\" \\\n");
		sb.append("\"" + colEdgesId + " " + getIdType() + " NOT NULL PRIMARY KEY\" \\\n");
