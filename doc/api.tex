...
bar.Modify(graph, matches.First, out b);
\end{verbatim}
...
\texttt{debugger.Init\-New\-Rewrite\-Sequence(sequence, true);} and \texttt{procEnv.Apply\-Graph\-Rewrite\-Sequence(sequence);}.
...
You have to specify the \texttt{persistenceProviderDllName}, as of now only \texttt{libGr\-Persistence\-Provider\-SQLite.dll} is supported, and the connectionParameters, these are the same that you give with the \texttt{new graph persist with to} shell command (cf. \ref{persistentgraphcreationcommands} -- there, you also find some words on the design in a subsection).
Also, persistent graph parameters may be given, see \ref{persistentgraphparameters} for more on them.
