...
%todo: see whether further extensions/updates/corrections are needed, should be up-to-date similar to developing, let's see how to handle shared content in between the two chapters, compare them against each other; at least pointers

This chapter describes the Application Programming Interface of \GrG, i.e. of the system runtime, explaining how to use the supplied or generated code from your own applications -- the user interface of the compiler and shell applications with the command line arguments, the languages, and the GUI parts was already introduced in the previous chapters.
The main part of the interface is defined by the \LibGr{}, which is implemented by the \LGSPBackend{} that also defines parts of the interface, as well as the assemblies generated from the model and rule specification files -- but also other components come into play like e.g. the \GraphViewerAndSequenceDebugger{} assembly.

We'll have a look at
\begin{itemize}
\item the interface to the graph and the model
\item the interface to the rules and the matches
...
\item the interface to the debugger and the graph viewer
\item the interface to the shell functionality
\item the interface to the persistent graph and the persistence provider
\item how to implement external class and function declarations
\item how to implement external match filter and external sequence declarations
\item the events fired when the graph is changed
\item the events fired during action execution
\end{itemize}
...
\noindent Normally you want to use the type-safe interface of the generated code as it is much more convenient.
...
At runtime, you could generate a new rule set file, apply the compiler, and dynamically link the resulting assemblies/dlls (similar to what \GrShell{} does), or re-generate the pattern matchers of some of the existing rules, dynamically compile them, and replace the old pattern matchers.
But you cannot extend an existing rule set at runtime with new rules.
...
For easier debugging though, especially when you are integrating the generated code with own extensions (as described in chapter \ref{chapextensions}), it is recommended to directly include the source code generated (which is thrown away normally after it was compiled into the assemblies \texttt{lgsp-FooModel.dll} and \texttt{lgsp-FooActions.dll}).
For this, use the \texttt{-keep} option when you call \texttt{grgen.exe}, and include the model and the actions file (excluding the intermediate actions file) directly as C\# source code files (preferably, you also specify the \texttt{-debug} option then).

We recommend that you read this chapter together with one or several examples from the \texttt{examples-api} folder, and access to the API documentation at \url{https://grgen.de/doc/API/index.html} (including comments in addition to the raw type specifications) or even better the source code from \url{https://github.com/ejaku/grgen}, as we only replicate small parts of the classes and interfaces written about -- the contained examples should be sufficient to give a first impression, though.
%maybe TODO: replicate/duplicate at least some of the contents spoken about, showing them or at least key parts, e.g. the IAction interface, the PatternGraph interface, the IGraph interface, etc, so things are self-contained; chapter specifies relationship to concepts from code specified in GrGen-languages
...
This builds the exact interface of the model (with sharp types), it is implemented by a sealed class \texttt{Bar} with generated code and with code from the lgsp backend.
...
which supports iteration over the entities defined in the model, using further generic/inexact (we use generic in a wide sense here, not in the narrow sense meaning template methods or generic collections) interfaces from \LibGr{} (plus gives access to some model properties).
...
Some examples with code will be given in the following section.
...
The named graph offers persistent names (cf. \ref{persistentex}) for all of its graph elements, but besides that, it is identical to an \texttt{LGSPGraph}. %todo: own section for persistent names pointing to instances/occurrenes of this concept? also: unique names, persistence is only a feature that requires unique names, so somewhat misnamed
...
but be aware that the storage of the names requires about the same amount of memory as an unnamed graph (without attributes) as such.
...
The interface types for the node and edge root types are already supplied in \LibGr{} and are thus common to all graph rewrite systems (this allows for the "generic" handling); 
...
The interfaces of the types you specified in the model are generated, as well as the backing implementation types of all these interface types.

\begin{example}\label{ex:api2:modelonly}
We start by including the namespaces of the libGr and the lgsp backend shipped with \GrG\,
plus the namespaces of our model, generated from \texttt{Foo.grg}.
\begin{verbatim}
...
Then we create the model, followed by the graph, as an instance of the generic \texttt{LGSPGraph}, plus the global variables.
We create a single node of type \texttt{Bar} in the graph, which we assign to a variable \texttt{b}.
Afterwards, we read and write its attribute \texttt{value}.
\begin{verbatim}
IGraphModel model = new FooGraphModel();
IGraph graph = new LGSPGraph(model, new LGSPGlobalVariables());
Bar b = Bar.CreateNode(graph);
b.value = b.value + 1;
\end{verbatim}

We could have created a named graph instead, that is also allowing to access graph elements by their persistent names:
\begin{verbatim}
INamedGraph graph = new LGSPNamedGraph(model, new LGSPGlobalVariables(), 0);
INode node = graph.GetNode("\$1");
\end{verbatim}
\end{example}

\begin{example}\label{ex:api1:modelonly}
\GrG{} also generates a specific graph class with the model bound to it at generation time, offering convenience functions based on the model.
This is an example showing how \ref{ex:api2:modelonly} is realized with that graph version.
...
Also a dedicated named graph class is available.
\begin{verbatim}
...
\begin{example}
This is an example showing how to use the \texttt{IGraph} interface.
\begin{verbatim}
// query the IGraph graph, its model and the INode n
foreach(INode node in graph.Nodes) doSomething(node);
NodeType nt = graph.Model.NodeModel.GetType("Bar");
    //nt = de.unika.ipd.grGen.Model_Foo.NodeType_Bar.typeVar;
foreach(INode node in graph.GetCompatibleNodes(nt)) doSomething(node);
foreach(INode node in graph.GetExactNodes(nt)) doSomething(node);
foreach(IEdge edge in n.Outgoing)
{
    if(edge.Source == edge.Target) doSomething(edge.Source);
}

// update the graph by commands
INode n = graph.AddNode(nt);
IEdge e = graph.AddEdge(graph.Model.EdgeModel.GetType("Baz"), n, n);
n = Retype(n, graph.Model.NodeModel.GetType("Qux"));
graph.Remove(n);
\end{verbatim}
\end{example}
...
The generated file \texttt{FooActions.cs} opens the namespace \texttt{de.unika.ipd.grGen.Action\_Foo} containing all the generated entities.
...
a description of the pattern to match (the rule representation generated by the compiler frontend), and the modify methods for carrying out the rewriting.
...
implementing the abstract class \texttt{BaseActions} from \texttt{de.unika.ipd.grGen.libGr} (in fact \texttt{LGSPActions} from the namespace \texttt{de.unika.ipd.grGen.lgsp}),
...
Here we'll show a short example of how to use \GrG\ with the type-safe API, an extended version of the model-only \ref{ex:api1:modelonly}.

We start by including the namespace of our actions, generated from \texttt{Foo.grg}, assuming the namespaces of the libGr, the lgspBackend, and the model were already included like in Example \ref{ex:api2:modelonly}.
\begin{verbatim}
...
Finally, we apply the action \texttt{bar(Bar x) : (Bar)} on the graph via the graph processing environment \texttt{procEnv} (introduced in the following section), with \texttt{b} as input, also receiving the output.
The rule is taken from the actions via the member named as the action.
\begin{verbatim}
FooGraph graph = new FooGraph(new LGSPGlobalVariables());
FooActions actions = new FooActions(graph);
Bar b = graph.CreateNodeBar();
actions.bar.Apply(procEnv, b, ref b); // input of type Bar, output of type Bar
\end{verbatim}
\end{example}

\begin{example}
This is an extended version of the previous example \ref{ex:api1}, based on \ref{ex:api2:modelonly}.
Again, we create the actions to apply on the graph (here with a graph not bound to a model).
...
IMatchesExact<Rule_bar.IMatch_bar> matches = bar.Match(procEnv, 0, b);
if(!someCheck(matches.FirstExact.node_someNodeFromRuleBar.GetAttribute("a")))
    return;
bar.Modify(procEnv, matches.First, out b);
\end{verbatim}
\end{example}

\begin{example}
If you want to work with the generic (object and string based) interface not knowing the exact generated types, your entry point is the \texttt{LGSPBackend} instance implementing the \texttt{IBackend} interface.
\begin{verbatim}
IBackend backend = LGSPBackend.Instance;
...
backend.CreateNamedFromSpec("Foo.grg", globalVariables, "graph-name", null,
                    flags, new List<String>(), 0,
                    out graph, out actions);
\end{verbatim}
\end{example}
%todo: explain generic interface with (more) examples

Regarding actions generation:
...
Besides, by now you could also request to generate initial matchers based on a statistics file with the compiler parameter \texttt{-statistics} and graph analyzation data saved in a previous session with the \texttt{custom graph statistics save} shell command.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Interface of the Graph Processing Environment}\label{sec:procenv}

The graph processing environment -- which is by now needed for all transformations -- offers most of the additional processing-related functionality of \GrG~exceeding what is offered by the graph and the actions.
It is made available by the interface \texttt{IGraph\-Processing\-Environment}, which is implemented in the \texttt{LGSPGraph\-Processing\-Environment} class.

\begin{example}\label{ex:procenv}
The graph processing environment \texttt{procEnv} is constructed on top of the graph and the actions, both of which are handed in.
It esp. allows to carry out transformations by executing sequences, combining actions with operators (for control-flow) and variables (for data-flow).
...
The \texttt{IGraph\-Processing\-Environment} inherits from the \texttt{ISubaction\-And\-Output\-Addition\-Environment} which in turn inherits from the \texttt{IAction\-Execution\-Environment}.
The \texttt{IAction\-Execution\-Environment} contains certain configuration options, multiple helpers for action execution which are used by the sequences and could/should be preferred to using the actions directly if tight integration with the framework is targeted, and gives access to the most important parts of a graph rewrite system or transformation.
The \texttt{ISubaction\-And\-Output\-Addition\-Environment} enriches it by subgraph handling and output emitting plus graph change recording. 
The \texttt{IGraph\-Processing\-Environment} contains the application logic \texttt{Transaction\-Manager} and gives access to the \texttt{Persistence\-Provider\-Transaction\-Manager} iff one is available because a persistent graph is used, it contains helper code to interact with the graph global variables and allows to parse and apply sequences.
All environments offer events, you find more on them in Section \ref{sec:actionevent} following below.

The most important of the helper classes is the transaction manager which is utilized when \GrG~ is used for crawling through a search space or for enumerating a state space, see section \ref{sec:extctrl}.
...
For assignments of complete attribute values, you must implement \texttt{Parse} and \texttt{Serialize}.
...
It forwards the call to \texttt{AsGraphImpl}, which needs to be implemented by \emph{you}, in the same way as specified above for the functions called when \texttt{external emit class;} is given.
...
The \texttt{graph\-Viewer\-And\-Sequence\-Debugger} project contains convenience interfaces that allow you to access the graph viewer and the sequence debugger easily from API level (you have to include the \texttt{graph\-Viewer\-And\-Sequence\-Debugger.dll} in order to do so, and in case you want to use the MSAGL based debugger, you have to make the \texttt{graph\-Viewer\-And\-Sequence\-Debugger\-Windows\-Forms.dll} available beside the aforementioned dynamic link library, note its package reference to the NuGet package \texttt{Microsoft.Msagl.Graph\-Viewer\-GDI} requiring the inclusion of \texttt{System.Resources.Extensions} with a \texttt{binding\-Redirect} in the \texttt{app.config} of your application (the \texttt{GrShell.csproj} from the sources may serve as an example)).
...
The \texttt{Debugger} class allows you to execute a graph rewrite sequence under debugger control (\texttt{Apply\-Graph\-Rewrite\-Sequence}) from your own application, once it is configured properly esp. with the required environments (\texttt{Debugger\-Environment} and \texttt{Debugger\-Graph\-Processing\-Environment}) by utilizing 
\texttt{debugger.Init\-New\-Rewrite\-Sequence(sequence, true);} and the \texttt{procEnv.Apply\-Graph\-Rewrite\-Sequence(sequence);}.

The \texttt{examples-api/DebuggerExample/DebuggerExample.cs} shows how to execute a sequence in the debugger (in a stdout command line window in case of \texttt{GraphViewerTypes.YComp}, or in a WindowsForms command line window emulation in case of \texttt{GraphViewerTypes.MSAGL}, or \texttt{GraphViewerTypes.MSAGLExt} if you specify it) (utilizing parts of the \texttt{Mutex} example; and a \texttt{Gui\-Console\-Control}, hosted in a \texttt{Gui\-Console\-Debugger\-Host} in case of the WindowsForms example).


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Shell functionality}

The \texttt{libGrShell} project allows you to include \GrShell{}-functionality (this means esp. executing a \texttt{.grs} script file, but you could also use it as an in-place interactive command line) into your own application (you have to reference the \texttt{lib\-Gr\-Shell.dll} (and the \texttt{graph\-Viewer\-And\-Sequence\-Debugger.dll} in order to do so)).

The \texttt{examples-api/ShellExample/ShellExample.cs} shows how to achieve this (utilizing parts of the \texttt{Mutex} example).

The \texttt{examples-api/Shell\-Example\-Windows\-Forms/Program.cs} and \texttt{examples-api/Shell\-Example\-Windows\-Forms/ShellForm.cs} show how to utilize the \LibGrShell{} from a Windows Forms application (note that it requires the \texttt{lib\-Console\-And\-OS\-Windows\-Forms.dll}).
...
The console application \GrShell{} and the Windows Forms application \GGrShell{} could also serve as examples, but then you'd need the source code of \GrG.
...
You have to specify the \texttt{persistenceProviderDllName}, as of now only \texttt{libGr\-Persistence\-Provider\-SQLite.dll} is supported, and the \texttt{connection\-Parameters}, these are the same that you give with the \texttt{new graph persist with to} shell command (cf. \ref{persistentgraphcreationcommands} -- there, you also find some words on the design in a subsection).
Also, \texttt{persistent\-Graph\-Parameters} may be given, see \ref{persistentgraphparameters} for more on them.
...
\begin{example}\label{ex:pptransman}
\begin{verbatim}
...
The \texttt{Start} starts a transaction if none exists yet (nested transactions are \emph{not} supported).
The \texttt{CommitAndRestart} carries out an intermediate commit, afterwards the changes up to that point are persistently stored (for sure), and a transaction is active; this is what is typically used to balance performance against the granularity of persistence -- for real transactions, use the application layer transaction manager.
The \texttt{Commit} commits the transaction, afterwards the changes up to that point are persistently stored (for sure), and a transaction is not active anymore.
The \texttt{Rollback} rolls the changes since that last CommitAndRestart() or the last Start() if no restart occurred back, afterwards the transaction is not active anymore. Pending changes are forgotten when no commit occurs.

The persistence provider depends on an SQLite ADO.NET driver, which is included by a \texttt{System.Data.SQLite} package reference, and on the SQLite itself, a build of it is included by a \texttt{SourceGear.sqlite3} package reference.
The release is a \texttt{x64} version only by now, which contains Windows and LINUX binaries of the native SQLite (\texttt{.dll} and \texttt{.so}).
See \ref{sub:building} for more on this (e.g. if you want to build an \texttt{x86} version).
...
inheriting from each other as stated in the external class declarations, containing a body of comments giving the expected function and procedure method prototypes.
...
This occurs by employing the following methods of the \texttt{IAction} (or better \texttt{IActionCore}) interface:
\begin{csharplet}
...
\begin{csharplet}
IMatches myMatches = myAction.Match(myProcEnv, -1, null); // -1: get all the matches
for(int i=0; i<myMatches.NumMatches; ++i)
{
	if(inspectCarefully(myMatches.GetMatch(i)))
	{
		myAction.Modify(myProcEnv, myMatches.GetMatch(i));
		break;
	}
}
\end{csharplet}
...
Many interesting filters can be even generated automatically for you, e.g. the \texttt{auto} filter for filtering symmetric matches of automorphic patterns, see \ref{sub:extflt} and \ref{sub:filters} for more on this.
You can call the (internal) filters by employing the following method of the \texttt{IAction} interface:
\begin{csharplet}
...
The debugger, the graph viewers, the transaction handler, the graph change recorder, and the persistent graph implement their functionality by listening and reacting to these events.
...
Otherwise the changes won't be visible in the debugger, they won't be rolled back at the end of a transaction or during backtracking, and they won't be recorded in case of change recording.

Quite a lot of a three layer architecture graph rewrite application is available out of the box due to the events, see also \ref{birdseyeview} (a user application you supply, working at API level, or the \GrShell{}/\GGrShell{} system applications).
...
event EnterSequenceHandler OnEnteringSequence;
