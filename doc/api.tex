...
The \texttt{graph\-Viewer\-And\-Sequence\-Debugger} project contains convenience interfaces that allow you to access the graph viewer and the sequence debugger easily from API level (you have to include the \texttt{graph\-Viewer\-And\-Sequence\-Debugger.dll} in order to do so, and in case you want to use be MSAGL based debugger, you have to make the \texttt{graph\-Viewer\-And\-Sequence\-Debugger\-Windows\-Forms.dll} available beside the aforementioned dynamic link library, note its package reference to the NuGet package \texttt{Microsoft.Msagl.Graph\-Viewer\-GDI} requiring the inclusion of \texttt{System.Resources.Extensions} with a \texttt{binding\-Redirect} in the \texttt{app.config} of your application (the \texttt{GrShell.csproj} from the sources may serve as an example)).
...
The \texttt{Commits} commits the transaction, afterwards the changes up to that point are persistently stored (for sure), and a transaction is not active anymore.
The \texttt{Rollback} rolls the changes since that last CommitAndRestart() or the last Start() if no restart occurred back, afterwards the transaction is not active anymore. Pending changes are forgotten when no commit occurrs.
