...
\item[\texttt{.orderDescending()}] returns an array with the content of the input array ordered descendingly, only available for basic types
\item[\texttt{.keepOneForEach()}] returns an array with the content of the input array freed from duplicates, only available for basic types
...
\item[\texttt{.med()}] returns the median of the elements in the array that must be sorted, as \texttt{double}
\item[\texttt{.medUnsorted()}] returns the median of the elements in the array, as \texttt{double}
...
\begin{description}
\item[\texttt{.extract<attr>()}] returns for an \texttt{array<T>} an \texttt{array<S>}, where \texttt{S} is the type of the attribute \texttt{attr}.
...
\item[\texttt{.orderDescendingBy<attr>()}] returns an array with the content of the input array ordered descendingly; only available for an attribute of basic type, contained in a node or edge type, with the array storing graph elements of that type.
\item[\texttt{.keepOneForEach<attr>()}] returns an array with the content of the input array freed from duplicates; only available for an attribute of basic type, contained in a node or edge type, with the array storing graph elements of that type.
