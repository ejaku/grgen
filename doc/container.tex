...
\noindent Some accumulation method calls are available on arrays of numbers:
...
Furthermore, several query method calls are available on 
\begin{itemize}
	\item arrays of node or edge types bearing attributes;
	\item some also on arrays of match types having match member entities
	\item and arrays of iterated match types having match member entities
	\item and arrays of match class types having match class member entities
\end{itemize}
They inspect the value of the attribute or member defined with the query method of the graph elements or matches stored in the array, instead of the values contained in the array directly:

\begin{description}
\item[\texttt{.extract<attr>()}] returns for an \texttt{array<T>} an \texttt{array<S>}, where \texttt{S} is the type of the attribute or member \texttt{attr}.
...
\item[\texttt{.indexOfBy<attr>(valueToSearchFor)}] returns the first position \texttt{valueToSearchFor:T} appears at, as \texttt{int}, or -1 if not found; only available for an attribute \texttt{attr} of a graph element type.
\item[\texttt{.indexOfBy<attr>(valueToSearchFor, startIndex)}] returns the first position \texttt{valueToSearchFor:T} appears at (moving to the end), when we start the search for it at array position \texttt{startIndex:int}, as \texttt{int}, or -1 if not found; only available for an attribute \texttt{attr} of a graph element type.
\item[\texttt{.lastIndexOfBy<attr>(valueToSearchFor)}] returns the last position \texttt{valueToSearchFor:T} appears at, as \texttt{int}, or -1 if not found; only available for an attribute \texttt{attr} of a graph element type.
\item[\texttt{.lastIndexOfBy<attr>(valueToSearchFor, startIndex)}] returns the last position \texttt{valueToSearchFor:T} appears at (moving to the begin), when we start the search for it at array position \texttt{startIndex:int}, as \texttt{int}, or -1 if not found; only available for an attribute \texttt{attr} of a graph element type.
\item[\texttt{.indexOfOrderedBy<attr>(valueToSearchFor)}] returns a position where \texttt{valueToSearchFor:T} appears at, as \texttt{int}, or -1 if not found; only available for an attribute \texttt{attr} of a graph element type. The array must be ordered, otherwise the results returned by the binary search employed will be wrong; in case of multiple occurences, an arbitrary one is returned.
\end{description}
%And again: the \verb#X_by<attr>#-versions inspect the value of the attribute of the graph element stored in the array.

The ordering methods utilize the ordering relation, so they support only members or attributes of numeric type (\texttt{byte}, \texttt{short}, \texttt{int}, \texttt{long}, \texttt{float}, \texttt{double},) or \texttt{string} or \texttt{enum} type -- also \texttt{boolean} is supported, even if not of much use).
