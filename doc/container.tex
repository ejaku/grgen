...
Left associative binary expressions allow you to compute a new container from two input containers, or to compare two containers.
...
And finally iteration over the elements in the container is supported with a \texttt{for} loop, cf.~\ref{sub:controlflow}.

\begin{rail}
  ForContainerHeader: 'for' '(' Var ':' Type 'in' ContainerVar ')';
  ForContainerWithIndexVarHeader: 'for' '(' IndexVar ':' Type '->' Var ':' Type 'in' ContainerVar ')' ;
\end{rail}\ixkeyw{in}\ixkeyw{for}\ixnterm{ForContainerHeader}\ixnterm{ForContainerWithIndexVarHeader}
...
\begin{rail}
  ...
  SetConstructor: ('new')? 'set' '<' Type '>' \\ lbrace ( Expression*',' ) rbrace ;
...
\item[\texttt{.min()}] returns the minimum of the elements in the set, as \texttt{T'} for \texttt{set<T>} (see \ref{sec:accumulation} regarding \texttt{T'})
\item[\texttt{.max()}] returns the maximum of the elements in the set, as \texttt{T'} for \texttt{set<T>} (see \ref{sec:accumulation} regarding \texttt{T'}).
...
\item[Set union:] \texttt{s.addAll(os)} adds all elements in set \texttt{os} to the set \texttt{s}. (Only available on targets that are set variables, not on attributes.)
\end{description}
...
It is an O(1) operation for sets.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{|l|X|}
    \hline
    \texttt{A == B} & True, iff the content of $A$ and $B$ is identical. \\
    \texttt{A != B} & True, iff the content of $A$ and $B$ is not identical. \\
    \texttt{A \textasciitilde\textasciitilde\ B} & True, iff $A$ and $B$ are by-value equal (deeply).\\
    ...
    \texttt{A >= B} & True, iff $A$ is a superset of $B$ or $A$ and $B$ are identical. \\
    \hline
  \end{tabularx}
...
The elements given in the type constructor are cast to the specified member type if needed and possible.
...
\begin{rail}
  ...
  MapConstructor: ('new')? 'map' '<' Type ',' Type '>' \\ lbrace ( (Expression '->' Expression)*',' ) rbrace ;
...
It is an O(1) operation for maps.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{|l|X|}
    ...
    \texttt{A >= B} & True, iff $A$ is a supermap of $B$ or $A$ and $B$ are identical. \\
    \hline
  \end{tabularx}
...
It is constant if only primitive type literals, enum literals, or constant expressions are used; this is required for container initializations in the model.
It is non-constant if it contains nodes/edges/or member accesses, which may be the case if used in the rules.
The elements given in the type constructor are cast to the specified member types if needed and possible.
...
\begin{rail}
  ...
  ArrayConstructor: ('new')? 'array' '<' Type '>' \\ '[' ( Expression*',' ) ']' ;
...
\item[\texttt{.indexOfOrdered(valueToSearchFor)}] returns a position where \texttt{valueToSearchFor:T} appears at, as \texttt{int}, or -1 if not found. The array must be ordered, otherwise the results returned by the binary search employed will be wrong; in case of multiple occurrences, an arbitrary one is returned
\item[\texttt{.orderAscending()}] returns an array with the content of the input array ordered ascendingly, only available for basic types that are orderable
\item[\texttt{.orderDescending()}] returns an array with the content of the input array ordered descendingly, only available for basic types that are orderable
\item[\texttt{.group()}] returns an array with the content of the input array grouped, i.e. equal values being neighbours, only available for basic types or node and edge types
\item[\texttt{.keepOneForEach()}] returns an array with the content of the input array freed from duplicates, only available for basic types or node and edge types
\item[\texttt{.reverse()}] returns an array with the content of the input array reversed
\item[\texttt{.shuffle()}] returns an array with the content of the input array permuted
...
The other query methods utilize equality comparisons, so they support additionally elements of node or edge type.

Furthermore, some array accumulation, by-member-or-attribute, and per-element methods are available, you find more on them in Section~\ref{sec:accumulation}.
...
\item[Array removal:] \texttt{a.rem()} removes the value at the end of the array \texttt{a}.
...
\item[Array concatenation:] \texttt{a.addAll(oa)} adds all elements in array \texttt{oa} to the array \texttt{a}. (Only available on targets that are array variables, not on attributes.)
\end{description}
...
It is an O(n) operation for arrays.
...
\begin{note}
\GrG{} does not support multi-dimensional arrays, but note that you can easily emulate them, e.g. a two-dimensional array \texttt{a} with dimensions \texttt{n} and \texttt{m} by initializing it to \texttt{n*m} elements and accessing it by \texttt{a[i*n + j]} instead of \texttt{a[i,j]}.
Jagged arrays require intermediate helper objects storing the nested array, i.e. \texttt{a:array<NestedArray>} with \texttt{class NestedArray \{ nestedArray : array<TargetType>; \}}.
\end{note}

Furthermore, the container may be iterated over with a \texttt{for} loop, as introduced in  \ref{sub:controlflow}.
The array allows for non-indexed as well as indexed iteration; if non-indexed iteration is used the array values are iterated over, if indexed iteration is used the index is assigned to the index variable and the corresponding value is assigned to the value variable. 
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{|l|X|}
    ...
    \texttt{A >= B} & True, iff $A$ is a superarray of $B$ or $A$ and $B$ are identical. \\
    \hline
  \end{tabularx}
...
\begin{rail}
  ...
  DequeConstructor: ('new')? 'deque' '<' Type '>' \\ '[' ( Expression*',' ) ']' ;
...
\item[\texttt{.lastIndexOf(valueToSearchFor)}] returns the last position \texttt{valueToSearchFor:T} appears at, as \texttt{int}, or -1 if not found
\item[\texttt{.lastIndexOf(valueToSearchFor, startIndex)}] returns the last position \texttt{valueToSearchFor:T} appears at (moving to the begin), when we start the search for it at deque position \texttt{startIndex:int}, as \texttt{int}, or -1 if not found
...
\item[Deque removal:] \texttt{d.rem()} removes the value at the begin of the deque \texttt{d}.
...
It is an O(n) operation for deques.
...
The relational expressions (already introduced in \ref{sec:relational}) used to compare entities of different kinds, mapping them to the type boolean, are extended to sets according to table~\ref{compdeque}:
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{|l|X|}
    ...
    \texttt{A >= B} & True, iff $A$ is a superdeque of $B$ or $A$ and $B$ are identical. \\
    \hline
  \end{tabularx}
...
\begin{note}
The double ended queue allows for fast addition and removal both at the front and at the back, in contrast to arrays that only support this at the end.
...
The following syntax diagram gives an extension to the syntax diagrams of the Rule Set Language chapter \ref{chaprulelang}, pattern part:
\begin{rail}
  StorageAccess:
...
As remedy the \texttt{clone} function may be used to clone the container before the iteration, so that the clone can be iterated over and the original container changed.

\begin{description}
\item[\texttt{clone(set<T>):set<T>}] returns a clone of the original set given as argument.
\item[\texttt{clone(map<S,T>):map<S,T>}] returns a clone of the original map given as argument.
\item[\texttt{clone(array<T>):array<T>}] returns a clone of the original array given as argument.
\item[\texttt{clone(deque<T>):deque<T>}] returns a clone of the original deque given as argument.
\end{description}
...
But when a container is assigned to a node or edge or internal class object attribute, it is copied by value. 
...
If you want to change a node or edge or internal class object attribute of container type, you should use for performance reasons \texttt{add}, \texttt{rem}, and indexed assignments (this esp. holds if you are using transactions).
	\begin{grgen}
