...
This chapter gives an introduction to the visualization and debugging capabilities of \GrG.
They are offered by a console debugger (either rendered on a stdout-console or a WindowsForms console like display) or a GUI debugger containing an input/output console,
...
You may also specify \texttt{MSAGL} or \texttt{MSAGLExt} (see \ref{sec:MSAGL} for more on MSAGL).
Note that no file will be dumped in this case, and that the shell will be frozen until you close the graph viewer window (furthermore, parameters will be ignored). 
...
The following commands control the style of the VCG output and the online graph display.
...
The keyword \texttt{only} excludes the subtypes from exclusion, i.e.\ subtypes of \emph{NodeType}/\emph{EdgeType} are dumped.
...
These commands allow you to still use the debugger if the graph as such is too large to be laid out (or laying it out takes too long to be convenient).
...
Small graphs allow for fast visual understanding, but with an increasing number of nodes and edges they quickly lose this property.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{MSAGL Usage}\label{sec:MSAGL}
...
\GrG\ itself is licensed as LGPL, so you can ship it with your own (even commercial) program - the functionality of the \MSAGL{} based graph viewer is limited compared to what \yComp{} offers, though (and under Mono/Linux, sometimes the text labels disappear, you may have to pan with the mouse as a workaround then).
...
\begin{rail}
  'debug' 'with' ( 'yComp' | 'MSAGL' | 'MSAGLExt' )
\end{rail}\ixkeyw{debug}\ixkeyw{with}
Switches the graphical part of the debugger (the graph viewer) in between \yComp{} (default) and \MSAGL{} or extended \MSAGL{}.
Extended \MSAGL{} supplies an additional left pane similar to \yComp{}, with a map-like view, a search pane, a node subgraph nesting tree, and an attributes/properties view.
Here, too, you can search with \texttt{Ctrl-f} or \texttt{/}.

If \MSAGL{} (or its extended version) is used, the debugger is displayed with a Windows Forms\cite{WinForms} GUI, potentially consisting only of a console like control -- similar to the stdout console of the \GrShell{} used to display the debugger in case of \yComp{}.

The program graph from \texttt{examples/JavaProgramGraphs-GraBaTs08} looks like depicted in Figures~\ref{figprogramgraph3} and \ref{figprogramgraph4} (compared to the \yComp{}-rendering in Figures~\ref{figprogramgraph1} and \ref{figprogramgraph2}).
...
\begin{figure}[hptb]
  \centering
  \includegraphics[width=0.99\textwidth]{fig/screen-msagl-detail}
  \caption{Some contents of the ``send'' method of the initial program graph rendered with \MSAGLExt{}, shown in the GUI debugger, with an upper pane sequence rendered in graph form, i.e. as an abstract syntax tree}
  \label{figprogramgraph4}
\end{figure}

The WinForms based debugger that uses \MSAGL{} as graph viewer supplies a two pane mode, it is enabled by default when gui mode is not enabled (when \yComp{} is used, only the single \GrShell{}-console is employed, irrespective of this setting or the gui mode setting), you may disable it (or enable it again) with the configuration option:
\begin{rail}
...
Figure~\ref{figprogramgraph3} shows an example screenshot of the two pane mode.

The debugger also offers a gui mode as an extension of twopane mode, it is enabled by default in case you debug with \MSAGL{} (when \yComp{} is used, only the single \GrShell{}-console is employed, irrespective of this setting), you may disable it (or enable it again) with the configuration option:
\begin{rail}
  'debug' 'set' 'option' 'gui' ( 'true' | 'false' )
\end{rail}\ixkeyw{debug}\ixkeyw{set}\ixkeyw{option}\ixkeyw{gui}
In gui mode of the debugger, the two consoles are enriched by a GUI frame offering a menu bar and a toolbar.
The menu bar contains the main debugger menu commands (the same that are available in the console main menu), some commands from the submenus/subdialogs, plus the continuation options that are offered during a debugging break; some commands from the submenus/subdialogs on the other hand are only available on the input/output console as of now.
The toolbar contains the execution controlling commands plus the continuation options.

The data rendering console view can be switched to a display in graph form, in that case sequences are rendered graphically in form of an abstract syntax tree (and the call stack in form of group nodes representing stack frames, containing their sequences, and local variables in case of a full state dump).
Figure~\ref{figprogramgraph4} shows an example screenshot of the gui mode.

You may find out which mode is enabled with the following command:
\begin{rail}
  'debug' 'get' 'options'
...
The following layout algorithms are supported by \MSAGL{} or \MSAGLExt{}: \texttt{SugiyamaScheme}, \texttt{MDS}, \texttt{Ranking}, \texttt{IcrementalLayout}.
...
The following modes are supported: \texttt{matched}, \texttt{created}, \texttt{deleted}, \texttt{retyped}, \texttt{redirected}.
...
When pre-match debugging is enabled, the matches of a (multi) rule execution before filtering/selection are displayed in detail steps, the default is disabled.
When post-match debugging is enabled, the matches of rules in embedded execs are displayed in detail steps
(with post-match disabled, only the match(es) of the very rule followed by the rewrite of the rule), the default is enabled.
...
In each debugging situation or step, the execution state or a subset of it is rendered.
...
at the beginning of a new loop iteration the highlighting state of the rules contained in the loop is reset.
...
At least some debugging aid is available for them, at least the actions called from the embedded sequences are shown in the debugger -- in case detailed mode is used.
...
But pressing the \texttt{v} key in the shell debugger is more convenient in the former case, as is searching with \texttt{Ctrl-f} or \texttt{/} in \yComp{} or \MSAGLExt{} for the persistent name or an attribute value, hovering over the then highlighted graph element, in the latter case.
...
Another development aid comes in the form of validity checking code that gets emitted when the \texttt{-debug} option is supplied to \texttt{grgen.exe} (cf. \ref{grgenoptions}), the usage of that option can be also configured in the shell (cf. \ref{sec:compilerconfigshell}).
...
Regarding the third case, note that an edge not contained in the graph anymore is not reported as invalid in case its source or target nodes are not contained in the graph anymore (this saves you from spurious error reports, at the price of potentially missing a situation where this is not anticipated).
...
In addition to the \texttt{-debug} flag, you could specify the \texttt{-keep} flag, then the generated source code is kept, which allows for easier bug hunting (we recommend you do so when you run into debugging issues).

The persistent graph (cf. \ref{persistentgraphcreationcommands}) could also be used as a debugging aide.
It allows to find dangling references to zombie nodes/edges, that are contained in attributes of graph element type, or attributes of a container type containing graph elements --- they are detected and reported when a persistent graph is loaded (i.e. deferred, not when a node/edge turns into a zombie, because it gets deleted/retyped).
