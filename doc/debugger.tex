...
A further development aide is available with the \texttt{-debug} option of \texttt{grgen.exe} (cf. \ref{grgenoptions}), when supplied some validity checking code is emitted (the usage of that option can be also configured in the shell, cf. \ref{sec:compilerconfigshell}).
The validity checking code ensures at runtime that rules are not called with null parameters (unless specified to be allowed; this typically happens when initialization is missing, or when a graph global variable is getting auto-nulled when the graph element is deleted) and that rules are not called with graph elements that are not contained in the graph anymore as parameters (this typically happens when a node or edge was removed from the graph but is still held in a local variable or a storage).
It also ensures before rewriting that the graph elements from a match are still contained in the graph (this can be violated for all-bracketed rule applications, maybe without causing a real issue, but maybe conflicting modifications will arise, e.g. the rewrite of a match deletes a node, while the rewrite of another match of the rule tries to retype the same node (the node could be matched to the same pattern element, but also to a different pattern element)).
