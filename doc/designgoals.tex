...
They allow processing substructures of arbitrary depth and breadth with a single rule application.
...
While the patterns can be combined in a functional way with nested and subpatterns to build complex rules, the rules can be combined in an imperative way with graph rewrite sequences, executing one graph state change after the other.
...
And that features were added in a way that just satisfies the needs of the domain at hand and its tasks, instead of striving for a more general solution (which would have caused higher costs at designing and implementing these features).
...
was the new \GrG\ built from the beginning as a general-purpose graph transformation tool for external use, somewhat towards the vision presented in \cite{blostein95practicalissues}
-- to be employed in areas as diverse as computer linguistics, engineering, computational biology or software engineering --
for reasoning with semantic nets or knowledge graphs, transformation of natural language to UML models,
model transformation, processing of program graphs, genome simulation, or pattern matching in social nets (or RDF graphs).
Have a look at \cite{usecomputerlinguistics} (describing a usage in computer linguistics; but the dedicated tool grew \cite{grew:book} also supporting feature structures is maybe a better match in that field) or \cite{usemodeltransformation} or \cite{usegeneexpression} for some of the results.
More recent uses are from the field of computational design synthesis, with booggie \cite{booggie} as an early work, and \cite{graph:engineering} as a more current overview paper.
...
The generative approach and its \emph{compilation} of the rules into executable code is helping tremendously regarding performance, employing nested loops for matching the pattern elements (and a recursive descent based pushdown machine for combining the patterns, which gets optimized by one of the whole-program i.e. whole-rule-set optimizations with subpattern inlining; whole-world knowledge is brought into play also regarding other compiler functionalities).
...
-- besides being a help in modeling the domain, and besides easing your life by eliminating large classes of errors at compile time.
...
Another rich control language is offered by the port-graph rewriting tool Porgy \cite{porgy}, a less rich but visual one by the story diagrams \cite{storydiagrams} of Fujaba (similar to UML activity diagrams).
(Explicit control can be also exercised from a programming language of your choice, e.g. C\# \cite{csharp} or a .NET version of Python\cite{python}, utilizing the rules at .NET-API level, with the benefit that you don't have to learn a dedicated strategy language.)
...
While striving for performance, we still balance with simplicity.
This can be seen in the choice of the AA-tree\cite{Andersson93balancedsearch} as balanced binary search tree to implement the indices, and in the choice of the (also more general) multi-index queries instead of dedicated spatial indices.

\section{Understandability and Learnability}
was taken care of by evaluating for each language construct several options,
...
the ones which seemed most clean and intuitive while satisfying the other constraints were chosen (satisfying other constraints also includes fitting to the already available constructs to achieve conceptual integrity).
...
You may have a look at the GrGen.NET solution of the Hello World! case \cite{HelloWorld} to judge on your own (as of today, the solution would look even better because of the improved query support).
As we know that even the best designed language is not self explaining we put an emphasis on the \emph{user manual} currently read by you.
...
A further point easing development is the \emph{application programming interface} of the generated code,
...
Besides, textual languages bring some benefits on their own: graph transformation specifications to be processed by \GrG\ can be easily \emph{generated}, maybe by agentic coding tools like Claude Code\cite{claudecode}, stored in a source code management system, and \emph{textually diff'ed}.
...
And another remark about design: errors should be caught early.
\GrG{} supports you in this regard with its well-founded semantics (you could use for formal verification), its static type checking, and the means for validation/testing described in Chapter~\ref{chapvalidation}.

\section{Platform Independence}
is achieved by using languages compiled to byte code for virtual machines backed by large, standardized libraries, specifically: C\# and to a small degree Java.
...
Java is only needed to build \GrG{} itself (to generate some of its parsers) and run \yComp{}, the code generator as well as the generated code and its supporting libraries are entirely .NET based.
...
Many graph rewriting and model transformation tools are from the Java technical space, a few are from the .NET technical space, e.g. GraphSynth \cite{graphsynth}, some are native application e.g. written in C++ \cite{cplusplus}, but there you find more graph libraries and graph algorithm libraries than graph rewrite engines. %TODO some more cites, also add a bit on programming language design
In contrast to many of the graph based tools, \GrG{} has a strong background in formal language processing (a realm in which parser generator tools like ANTLR \cite{antlr} are used -- we use it to implement our domain specific languages in the frontend). %sprachtechnologie ... die wurde aber um machine-learning-based natural language processing erweitert ... todo weiter ausführen
...
persistence of online modifications is offered by writing to an embedded SQLite,
or by recording to a change log that needs to be replayed at next program start
...
A well equipped single machine with 128GB of main memory is capable of storing somewhat under about 500 Million nodes plus 1 Billion edges (naked, without attributes and without names), see Chapter \ref{cha:performance} for the maths.
This should be sufficient for many tasks, but still there are task that grow beyond it.
Besides, other things come into play at those sizes: importing such a graph will take some time, maybe longer than you're willing to wait -- the GRS importer is munching at some ten thousand elements per second.
...
\GrG\ is a tool that was built with achieving goals in practice in mind.
...
\GrG\ can be used for meta-programming (esp. due to its compiler construction roots), but otherwise you work with it at the object and class level, not at the meta, meta-meta, or meta-meta-meta tralala-ding-ding-dong level (the abstraction levels of the true model-based software engineers).
