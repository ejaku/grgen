...
Over the course of time it moved from high-performance fixed-shaped patterns and limited sequences for control to flexibly-shaped patterns and very high programmability, for one with the sequences and for the other with an integrated imperative programming language, while retaining its performance.

\GrG\ offers developement at the abstraction level of graph representations.
...
\begin{itemize}
	...
	\item debugability: the processing state and esp. the graph can be inspected, esp. graphically
\end{itemize}
 
In the following, we'll take a deeper look at the development goals of \GrG.
...
We're following a first things first policy here, and graphical debugging is a must-have, while a graphical rule editor is only nice-to-have (we consider tools offering the other way round toys, created by people without any own experience in graph-representation processing, just praying at the altar of some visual programming idea).
...
But they are also a good deal cheaper to implement.
...
They do so because the constructs make prooving propositions about the code much harder.
...
We optimized our languages for people that need to get real work done in them, not for people who want to proove that the program is up to a specification coded in predicate or some other kind of logic. 
...
\GrG\ can be used for meta-programming (esp. due to its compiler construction roots), but otherwise do you work with it at the object and class level, not at the meta, meta-meta, or meta-meta-meta tralala-ding-ding-dong level (the abstraction levels of the real model-based software engineers).
