...
Under LINUX you may use \texttt{make\_linux.sh} to get the parsers generated.
It was able to carry out a complete build with \texttt{mdtool} from MonoDevelop (the old way), but this does not work anymore since the switch to SDK-style projects (after version 8.0 of \GrG{}).
The new way is to employ VisualStudio Code with the Mono Debug extension; it uses \texttt{dotnet} for building ---you may also use \texttt{dotnet} directly from the command line for that purpose instead of from within VisualStudio Code/the C\#-extensions.
But note that for running/debugging, you still have to use \texttt{mono} (\GrG{} still targets the .NET framework, largely because of the WindowsForms GUI (a few adaptations yet missing would be needed regarding the compilation of the generated code so that the system core could also work with .NET standard 2.0)).
