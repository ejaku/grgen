...
\footnote{which may be special extensions for a dedicated task, but also may be of general interest to other users; note that \GrG{} is licensed under LGPL}
\end{itemize}
...
In case you want to build \GrG~on your own (which is available from the public git repository \texttt{https://github.com/ejaku/grgen}), you should recall the system layout \ref{figsys}. 
The graph rewrite generator consists of a frontend and a backend written in C\# \cite{csharp} (the \texttt{engine-net-2}).
The frontend orginally written in Java \cite{Java} was recently ported to C\#; it was extended and changed since its first version, but never replaced (the C\#-port is somewhat of an one-to-one mapping of the Java version).
...
a C version executing a search plan with a virtual machine, a C\# engine executing code generated from a search plan and finally the current C\# engine version 2 featuring attributes stored in the graph elements instead of in an extra object, and being capable of matching nested and subpatterns.

The frontend is available in the \texttt{frontend} subdirectory.
It contains a VisualStudio 2022 solution file \texttt{Frontend.sln}, that results in a \texttt{FrontendGrGen.exe} when built.

Before you can build the solution, you have to execute \texttt{./gen\-parser.bat} under Windows, or you have to execute \texttt{./gen\-parser.sh} under Linux in order to get the ANTLR parsers for the model and actions languages, and the embedded/compiled sequences generated.

The backend is available in the \texttt{engine-net-2} subdirectory. 
It contains a VisualStudio 2022 solution file \texttt{EngineNet2.sln} (if you want to use VisualStudio 2017, you have to remove the comments around the binding redirect of \texttt{System.Resources.Extensions} in the \texttt{app.config}s of the application projects), containing projects for the \texttt{lib\-Console\-And\-OS} environment abstraction library, the \texttt{libGr}, the \texttt{lgspBackend} (libGr-Search-Plan-Backend), the \texttt{graph\-Viewer\-And\-Sequence\-Debugger} library, the \texttt{lib\-Gr\-Shell}, the \texttt{GrGen} compiler (backend) as well as the \texttt{GrShell}.

Plus the WindowsForms versions/extensions \texttt{lib\-Console\-And\-OS\-Windows\-Forms}, the \texttt{graph\-Viewer\-And\-Sequence\-Debugger\-Windows\-Forms} library employing \MSAGL{}, as well as the WindowsForms \texttt{GGrShell} application.
Also available is a \texttt{lib\-Gr\-Persistence\-Provider\-SQLite} (implementing the persistent graph by mapping to an SQLite database --- in contrast to the previous database-based backends it is a strictly optional extension and only read once at session start, i.e. it is esp. not used for runtime pattern matching).

Before you can build the solution, you have to execute \texttt{./gen\-parsers.bat} under Windows (which will call \texttt{./src/lib\-Gr/gen\-parser.bat}, \texttt{./src/graph\-Viewer\-And\-Sequence\-Debugger/gen\-parser.bat}, \texttt{./src/lib\-Gr\-Shell/gen\-parser.bat}), or you have to execute \texttt{./gen\-parsers.sh} under Linux (which will call \texttt{./src/lib\-Gr/gen\-parser.sh}, \texttt{./src/graph\-Viewer\-And\-Sequence\-Debugger/gen\-parser.sh}, \texttt{./src/lib\-Gr\-Shell/gen\-parser.sh}) in order to get the CSharpCC parsers for the rewrite sequences, the constant literals, and the shell generated.

Under Windows you may use \texttt{make\_cygwin\_windows.sh} to get a complete build (re-generating the parsers each time, which takes long for the sequences parser).
But note that it employs \texttt{dotnet} for building, while regular releases are built manually with VisualStudio, i.e. \texttt{msbuild}.
...
It was able to carry out a complete build with \texttt{mdtool} from MonoDevelop (the old way), but this does not work anymore since the switch to SDK-style projects (after version 8.0 of \GrG{}) --- now \texttt{dotnet build} is used.
The new way of development under LINUX is to employ VisualStudio Code with the C\#-extensions and the Mono Debug extension; the \texttt{dotnet} tool is used then also.
Note that for running/debugging, you still have to use \texttt{mono} (\GrG{} still targets the .NET framework, largely because of the WindowsForms GUI (a few adaptations yet missing would be needed regarding the compilation of the generated code so that the system core could also work with .NET standard 2.0)).

On both systems, you may use \texttt{GrGen.sln} to build the complete code (frontend plus engine-net-2), but without parser generation.
%\pagebreak

The VisualStudio solutions contain build configurations for \texttt{x64}, \texttt{x86}, and \texttt{Any CPU}.
The binary release contains the binaries from the \texttt{x64} build only.
If you want to run \GrG\ in 32bit mode, or esp. want to include it in a native 32bit application, you have to build the assemblies on your own.
Before version 8.0, the \texttt{Any CPU} build was released, this changed because of the introduction of the persistent graph.
The SQLite used by the persistent graph cannot be built as \texttt{Any CPU}, most likely because the native binaries for the matching platform have to be fetched (the rest of the project can be built as \texttt{Any CPU}, you can still use it, only the persistent graph may be unavailable).
The corresponding nuget-package contains builds for quite some platforms, so it should be not much of an issue to get e.g. an ARM-version of \GrGen\ including the persistent graph running.
...
The \texttt{engine-net-2} subdirectory contains a \texttt{GrGen-API.shfbproj} file that is used by the Sandcastle Help File Builder to build the API documentation.
The \texttt{syntaxhighlighting} subdirectory contains syntax highlighting specifications for the \texttt{.gm},\texttt{.grg}, and \texttt{.grs}-files for \texttt{Notepad++}, \texttt{vim}, and \texttt{Emacs} (the \texttt{N++} versions are typically more up-to-date/complete than the others).

\pagebreak


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{How to Test}\label{sub:testing}

You can check the result of your build with the test suite we use to check against regressions.
It is split into syntactic (/frontend) tests checking language acceptance by the compiler
and semantic (/backend) tests checking the execution semantics of the languages.

The syntactic tests reside in \texttt{frontend/test} (the \texttt{test} folder in the binary release), they are checking that example model and rule files can get compiled by \texttt{grgen} (or not compiled, or only compiled emitting warnings) and that the resulting code can get compiled by \texttt{csc}.
The tests get executed by calling \texttt{./test.sh} or \texttt{./testpar.sh} from \texttt{bash} or \texttt{cygwin-bash} (\texttt{testpar.sh} executes them in parallel speeding up execution on multi core systems considerably, at the price of potential false positive reports); deviations from a gold standard are reported (at the time of writing, 3 mathematics tests are failing under mono/LINUX because of minor numeric differences).

The semantic tests reside in \texttt{engine-net-2/tests} (the \texttt{tests} folder in the binary release), they are checking that executing example shell scripts on example models and rules yields the expected results (utilizing the outputs of the commands described in Chapter~\ref{chapvalidation}).
They get executed by calling \texttt{./test.sh}.

The examples from \texttt{engine-net-2/examples} can be used as smoke tests by the contained \texttt{./test.sh} (under Windows).

For a very thorough test coverage, you could manually force parallelization of all pattern matchers in the frontend code (search for \texttt{uncomment} in \texttt{Unit.cs}, or you could manually force profiling instrumentation of all pattern matchers (search for \texttt{uncomment} in \texttt{GrGen.cs}).

A unit test suite was recently added to the \texttt{engine-net-2} folder (in the \texttt{unittests} subfolder) and to the solution file, you may run it directly in VisualStudio 2022 or use \texttt{dotnet test}.
It is based on NUnit3 
\footnote{you may need NUnit2 when you want to execute your own tests from a native application like AutoCAD hosting your graph rewrite system --- but beware of outdated knowledge}
and very limited in what it covers as of now.

A unit test suite was also recently added to the \texttt{frontend} folder (in the \texttt{unittest} subfolder), you may run it directly in VisualStudio 2022 or use \texttt{dotnet test}.
It is based on NUnit3 and comprises a small set of quick-check acceptance tests as of now (so also limited coverage).

The application tests checking that the languages work as expected (on the outside) had priority over checking internal components --- but the unit tests are to be extended, as they allow finding errors much earlier than the syntax/semantics test suites that take a long time to execute.
...
At the top level you find type ringlists, every node or edge is contained in a linked list of its specific type.
...
The node object contains a field \texttt{inHead} referencing an arbitrary edge object of the incoming edges (or \texttt{null} if there is none) and a field \texttt{outHead} referencing an arbitrary edge object of the outgoing edges (or \texttt{null} if there is none).
...
Other matching operations besides graph lookup and incoming/outgoing or source/target following are storage access, storage attribute access and storage mapping as well as attribute index access and name/unique index access.
...
It contains explicit instructions for isomorphy checking and connectedness checking (which depend on the exact schedule).
...
The frontend is spread over the directories \texttt{parser}, \texttt{ast}, \texttt{ir}, \texttt{be} and \texttt{util}, with their code being used from \texttt{Main.cs}.
...
Finally, an intermediate representation is built from the abstract syntax tree by reading from the \texttt{IR} property and calling \texttt{ConstructIR} methods.
...
A particularly interesting one is \texttt{Ensure\-Directly\-Nesting\-Pattern\-Contains\-All\-Non\-Local\-Elements\-Of\-Nested\-Pattern} from \texttt{Unit.cs}, it ensures that, from a pattern which contains a certain entity the first time, up to every pattern that references this entity, all intermediate patterns contain that entity, too.
...
This function allows keeping the backends simple.

The IR classes are the input to the backend of the frontend, which can be found in the \texttt{be/Csharp} folder
\footnote{
The directory \texttt{be/C} is only contained in the SCM history anymore, it contains the code generator for the C based backend, which is integrated into the IPD C compiler.
...
\subsubsection*{Backend (of the Frontend)}
...
The backend of the frontend does \emph{not} generate the complete source code for the rules including the matcher code or the code for the embedded rewrite sequences --- this is done by \texttt{grgen.exe}, which calls the \texttt{FrontendGrGen.exe} (formerly \texttt{grgen.jar}) of the frontend (generating a \texttt{Foo\-Actions.cs}).
You may call it on your own in order to get a visualization of the model and rewrite rules, in the form of a \texttt{.vcg}-dump of the IR, cf. Note\ref{note:modelruledump}.


\subsubsection*{Model Generation}

%potential todo: separate more clearly the explained/mentioned constructs from the code generator from the constructs in the generated code that are explained/mentioned: \texttt{\emph{Modify\-Gen}}

The code generation emitting the model is implemented in \texttt{Model\-Gen} (entry point \texttt{Gen\-Model}), it is rather straight forward:
\begin{enumerate}
	...
	\item and then the object and transient object classes are generated.
\end{enumerate}
This occurs for the global namespace and also by-package for the available model packages.

\noindent This is followed by the generation of
\begin{enumerate}
	\item the node model,
	\item the edge model,
	\item the object and transient object model,
	\item and finally, the graph model.
\end{enumerate}
...
Additionally, the (database-like) indices are generated by \texttt{Model\-Index\-Gen}, and functionality regarding external extensions by \texttt{Model\-External\-Gen}.
\texttt{Model\-Index\-Gen} generates first the index types with \texttt{Gen\-Index\-Types} (they inherit from \texttt{IAttribute\-Index} or \texttt{IIncidence\-Count\-Index}, and contain diverse \texttt{Lookup} functions), then the index implementations with \texttt{Gen\-Index\-Implementations} (each being an implementation of an AA-tree consisting of event handlers that are notified about graph changes, the maintenance functions for insertion, deletion and tree rebalancing, and the implementation of the lookup functions from the interface returning data from the tree), and finally the overall index set with \texttt{Gen\-Index\-Set\-Type}.


\subsubsection*{Rule Representation Pass}

The code generation emitting the actions -- better their representation -- is implemented in \texttt{ActionsGen} (entry point \texttt{Gen\-Actionlike}), with the main functionality (the LHS pattern descriptions and the RHS pattern implementations, in this section we concentrate on the former) in the code called from \texttt{Gen\-Subpattern} for the subpatterns and \texttt{Gen\-Action} for the rules and tests.

The main functionality is complemented by the code called from \texttt{Gen\-Sequence}, \texttt{Gen\-Functions}, \texttt{Gen\-Procedures}, \texttt{Gen\-Filter\-Functions}, \texttt{Gen\-Match\-Class\-Filter\-Functions}, and finally \texttt{Gen\-Match\-Classes}.
This occurs for the global namespace and also by-package for the available action packages.

Then the representation classes of the external functions and procedures are generated with \texttt{Gen\-External\-Function\-Infos} and \texttt{Gen\-External\-Procedure\-Infos}, and finally the actions representation class (which is inheriting from \texttt{LGSP\-Rule\-And\-Matching\-Patterns}).
...
In the rule representation pass, from \texttt{Gen\-Rule\-Or\-Subpattern\-Init} on, the subpattern- or rule representations of type \texttt{Matching\-Pattern} or \texttt{Rule\-Pattern} are generated, including the contained \texttt{Pattern\-Graph}-objects for the (nested) pattern graph(s), by mutually recursive calls of \texttt{Gen\-Pattern\-Graph} and \texttt{Gen\-Elements\-Required\-By\-Pattern\-Graph} (the former assembling the rule representation with the elements generated by the latter).

The method \texttt{Gen\-Elements\-Required\-By\-Pattern\-Graph} generates for a pattern graph its contained elements, the pattern nodes, the pattern edges, the used subpatterns as \texttt{Pattern\-Graph\-Embedding} members, the contained alternatives, and the iterateds; the representations of contained negative and independent patterns as well as alternative cases are generated by \texttt{Gen\-Pattern\-Graph} before the containing pattern. 
...
The \texttt{Pattern\-Graph} in which it was used first is remembered in a \texttt{Point\-Of\-Definition} member variable (in the generated code, but a similar scheme is used in the compiler frontend).
...
As all graph elements, including the ones of the nested patterns, are created flatly in the \texttt{initialize}-method of the \texttt{Matching\-Pattern} (in the generated code), name prefixes are needed to prevent name clashes; this is ensured by the parameter \texttt{path\-Prefix}.
Correct naming of already declared elements is ensured with an \texttt{already\-Defined\-Entity\-To\-Name}-hash-table.
...
Additionally, expression trees for the attribute checks and statement blocks for the yieldings are generated, by \texttt{Gen\-Expression\-Tree} and \texttt{Gen\-Yield} from \texttt{Actions\-Expression\-Or\-Yielding\-Gen}, called by \texttt{Gen\-Pattern\-Condition} and \texttt{Gen\-Pattern\-Yielding}; the entities referenced in them are gathered by \texttt{Collect\-Needed\-Entities}.
The real code for these representations is emitted in the \texttt{engine-net-2}-backend as is the real code (the pattern matchers) for the pattern representations.

Then the rewrite code pass is executed (explained in more detail further below), followed by the handling of embedded sequences.

For embedded sequences, representations in form of \texttt{LGSP\-Embedded\-Sequence\-Info} objects are generated, which contain the sequence as string, plus additional parameter information; the real sequence code is generated in the \texttt{engine-net-2} backend.
...
This is carried out in \texttt{ActionsExecGen}; the representations of the non-embedded defined sequences bearing a name are directly emitted in \texttt{Gen\-Sequence} of \texttt{ActionsGen}.

Finally (the exact) match classes are generated by calls of \texttt{Gen\-Match} utilizing \texttt{Actions\-Match\-Gen}, for every pattern besides negative patterns an interface and a class implementing this interface (they are not to be confused with the match class types defined by the user directly, getting emitted by calls of \texttt{Gen\-Match\-Class}, utilizing the same \texttt{Actions\-Match\-Gen}).
...
Note that \texttt{Gen\-Function}, \texttt{Gen\-Procedure}, \texttt{Gen\-Filter\-Function}, and \texttt{Gen\-Match\-Class\-Filter\-Function} are largely using \texttt{Modify\-Eval\-Gen} to generate the body with their functionality (besides their interfaces/representations), the procedures that may contain embedded sequences also utilize \texttt{Modify\-Exec\-Gen}.

For the actions in the narrow sense (rules and tests), some further (match processing) helper code is emitted by \texttt{Gen\-Extractor}, \texttt{Emit\-Convert\-As\-Needed\-Helper} and \texttt{Gen\-Array\-Sort\-By}.


\subsubsection*{Rewrite Code Pass}
In the rewrite pass (a virtual pass woven into the code generation, not being a separate top-level pass in its strict sense in the code generation), the rewrite code gets generated, via \texttt{Gen\-Modify} of \texttt{Modify\-Gen}.
...
The \texttt{Modify\-Generation\-State} holds the state during generation, while the \texttt{Modify\-Generation\-Task}s allow to give a left and right graph independent of the rewriting specified with the rule (so the creation and deletion code can be generated in addition to the dependent rewrite as explained in the following).
...
The code for dependent rewriting is generated by simply referencing the left and right pattern graphs from \texttt{IR::Rule}.
...
%potential todo: crisp and uniform usage of pattern and graph, the basic scheme is pattern matched in graph (non-graph pattern? when speaking about rules as in the actions code-generation always only patterns?)
...
Their role is distinguished by one of the name postfixes \texttt{Modify}, \texttt{Create} or \texttt{Delete}; for keeping a graph unmodified obviously no methods are needed.

Also during the rewrite pass, the statements of the \texttt{eval} blocks are generated, this happens in \texttt{Modify\-Eval\-Gen}, and the \emph{calls of the routines} implementing the embedded \texttt{exec}s, in \texttt{Modify\-Exec\-Gen} (the non-embedded sequences are called explicitly, no implicit calls have to be inserted automatically).

Note that the non-pattern-matching code (the rewrite part plus functions, procedures and alike) that will be executed is emitted directly (in \texttt{Modify\-Gen} and \texttt{Modify\-Eval\-Gen}), instead of only a representation that is the input to further processing in the \texttt{engine-net-2} backend, as is the case for the rule/test/subpattern descriptions including the attribute conditions and yieldings (the parts that can be recompiled at runtime; also the real sequences implementation is part of the \texttt{engine-net-2} backend).
The code to emit the expression code is contained in \texttt{CSharp\-Base}, the \texttt{Modify\-Eval\-Gen} extends it by the code for the statements.
A lot of non-trivial functionality is mapped to calls of functions supplied by the runtime (many helper classes from the \texttt{libGr}, some from the \texttt{lgspBackend}); besides, some further code generation exists, e.g. to fill constant sets or maps or to implement the functions with lambda-expression parameters.
% TODO: the paragraph may duplicate content that was already explained elsewhere
...
The generation is carried out in several passes, use as entry points
\begin{itemize}
	\item \texttt{Generate\-And\-Insert\-Matcher\-Source\-Code} from \texttt{lgspGrGen.cs} in case of the static compilation of the actions dll
	\item the \texttt{Generate\-Actions} function from the \texttt{lgsp\-Matcher\-Generator.cs} in case of dynamic matcher recompilation at runtime
\end{itemize}
...
(This allows the user to extract common patterns into subpatterns increasing readability and modularity without losing performance (but as of now only one level of inlining is supported, so excessive subpattern extraction has to be avoided, especially if the containing pattern would become disconnected by this)). 
Before and after this kind of pattern rewriting, the patterns and their relations are analyzed by the \texttt{Pattern\-Graph\-Analyzer} -- the analyzation results are used for emitting better code later on (choosing more efficient but more limited implementations of language features, which are not sufficient in the general case but are so for the specification at hand).
The \texttt{Pattern\-Graph\-Analyzer} that also contains the inlining functionality
is called from \texttt{Analyze\-And\-Inline\-Matching\-Patterns} in \texttt{lgspGrGen.cs} in case of static compilation based on the results emitted by the frontend,
it is called from the generated actions code in \texttt{Init\-Actions} at runtime, its results are then used in case of dynamic matcher recompilation.
...
Independents are inlined during plan graph creation unless the feature is disabled, the main part of independent inlining is occurring in \texttt{Generate\-Static\-Plan\-Graph} or \texttt{Generate\-Plan\-Graph} of the \texttt{Plan\-Graph\-Generator}, when \texttt{inline\-Independents} is requested.
...
This is carried out in \texttt{Generate\-Scheduled\-Search\-Plans} in \texttt{lgspGrGen.cs} during compilation or in \texttt{Generate\-Scheduled\-Search\-Plans} in \texttt{lgsp\-Matcher\-Generator.cs} during runtime recompilation.
...
Finally, in a third step, code is generated by \texttt{Generate\-Action\-And\-Matcher} (from \texttt{lgsp\-Matcher\-Generator.cs}),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Further Backend Parts and Applications}

The \texttt{engine-net-2} folder contains besides the pattern matching parts of the backend code generator also the implementation of the graph rewrite sequences, the language runtime, the \GrShell{} with the debugger, and further libraries.


\subsubsection*{libGr and lgspBackend}

The \texttt{src/libGr} subfolder contains the libGr, offering the base interfaces you see on the API level for the model, the actions and the pattern graphs, as well as the host graph.
...
and is building a graph rewrite sequence AST out of a sequence string, from the classes in \texttt{Sequence.cs} (and \texttt{Sequence\-Base.cs} plus \texttt{Sequence\-Computation.cs} and \texttt{Sequence\-Expression.cs}), further utilizing a \texttt{Symbol\-Table} and the \texttt{Sequence\-Parser\-Environment}.
After building, the sequence AST is type checked by the \texttt{Sequence\-Checking\-Environment}, which -- like the parser environment -- comes in a version for the interpreted sequences and a version for the compiled sequences, abstracting away the differences in available (type) information while generating the action code (compiled) compared to just using the generated action code (interpreted).

The graph rewrite sequence classes contain a method \texttt{ApplyImpl(IGraph\-Processing\-Environment procEnv)}, which is called for executing/interpreting the sequence.
The compiled graph rewrite sequences instead are emitted by the \texttt{lgsp\-Sequence\-Generator} as C\# source code.
...
Graph transaction handling is implemented in \texttt{lgsp\-Transaction\-Manager.cs}, with a list of undo items, which know how to undo the effect that created them.
...
The runtime library \texttt{libGr} further contains the interface \texttt{IGraph\-Processing\-Environment} to the graph processing environment (built up from a series of environment interfaces with increasing functionality), offering e.g. graph global variables, sequence execution, convenient action execution, as well as graph change recording and transaction handling (implemented by the aforementioned classes in the \texttt{lgsp\-Backend}).
Additionally, it implements helper code in \texttt{XXXHelper.cs} files.

The \texttt{Container\-Helper.cs} contains code that is common for all container types (cf. Chapter~\ref{cha:container}), the \texttt{Dictionary\-Helper.cs} contains code that is common for sets and maps, while the \texttt{Sequential\-Type\-Helper.cs} contains code that is common for array and deques; code specific to each container type is available in correspondingly named files.
The \texttt{Graph\-Helper.cs} contains code that implements the diverse graph query functions and graph manipulation procedures (cf. Chapter~\ref{cha:graph}), while the \texttt{Graph\-Helper\-Parallel.cs} contains code that implements the diverse graph query functions in versions that can be employed from parallelized pattern matchers (which are not allowed to use the isomorphy checking flags contained in the graph elements that are employed by the non-parallel pattern matchers).
...
The \texttt{src/GrShell} subdirectory contains the \GrShell{} application.
The bulk of the implementation is contained in the \LibGrShell{} in \texttt{src/libGrShell}, though.
Its command line parser is generated by CSharpCC out of the \texttt{GrShell.csc} grammar file, the shell implementation/functionality is given in \texttt{GrShell\-Impl.cs}, execution starts in the \texttt{Main} method in \texttt{GrShell\-Main.cs}.
The library and accordingly the shell builds upon the sequence interpretation facilities offered by the \LibGr{} and by the \texttt{GrShell\-Sequence\-Applier\-And\-Debugger.cs}, and the "generic" interface of the \LibGr{}, because it has to be capable of coping with arbitrary user defined models and actions at runtime.
Another shell application on top of the \LibGrShell{} is the \GGrShell{}, the graphical \GrShell{}, which is a Windows Forms application based on a \texttt{Gui\-Console\-Control} (controlled by a GUI message loop).

Stepwise and graphical debugging is offered by the \texttt{Debugger.cs} and several helper classes, esp. including the \texttt{Graph\-Viewer\-Client}, which contains higher-level graph viewer functionality (with parts of the visualization configuration and the implementation of the hierarchical node grouping specification), delegating to a lower-level \texttt{basic\-Client} implementing the \texttt{IBasic\-Graph\-Viewer\-Client} interface for real graph rendering.
Further notable is the \texttt{Graph\-Viewer} class, an auxiliary class with helper methods for viewing and dumping graph (which in turn delegates most of the tasks to the \texttt{Graph\-Viewer\-Client}).
One of the usable basic clients implementing \texttt{IBasic\-Graph\-Viewer\-Client} is the \texttt{YComp\-Client.cs}, it communicates with the \yComp{} application with a custom protocol over a TCP connection to localhost.
They are contained in the \GraphViewerAndSequenceDebugger{} library.

Another graph viewer is offered by the \texttt{MSAGL\-Client.cs} and its extended version, the \texttt{MSAGL\-Ext\-Client.cs}, they also implement the \texttt{IBasic\-Graph\-Viewer\-Client} interface.
They are contained in the \GraphViewerAndSequenceDebuggerWindowsForms{} library, together with the \texttt{Gui\-Debugger\-Host.cs}, which offers a GUI debugger (based on the same debugger application logic used by the console debugger, just embedded into a different hosting environment).

The dlls are loaded on demand, and can be used from API level; you could implement a debugger for another GUI framework and OS with the same mechanisms.
%maybe TODO: explain some parts in more detail


\subsubsection*{Further Libraries and Applications}

The \LibGrPersistenceProviderSQLite{} offers an implementation of the persistent graph based on the SQLite database.

The \LibConsoleAndOS{} is a console and OS abstraction library defining console interfaces and a mapping to the normal stdout/stdin console.

The \LibConsoleAndOSWindowsForms{} library offers a Windows Forms implementation of the console interface (based on a \texttt{RichTextBox}).

You could implement a persistence provider for another database or a console implementation for another GUI framework and OS. 

You find more detailed information (on them) in the \texttt{CLAUDE.md} files in the project directories.
...
The \texttt{examples} subdirectory contains a bunch of examples for using \GrG~with \GrShell{}.
