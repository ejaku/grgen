...
The graph rewrite generator consists of a frontend written in Java \cite{Java} and a backend written in C\# \cite{csharp}.
...
The VisualStudio solution contains build configurations for \texttt{x64}, \texttt{x86}, and \texttt{Any CPU}.
...
For a very thorough test coverage, you could manually force parallelization of all pattern matchers in the frontend code (search for \texttt{uncomment} in \texttt{Unit.java}, or you could manually force profiling instrumentation of all pattern matchers (search for \texttt{uncomment} in \texttt{GrGen.cs}).
...
A unit test suite was also recently added to the \texttt{frontend} folder (in the \texttt{unittest} subfolder), you may run it directly in Eclipse or use \texttt{unittest/make\_unittest.sh} under Linux (the bash script also calls make on the frontend and builds the unit-tests).
It is based on JUnit 4 and comprises a small set of quick-check acceptance tests as of now (so also limited coverage).

The application tests checking that the languages work as expected (on the outside) had priority over checking internal components --- but the unit tests are to be extended, as they allow you to find errors much earlier than the syntax/semantics test suites that take a long time to execute.
...
A particularly interesting one is \texttt{ensure\-Directly\-Nesting\-Pattern\-Contains\-All\-Non\-Local\-Elements\-Of\-Nested\-Pattern} from \texttt{Unit.java}, it ensures that, from a pattern which contains a certain entity the first time, up to every pattern that references this entity, all intermediate patterns contain that entity, too.
...
\subsubsection*{Backends (of the Frontend)}
...
The backend of the frontend does \emph{not} generate the complete source code for the rules including the matcher code or the code for the embedded rewrite sequences --- this is done by \texttt{grgen.exe}, which calls the \texttt{grgen.jar} of the frontend (generating a \texttt{Foo\-Actions.cs}).
