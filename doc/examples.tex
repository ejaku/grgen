...
The machine's current configuration is modeled with an \texttt{RWHead} edge pointing to a \texttt{TapePosition} node.
...
You can see the difference between the search plans causing that improvement in execution time by utilizing the \texttt{explain} command.
...
The crucial difference is the changed lookup, the rule begins matching at the single \texttt{rwHead} edge instead of one of the many \texttt{right} edges on the tape.
