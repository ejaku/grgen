...
It lists the ways you can interact with the external world outside of \GrG.
...
You may want to use them for performance reasons, to implement linked lists or trees more efficiently than with the nodes and edges supplied by \GrG\{} -- but regarding this, you can use internal object classes\ref{sec:objecttypes} by now.
...
You may extend \GrG~with external attribute types if the built-in attribute types (cf. \ref{sec:builtintypes}) are insufficient for your needs because you need functionality not related to graph-rewriting (in order to just implement lists or trees more efficiently than with the node and edge types you may use the internal (object) class type instead).
The external types can be explicitly cast to \texttt{object} but are not implicitly cast.
...
The external functions and procedures declared within the body of the external class are \emph{methods}, in contrast to the external functions and procedures from the global namespace that use the same declaration syntax that are introduced in the following sections.
...
It allows inspecting types not defined in \GrG~in the debugger (incl. yComp),
and it allows serializing/deserializing graphs with types not defined in \GrG.
...
This allows for a tight integration of external types with the built-in functionality, 
and esp. allows implementing value semantics (objects are normally compared by checking reference identity and cloned by copying the reference).
