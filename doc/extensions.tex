...
You may want to use them for performance reasons, to implement linked lists or trees more efficiently than with the nodes and edges supplied by \GrG.
...
It allows you to inspect types not defined in \GrG~in the debugger (incl. yComp),
and it allows you to serialize/deserialize graphs with types not defined in \GrG.
...
This allows for a tight integration of external types with the built-in functionality, 
and esp. allows you to implement value semantics (objects are normally compared by checking reference identity and cloned by copying the reference).
