...
\begin{rail}
  CopyCompareDeclaration: 'copy' 'class' ';';
\end{rail}\ixnterm{CopyCompareDeclaration}\ixkeyw{copy}\ixkeyw{class}
Tells \GrG~that the developer wants to extend graph element copying and graph isomorphy comparison, so that they can handle external types, or the type \texttt{object}.
This allows for a tight integration of external types with the built-in functionality, 
