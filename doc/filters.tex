...
\begin{description}
\item[\PVerb{\assign<member>{element:match<r> -> lambda-expr}}] assigns to the \texttt{member:T} of a match of a rule \texttt{r} the result of the sequence lambda expression fed by the \texttt{element:match<r>} of the matches (the name can be chosen freely), for each element in the matches yielded by the rule.
\item[\PVerb{\removeIf{element:match<r> -> lambda-expr}}] removes the matches for which the predicate given by the sequence lambda expression (which must be of type \texttt{boolean}) fed by the \texttt{element:match<r>} of the matches (the name can be chosen freely) yields \texttt{true}.
\end{description}
...
\verb#\mc.removeIf{element:match<class mc> -> lambda-expr}#.\\
...
\verb#\removeIf{element:match<r.it> -> lambda-expr}#.

They all allow for an optional index: the variable \texttt{index:int} (the name can be chosen freely) iterates the matches-array indices [0,a.size()-1] of the corresponding elements in the array \texttt{a:array<match<r>>} in the\\ \verb#a.assign<member>{index:int -> element:match<r> -> lambda-expr}# and\\
\verb#a.removeIf{index:int -> element:match<r> -> lambda-expr}# expressions.
...
\hline
\verb#assign<entity>{[index:int->]elem:T->lambda-expr}#\\
\verb#removeIf{[index:int->]elem:T->lambda-expr}#\\
\hline
\end{tabular}
\\
