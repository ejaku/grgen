...
But more important are the large number of global functions (with call syntax as already inroduced in \ref{sec:primexpr}) and global procedures (with call syntax as already inroduced in \ref{sec:proccall}) that implicitely operate on the single host graph (and thus on the graph type).
...
Functions that allow to compute (node-or-edge) induced subgraphs, and procedures to insert clones of induced subgraphs are available.
...
They may be queried with a function of the expressions, and set or administered with procedures of the computations.
