...
But more important are the large number of global functions (with call syntax as already introduced in \ref{sec:primexpr}) and global procedures (with call syntax as already introduced in \ref{sub:proccall}) that implicitly operate on the single host graph (and thus on the graph type).
...
There are procedures available to update the graph by adding or removing or retyping, on nodes and edges:
...
\noindent Besides those basic graph manipulation functions, some advanced rewriting operations are available as procedures, too:
...
\begin{description}
\item[\texttt{for(n:NodeType in nodes(NodeType)) \{Statements\}} ] iterates over all nodes in the graph compatible to the given node type.
...
\begin{description}
	\item[Set functions:] The neighbourhood function returns a set of neighbouring entities of the start node. It builds a set that is likely thrown away thereafter, so esp. for large sets this function is less efficient than the other versions.
\begin{rail}
Expression:
  FunctionName '(' StartNodeExpr ')' ;
\end{rail}

	\item[Iteration loops:] The neighbourhood function is employed from a for loop (cf.~\ref{sub:controlflow}) that allows to iterate the neighbouring entities of the start node. No set needs to be built here. But if the source node has multiple edges to a target node, it might be iterated multiple times. And an edge may be iterated twice in case of the undirected functions.
\begin{rail}
  ForFunctionHeader: 'for' '(' Var ':' Type 'in' FunctionName '(' Arguments ')' ')';
\end{rail}\ixkeyw{in}\ixkeyw{for}\ixnterm{ForFunctionHeader}
Such a for graph accessing function loop is also available for the nodes/edges functions introduced above,
and many of the following (graph query by neighbourhood, including the transitive, only excluding the with-remaining-depth) functions, defining a general way of accessing the graph elements from the host graph.
...
\end{rail}
The \emph{CountedFunctionName} is built from the \emph{FunctionName} by prepending \texttt{count} and switching the first character of \emph{FunctionName} to upper case.

	\item[Boolean functions:] The neighbourhood function is employed in a boolean predicate version that allows to check whether a second target entity is in the queried neighbourhood of the start node. The computation has the smallest internal processing overhead of the three options and stops as soon as a positive result is obtained.
\begin{rail}
Expression:
  FunctionNamePredicate '(' StartNodeExpr ',' TargetEntity ')' ;
\end{rail}
The \emph{FunctionNamePredicate} is built from the \emph{FunctionName} by prepending \texttt{is} and switching the first character of \emph{FunctionName} to upper case.
\end{description}
...
procedure example
{
	...
	{
		x.visited = true; // see the section on Visited Flags
...
The \texttt{import} function is similar to the \texttt{import} command from the GrShell, see Section~\ref{outputcmds}, applied to a \texttt{.grs}-file. It returns a graph object for further processing instead of replacing the host graph.
...
The \texttt{export} procedure behaves like the \texttt{export} command from the GrShell, see Section~\ref{outputcmds}, applied to a \texttt{.grs}-file. It allows for programmed exporting, and exporting of parts of the graph alongside subgraph containment.

% todo: mehr beispiele
% todo: connected components aufwändig programmiert berechnen statt einfach mit reachable (und strongly connected components von weak components unterscheiden, evt. examples/WeakComponents von Interesse)
...
You may furthermore check for (sub)graph equality against a set of (sub)graphs at once:
...
The following syntax diagram gives an extension to the \emph{Expression} clause of chapter \ref{cha:typeexpr} and an extension to the computation \emph{Statements} of chapter \ref{cha:computations}:
