...
\noindent Besidse those basic graph manipulation functions, the advanced rewriting operations are available as statements, too:
...
%-----------------------------------------------------------------------------------------------
\subsection{Induced Subgraph Computation and Insertion to Host Graph}
...
An example implementing \indexed{depth-first search} and \indexed{breadth-first search} as computations of the rule language utilizing the constructs introduced so far can be found in \texttt{test/should\_pass}, in \texttt{/DfsBfsSearch.grg}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Transaction Handling}\label{sub:transaction}

In addition to the functions implemented directly in the graph there are the transaction manager functions available, implemented in the graph processing environment.
While a transaction is underway, an undo log is filled with commands to undo the changes that occured in the graph in the meantime.
Those transaction handling functions are:
