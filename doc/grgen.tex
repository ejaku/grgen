...
{morekeywords={actions,using,rule,test,pattern,replace,if,eval,negative,node,edge,path,graph,modify,delete,class,model,connect,extends,enum,abstract,const,exec,emit},
sensitive=true,
morecomment=[l]{//},
morecomment=[s]{/*}{*/},
morestring=[b]",
...
\begin{titlepage}
  ...
  \LARGE Refers to \GrG\ Release 1.4\\[1ex]
  \LARGE ---DRAFT---\\[1ex]
...
\chapter*{Abstract}
\GrG\ is a graph rewrite tool enabling elegant and convenient development of graph rewriting applications with comparable performance to conventionally developed ones.
...
To get an prototype quickly, we delegated the costly task of subgraph matching to a relational database system~\cite{Hac:03}.
...
\noindent Karlsruhe in July 2007, the authors of \GrG\\
