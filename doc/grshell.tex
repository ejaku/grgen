...
\GrShell\ is a \indexedsee{shell}{GrShell} application of \LibGr\indexmain{libGr}. It belongs to \GrG's standard equipment. \GrShell\ is capable of creating, manipulating, and dumping graphs as well as performing graph rewriting and debugging graph rewriting.
...
\GrShell\ is \indexed{case sensitive}. A \indexed{comment} starts with a \indexed{\texttt{\#}} and is terminated by end-of-line or end-of-file. Any text left of the \emph{\#} will be treated as a statement.\\
...
\begin{description}
  ...
  \item[Color] One of the following \indexed{color} identifiers: \texttt{Black}, \texttt{Blue}, \texttt{Green}, \texttt{Cyan}, \texttt{Red}, \texttt{Purple}, \texttt{Brown}, \texttt{Grey}, \texttt{LightGrey}, \texttt{LightBlue}, \texttt{LightGreen}, \texttt{LightCyan}, \texttt{LightRed}, \texttt{LightPurple}, \texttt{Yellow}, \texttt{White}, \texttt{DarkBlue}, \texttt{DarkRed}, \texttt{DarkGreen}, \texttt{DarkYellow}, \texttt{DarkMagenta}, \texttt{DarkCyan}, \texttt{Gold}, \texttt{Lilac}, \texttt{Turquoise}, \texttt{Aquamarine}, \texttt{Khaki}, \texttt{Pink}, \texttt{Orange}, \texttt{Orchid}. These are the same color identifiers as in \indexed{VCG}/\yComp\ files.
\end{description}
...
\begin{example}
\label{persistentex} 
We insert a node, \indexed{anonymous}ly and with a \indexed{constructor}:
\begin{grshell}
...
\begin{rail}
  'help'
\end{rail}\indexmain{\texttt{help}}
Displays an information message describing supported commands. 

\begin{rail}
  'quit' | 'exit'
\end{rail}\indexmain{\texttt{quit}}\indexmain{\texttt{exit}}
...
\end{rail}\indexmain{\texttt{select}}\indexmain{\texttt{backend}}
Selects a \indexed{backend} that handles graph and rules representation. \emph{Filename} has to be a .NET assembly (e.g.\ \texttt{\indexed{lgspBackend.dll}}).
Comma-separated \indexed{parameter}s can be supplied optionally. If so, the backend must support these parameters.

\begin{rail}
  'show' 'backend'
\end{rail}\indexmain{\texttt{show}}\indexmain{\texttt{backend}}
...
\begin{rail}
  'include' Filename
\end{rail}\indexmain{\texttt{include}}
Executes the \GrShell\ script\indexmain{graph rewrite script} \emph{Filename}. A \GrShell\ script is just a plain text file containing \GrShell\ commands. They are treated as they would be entered interactively, except for parser errors. If an parser error occurs, execution of the script will stop immediately.

\begin{rail}
  'debug' ( 'enable' | 'disable' )
\end{rail}\indexmain{\texttt{debug}}\indexmain{\texttt{enable}}\indexmain{\texttt{disable}}
...
\end{rail}\indexmain{\texttt{debug}}\indexmain{\texttt{set}}\indexmain{\texttt{layout}}
...
\begin{rail}
  'echo' Text
\end{rail}\indexmain{\texttt{echo}}
Prints \emph{Text} onto the \GrShell\ command prompt.

\begin{rail}
  '!' CommandLine
\end{rail}\indexmain{\texttt{"!}}
Passes \emph{CommandLine}\indexmain{command line} to the operating system. \emph{CommandLine} is an arbitrary text the operating system attempts to execute.
\begin{example}
...
\end{rail}\indexmain{\texttt{new}}\indexmain{\texttt{graph}}
...
\end{rail}\indexmain{\texttt{open}}\indexmain{\texttt{graph}}
Opens the graph \emph{Text} stored in the backend. However, the \emph{LGSPBackend} doesn't support \indexed{persistent graph}s. \emph{LGSPBackend} is the only backend so far. Therefore this command is currently useless.

\begin{rail}
  'show' 'graphs'
\end{rail}\indexmain{\texttt{show}}\indexmain{\texttt{graph}}
Displays a list of currently available graphs.

\begin{rail}
  'select' 'graph' Graph
\end{rail}\indexmain{\texttt{select}}\indexmain{\texttt{graph}}
Selects the current \indexed{working graph}. This graph act as \emph{host graph} for regular graph rewrite sequences (see also sections \ref{ov:whatsallabout} and \ref{grsthings}). Though you can define multiple graphs, only one graph can be the active ``working graph''.

\begin{rail}
  'clear' 'graph' (() | Graph)
\end{rail}\indexmain{\texttt{clear}}\indexmain{\texttt{graph}}
Deletes all graph elements of the current working graph resp.\ the graph \emph{Graph}.

\begin{rail}
  'delete' 'graph' Graph
\end{rail}\indexmain{\texttt{delete}}\indexmain{\texttt{graph}}
Deletes the graph \emph{Graph} from the backend storage.

\begin{rail}
  'validate' ( ( ) | 'strict' )
\end{rail}\indexmain{\texttt{validate}}\indexmain{\texttt{strict}}
Validates\indexmain{validate} if the current working graph fulfills the \indexed{connection assertion} specified in the corresponding graph model. The \emph{strict} mode additionally requires all the edges of the working graph to be specified in order to get a ``valid''. Otherwise edges between nodes without specified constraints are ignored.\\
\begin{example}
...
\end{rail}\indexmain{\texttt{custom}}\indexmain{\texttt{graph}}
...
\begin{rail}
  'delete' 'node' Node
\end{rail}\indexmain{\texttt{delete}}\indexmain{\texttt{node}}
Deletes the node \emph{Node} from the current graph. Incident edges will be deleted as well.

\begin{rail}
  'delete' 'edge' Edge
\end{rail}\indexmain{\texttt{delete}}\indexmain{\texttt{edge}}
...
\end{rail}\indexmain{\texttt{show}}\indexmain{\texttt{num}}\indexmain{\texttt{nodes}}\indexmain{\texttt{edges}}\indexmain{\texttt{only}}
Gets the \indexed{persistent name}s and the types of all the nodes/edges of the current graph. If a node type or edge type is supplied, only elements compatible to this type are considered. The \texttt{only} keyword excludes subtypes. Nodes/edges without persistent names are shown with a pseudo-name.\\
If the command is specified with \texttt{num}, only the number of nodes/edges will be displayed.

\begin{rail}
  'show' ('node' | 'edge') 'types'
\end{rail}\indexmain{\texttt{show}}\indexmain{\texttt{node}}\indexmain{\texttt{edge}}
Gets the node/edge types of the current graph model.

\begin{rail}
'show' ('node' ('super' | 'sub') 'types' NodeType | 'edge' ('super' | 'sub') 'types' EdgeType)
\end{rail}\indexmain{\texttt{show}}\indexmain{\texttt{node}}\indexmain{\texttt{edge}}\indexmain{\texttt{super}}\indexmain{\texttt{sub}}\indexmain{inheritance}
...
\end{rail}\indexmain{\texttt{show}}\indexmain{\texttt{node}}\indexmain{\texttt{edge}}\indexmain{\texttt{only}}
Gets the available node/edge \indexed{attribute} types. If \emph{NodeType}/\emph{EdgeType} is supplied, only attributes defined in \emph{NodeType}/\emph{EdgeType}. The \texttt{only} keyword excludes inherited attributes.\\
\begin{note}
...
\end{rail}\indexmain{\texttt{show}}
Gets the value of a specific attribute.

\begin{rail}
  'node' 'type' Node 'is' Node | 'edge' 'type' Edge 'is' Edge
\end{rail}\indexmain{\texttt{node}}\indexmain{\texttt{edge}}\indexmain{\texttt{type}}\indexmain{\texttt{is}}
...
\begin{rail}
  'save' 'graph' Filename
\end{rail}\indexmain{\texttt{save}}\indexmain{\texttt{graph}}
...
\begin{rail}
  'dump' 'graph' Filename
\end{rail}\indexmain{\texttt{dump}}\indexmain{\texttt{graph}}
...
\end{rail}\indexmain{\texttt{dump}}\indexmain{\texttt{add}}\indexmain{\texttt{node}}\indexmain{\texttt{only}}\indexmain{\texttt{exclude}}
...
\end{rail}\indexmain{\texttt{dump}}\indexmain{\texttt{add}}\indexmain{\texttt{node}}\indexmain{\texttt{group}}
Declares \emph{NodeType} and subtypes of \emph{NodeType} as \indexed{group node} type. All the different typed nodes that points to a node of type \emph{NodeType} (i.e.\ there is a directed edge between such nodes) will be grouped and visibly enclosed by the \emph{NodeType}-node.
...
\end{rail}\indexmain{\texttt{dump}}\indexmain{\texttt{add}}\indexmain{\texttt{only}}\indexmain{\texttt{node}}\indexmain{\texttt{edge}}\indexmain{\texttt{infotag}}
...
\end{rail}\indexmain{\texttt{dump}}\indexmain{\texttt{set}}\indexmain{\texttt{edge}}\indexmain{\texttt{labels}}\indexmain{\texttt{on}}\indexmain{\texttt{off}}
Specifies, whether edge \indexed{label}s will be displayed or not (default to ``on'').

\begin{rail}
  'dump' 'reset'
\end{rail}\indexmain{\texttt{dump}}\indexmain{\texttt{reset}}
Reset all style options (\texttt{dump set}\dots) to their default values.

\subsection{Action Commands (GRS)}\indexmain{action command}\indexmainsee{action}{regular graph rewrite sequence}
\label{grsthings}
An \emph{action} denotes a graph rewrite rule.

\begin{rail}
  'select' 'actions' Filename
\end{rail}\indexmain{\texttt{select}}\indexmain{\texttt{actions}}
...
\begin{rail}
  'show' 'actions'
\end{rail}\indexmain{\texttt{show}}\indexmain{\texttt{actions}}
...
\end{rail}\indexmain{\texttt{custom}}\indexmain{\texttt{actions}}
...
\subsubsection*{Regular Graph Rewrite Sequences}\indexmain{regular graph rewrite sequence}\indexmainsee{graph rewrite sequence}{regular graph rewrite sequence}\indexmainsee{GRS}{regular graph rewrite sequence}
...
\makeatother
\mbox{\quad}\\
Table \ref{ruletab} lists graph rewrite expressions at a glance. The operators hold the following \indexed{order of precedence}, starting with the lowest precedence: 
...
Variables\indexmain{variable} can be assigned to graph elements returned by rules using \texttt{(Para\-meters) = \emph{Action}}\indexmain{parameter}. 
...
For \indexed{undefined variables} the specific element constraint of \emph{Action} will be ignored (every element matches).\\

Use the \texttt{debug}\indexmain{debugger} option to trace the rewriting process step-by-step. During execution \yComp\indexmain{yComp} will display every single step\footnote{Make sure, that the path to your \texttt{\indexed{yComp.jar}} package is set correctly in the \texttt{ycomp} shell script within \GrG's \texttt{/bin} directory.}. The debugger can be controlled by \GrShell. The debug commands are shown in table \ref{tabdebug}.\\
\begin{table}[htbp]
...
\end{rail}\indexmain{\texttt{custom}}\indexmain{\texttt{graph}}
Analyzes\indexmain{analyzing graph} the current working graph. The analysis data provides vital information for efficient \indexed{search plan}s. Analysis data are available as long as \GrShell\ is running, i.e.\ when the working graph changes the analysis data is still available but maybe obsolete.

\subsection{Action Related Commands}
\begin{rail}
  'custom' 'actions' gensearchplan (Action+)
\end{rail}\indexmain{\texttt{custom}}\indexmain{\texttt{actions}}
Creates a search plan for each rewrite rule \emph{Action} using a heuristic method and the analyze data (if the graph has been analyzed by \texttt{custom graph analyze\_graph}). Otherwise a \indexed{default search plan} is used. For efficiency reasons it is recommended to do analyzing and search plan creation during the rewriting procedure. Therefore the host graph should be in a stage ``similar'' to the final result. In deed there might be some trial-and-error steps necessary to get a efficient search plan. A search plan is available as long as the current rule set remains loaded. \\
