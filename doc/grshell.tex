...
The \GrShell\ language is a line oriented scripting language. \GrShell\ scripts are structured by simple statements separated by line breaks.
...
\begin{description}
  \item[\texttt{<Word>}] A character sequence consisting of letters, digits, and underscores. The first character must not be a digit.
  \item[\texttt{<DoubleQuotedText>}] Arbitrary text enclosed by double quotes (`` '').
  \item[\texttt{<SingleQuotedText>}] Arbitrary text enclosed by single quotes (` ').
  \item[\texttt{<Number>}] A sequence of digits.
\end{description}
...
\begin{description}
  ...
  \item[NodeType] Identifier of a node type defined in the model of the current graph.
...
\end{center}
The following table shows the effect of further debug commands:
\begin{center}
