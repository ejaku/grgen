...
The \texttt{\$} is a special attribute name: an unique identifier of the new graph element. This identifier is also called \newterm{persistent name} (see example \ref{persistentex}). This name can be specified by a constructor only.
...
Dumps the current graph formatted as \indexed{VCG} into the file \emph{Filename}.\\

\paragraph{Vizualisation Styles}
...
\end{rail}\indexmain{graph rewrite sequence}\indexmainsee{GRS}{graph rewrite sequence}
