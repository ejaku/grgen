...
Selects the current \indexed{working graph}. This graph acts as \emph{host graph} for graph rewrite sequences (see also sections \ref{ov:whatsallabout} and \ref{grsthings}). Though you can define multiple graphs, only one graph can be the active ``working graph''.
...
\subsubsection*{Graph Rewrite Sequences}\indexmain{graph rewrite sequence}\indexmainsee{GRS}{graph rewrite sequence}
