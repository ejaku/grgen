...
The \GrShell\ is case sensitive. A comment starts with a \emph{\#} and is terminated by end-of-line or end-of-file. Any thext left of the \emph{\#} will be treated as a statement.
...
Color: & One of the following color identifiers: Black, Blue, Green, Cyan, Red, Purple, Brown, Grey, LightGrey, LightBlue, LightGreen, LightCyan, LightRed, LightPurple, Yelloq, White, DarkBlue, DarkRed, DarkGreen, DarkYellow, DarkMagenta, DarkCyan, Gold, Lilac, Turquoise, Aquamarine, Khaki, Pink, Orange, Orchid. These are the same color identifiers as in VCG/\yComp\ files.
\end{tabularx}\\
...
%\lined 
\marginpar{\mbox{ }\\ \small \textbf{Example}} {\label{persistentex} \small \mbox{ }\\ We insert a node, anonymously and with a constructor:
\begin{grshell}
...
>
\end{grshell}}
\marginpar{\small \textbf{Note}} {\small Persistent names belong to a specific graph, whereas variables belong to the current \GrShell\ environment. Persistent names will be saved (\texttt{save graph\dots}, see \ref{outputcmds}) and, if you visualize a graph (\texttt{dump graph\dots}, see \ref{outputcmds}), graph elements will be labeled with their persistent names. Persistent names have to be unique for a graph.} 

\begin{rail}
  Variable '=' GraphElement   
\end{rail}
Assigns the variable or persistent name \emph{GraphElement} to \emph{Variable}. If \emph{Variable} has not been defined yet, it will be defined implicitly. As usual for scripting languages, variables have no type nor declarations.
...
{\small \\ We reuse a simplified version of the street map model from chapter \ref{chapmodellang}:
\begin{grgen} 
model Map;
...
\end{table}
\marginpar{\mbox{ } \\ \small \textbf{Example}}
{\small \\ We demonstrate the debug commands with a slightly adjusted script for the Koch snowflake from \GrG's examples (see also \ref{fractals}). The graph rewriting sequence is
\begin{grshell}
debug grs (makeFlake1* ; (beautify ; doNothing)* ; makeFlake2* ; beautify*){1}
\end{grshell}
\yComp\ will be opened with an inital graph (resulting from \texttt{grs init}):
\begin{center}
  \includegraphics[width=0.3\linewidth]{fig/debug0}
\end{center}
We type \texttt{d}(elaied step) to apply \texttt{makeFlake1} step by step resulting in the graphs of figure \ref{figdebug}.
\begin{figure}[htbp]
  \centering
  ...
  \caption{Delaied step rule application.}
  \label{figdebug}
\end{figure}
