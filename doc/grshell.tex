...
If \texttt{Filename} is \texttt{-}, the output is redirected to stdout again.
...
Opens the graph \emph{Text} stored in the backend. However, the \emph{LGSPBackend} doesn't support \indexed{persistent graph}s. The \emph{LGSPBackend} is the only backend so far. Therefore this command is currently useless. You may achieve persistence by using import/export or save/include instead.
...
Allocates space for a visited flag in the elements of the graph and returns the id of the visited flag (integer number), starting at 0. Afterwards, the visited flag of the id can be read and written by the isvisited and setvisited shell commands or within the rules by the \texttt{visited()} expression.
