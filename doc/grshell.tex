...
The console application \GrShell\ and its WindowsForms counterpart, the \GGrShell, are \indexedsee{shell}{GrShell} applications built on top of \LibGr\indexmain{libGr} (for the backend-independent interface) and \emph{libGrShell} (containing most of the implementation) that offer an execution environment for your generated graph transformations.
They belong to \GrG's standard equipment, and implement nearly everything in common, so in the following we use only the term \GrShell.
...
The \GrShell{} implements a command-line interpreter in the form of a read-eval-print loop, reading from the console or a file, evaluating (or better executing) the command, and printing the output (as applies), line by line (abstract semantics).

The elements of a graph (nodes and edges) can be accessed both by their (graph global) \indexed{variable}\indexmain{graph global variable} identifier and by their \newterm{persistent name} specified through a constructor (see Section~\ref{graphcreationcommands}) --- the \GrShell\ always uses named graphs, so a persistent name is always existing, if a name is not explicitly assigned, a pseudo-name will be automatically assigned.
...
Besides the \GrShell{}-commands defined in this chapter, you find in the following Chapter~\ref{cha:persistentstorage} commands for persistent storage, in Chapter~\ref{chapdebugger} commands for graph visualization and action debugging, and in Chapter~\ref{chapvalidation} commands for model/graph inspection and validation/testing.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Building Blocks}\label{shellbuildingblocks}

Here we describe some common building blocks (alongside their syntax) of the commands introduced in the upcoming and the following sections.
You may skip it and return to it when you are interested in learning more about the parts of the commands as you visit them.
...
\begin{description}
  ...
  \item[Variable] Identifier of a (graph global) variable that contains an entity or a value. \indexmainsee{GrShell variable}{graph global variable} A double colon prefix as required in the sequences may be given, but as the shell only knows graph global variables, it is optional (in plain shell commands, not in sequences or sequence expressions appearing within shell commands).
...
Its name is set to \emph{Text}; its unique name as used in exporting or recording may be a variant of this name in case another subgraph of the same name already exists.
...
\end{rail}\ixkeyw{eval}\indexmain{graph rewrite sequence expression}\ixnterm{GraphRewriteSequenceExpression}
...
\begin{rail}
  'show' 'profile' (Action)?
\end{rail}\ixkeyw{show}\ixkeyw{profile}\label{secshellvariables}
...
A variable is an identifier that can be bound to a value (scalars, but also containers and entities), which may be changed over time by assignments.
A graph rewrite system offers graph global variables \indexmainsee{GrShell variable}{graph global variable}s in the form of a global flat mapping of names to values.
These are identical with the \GrShell\ variables.
They can be accessed in the \GrShell\ directly by their name, whereas in the sequences (or the rule language), a double colon prefix is required (using them outside of the shell is discouraged; see Chapter \ref{cha:xgrs} for the distinction between graph global aka shell and sequence local variables).

\begin{rail}
  Variable '=' ( Variable | Entity | Literal )
\end{rail}
Assigns to the target \emph{Variable} the source \emph{Variable} (i.e. its value), or the \emph{Entity} denoted by its persistent name or unique id, or the \emph{Literal} (textual representation of a constant value).
If \emph{Variable} has not been defined yet, it will be defined implicitly.
As usual for scripting languages, variables have neither static types nor declarations (the values are typed, though, you run into an exception when you try to execute an operation on a mismatching object).
...
The \texttt{if} command is an exception, a corresponding amount of \texttt{endif} commands is required, and applies to its syntactically preceding if command, as does the \texttt{else} command. 
...
they allow you to assess the progress of long-running transformations (this functionality is \emph{not} available in the \GGrShell{}).
With this command they can be disabled (or enabled again).
Switching them off may be of interest if your own debug messages printed via emit from the sequences (or rules) should not be disturbed.
...
This way you can save the statistics of a characteristic graph of your domain and compile pattern matchers well adapted to that class of graphs straight from the beginning, saving you the costs of online analysis and matcher compilation.
...
Sets the size of the pool storing deleted elements for reuse to \emph{Number} (default: \texttt{10}).
