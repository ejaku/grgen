...
The following syntax diagram gives an extension to the syntax diagrams of the Rule Set Language chapter \ref{chaprulelang}:
\begin{rail}
...
The argument(s) must be of string type, but any type is automatically cast into its string representation as needed. Prefer comma separated arguments over string concatenation, they are more efficient as no intermediate strings need to be computed, just to be garbage collected thereafter.
...
The following syntax diagram gives an extension to the syntax diagrams of the Subpatterns chapter \ref{cha:sub}:
\begin{rail}
  SubpatternExecEmit:
...
First all sequences from a called rule are executed, before the current sequence is continued or other sequences of its parent rule get executed (depth first).
...
The \texttt{tryscan} function expects the same parameters, but it returns whether the given string can be transformed into an object of the requested type.
