...
They are a kind of inverse of emit (also available as procedure, see \ref{procstab}), to transform from/to strings that are intended for user consumption (this stands in contrast to machine consumption, and the full serialization/deserialization offered by the \texttt{export} procedure and the \texttt{import} function, cf. \ref{sec:fileop}).
...
It returns the obtained object (scalar values and containers are created, graph elements specified by their persistent name in contrast are fetched from the graph, object classes are not supported), or diverges (i.e. crashes).
The \texttt{tryscan} functions expects the same parameters, it returns whether the given string can be transformed into an object of the requested type.
