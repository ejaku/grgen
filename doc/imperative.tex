...
but additionally set/map/array member attributes may be used as storages,
...
A difference of storage sets and maps to the sets and maps in the rewrite rules is that they only offer imperative addition and removal instead of union, intersection, difference and construction.
...
\begin{description}
\item[Size assignment:] \texttt{v=sma.size()} writes the number of entries in the set or map or array \texttt{sma} to the variable \texttt{v}, succeeds always.
\item[Emptyness assignment:] \texttt{v=sma.empty()} writes to the variable \texttt{v} whether the set or map or array \texttt{sma} is empty, succeeds always.
...
The binary operator \texttt{el in sma} checks for set/map/array membership; it returns true if \texttt{el} is contained in the set or the domain of the map or the array, otherwise false.
...
The advantage of this storage querying inside the rule over handing in a value from a for loop iterating the storage values  outside the rule are: \emph{i)} a more concise syntax, \emph{ii)} the ability to access a storage attribute of an element just matched or to access a storage map with an element just matched in the same rule, which would require to break up the rule in two rules in the other case, and \emph{iv)}, a restriction of the iteration to the matching phase, so that at rewriting one can happily manipulaivte the storage without destroying the iterator/enumerator used in the loop which would be the case when using an outside loop.
...
Furthermore you can add and remove elements from the storages (or normal sets/maps/arrays) in the \texttt{eval} clause of the rewrite part:
