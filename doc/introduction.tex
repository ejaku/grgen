...
\begin{enumerate}
	...
	\item that stand to each other in 3 relations (control flow (red), data flow (brown), and memory flow (blue)), with an \emph{explicit} representation of those as edges \footnote{the edges denote dependencies, their direction is thus reversed compared to the flow}.
\end{enumerate}
...
When working in \emph{computer linguistics} you may be intersted in modeling an \emph{abstract syntax graph} in \GrG;
