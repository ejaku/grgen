...
{\scshape GrGen} (\textsc{G}raph \textsc{R}ewrite \textsc{Gen}erator) is a \emph{development tool} that offers \emph{programming languages} optimized for graph structured data, with \emph{pattern matching} and \emph{rewriting} at their core, supplemented by support for imperative and object-oriented programming, showing some traits of database query-result processing.
The generative programming system featuring a \emph{graphical debugger} eases the \emph{transformation} of \emph{graph-based representations}, as they are needed in e.g. model transformation, computer linguistics, or modern compiler construction.
