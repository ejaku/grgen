...
Altogether, \GrG\ offers the highest combined speed of development and execution you can find for those kinds of tasks.
...
The \emph{knowledge} of an agent about the external world can be \emph{represented} intuitively like this with \emph{semantic nets}, you could use \GrG\ for reasoning over them.
...
You find help not only in modelling them, but also in deriving all interesting configurations or simulating all interesting execution traces, in the form of built-in support for a backtracking search through a search space or even the enumeration of a state space with isomorphic state pruning, guided by match ordering and filtering.
...
In contrast to object-oriented programming, the topology in graph-oriented programming as offered by \GrG\ is not hidden and encapsulated in the objects, but globally open for inspection and modification;
...
An interesting transformation that can be applied to the compiler intermediate representation introduced in section \ref{sub:examplegraphrep} is constant folding.
...
Note that the rule is \emph{modular}, i.e. independent of other functionality --- traditionally, functionality is smeared over multiple graph representation traversal passes, and multiple steps of a pass.
...
And it implements efficient graph change rollback, thus allowing you to easily crawl search spaces without your own bookkeeping (for undoing the changes).
...
Its languages are brought to life by an \emph{optimizing compiler} that prunes unneeded generality and generates pattern matchers adapted to the characteristics of the host graph at hand.
...
The underlying framework supports automatic persistence and automatic visualization of network-like data (based on event-sourcing - besides, also built-in serialization / deserialization is offered).
A runtime library is supplied (in the form of several DLLs) that implements an easy-to-use API to these features, allowing you to build your own .NET (or native) applications based on it.
