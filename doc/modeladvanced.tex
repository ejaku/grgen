...
They allow you to group attributes that are commonly used together into an own entity.
...
\end{rail}\ixkeyw{class}\ixkeyw{extends}\ixnterm{ObjectClass}
...
\end{rail}\ixnterm{UniqueObjectConstraintDecl}\ixkeyw{unique}
...
In case it is not available, an id is assigned on the fly as needed -- but not stored, so another id may be assigned later on.
...
\subsection{Transient Object Types (preliminary)}

\begin{rail}
  TransientObjectClass: 'transient' 'class' IdentDecl (() | 'extends' (TransientObjectType+',')) \\
    (';' | lbrace AttributeDeclarations rbrace);
\end{rail}\ixkeyw{class}\ixkeyw{extends}\ixnterm{TransientObjectClass}
A \emph{TransientObjectClass} definition defines a new \indexed{transient object type}.
...
Note that transient classes are a preliminary concept that may change in the future.
...
\end{grgen}
Instead of implementing these procedures manually, you can simply write (and use in function context)
\begin{grgen}
...
Objects and transient objects can be compared by the \texttt{==} and \texttt{!=} operators, as well as by the \texttt{\textasciitilde\textasciitilde} operator (preliminary).
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{|l|X|}
    ...
    \texttt{A \textasciitilde\textasciitilde\ B} & True, iff $A$ is deeply equal (by-value) to $B$ (preliminary). \\
    \hline
  \end{tabularx}
...
Function methods are defined in exactly the same way as functions (cf.~\ref{sub:functions}) are defined, just inside the hosting class.
...
Such a function method may then be called as an expression atom from anywhere in the rule language file where an expression is required; or even from the sequence computations (where an expression is required).
...
Such a procedure may then be called as a statement atom from anywhere in the rule language file where an attribute evaluation statement (/computation) is required; or even from the sequence computations (where a statement is required).
...
%todo: explain reason why no full expression - graph changes are constrained but in return support fully automatic persistence, transactions, change recording; other constraints compared to well-known programming languages and their design decisions/choices
...
\begin{example}
In the following listing, we declare function and procedure methods inside a node class, and use them from a rule.
...
rule foo : (int)
{
	...
	---
		...
		return(i); // executing foo will return 42
	}
}
	\end{grgen}
\end{example}
...
rule foo : (int)
{
	...
	---
		...
		return(i); // executing foo will return 0 iff n was bound to node of type N, but 1 iff n was bound to node of type NN
	}
}
	\end{grgen}
\end{example}
...
The packages \texttt{Math}, \texttt{File}, \texttt{Time}, \texttt{Transaction}, and \texttt{Debug} as well as \texttt{Synchronization} are built-in, their functions and procedures can be used directly (see \ref{sec:primexpr}, \ref{sec:subgraphop}, \ref{sub:transaction} and \ref{secdebuggersubrule} as well as \ref{locking} for their content).
...
A package with a given name can only be defined once, either in the model or the actions.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Graph References / Graph Nesting / Hierarchical Graphs}\label{sec:graphnesting}
...
The syntax of rule application is extended by the grammar rule above to allow for this.

\begin{rail}
  ReferenceToContainingGraphDecl: 'node' 'edge' 'graph' ';';
\end{rail}\ixnterm{ReferenceToContainingGraphDecl}
...
The reference to the containing graph may be queried from the rule language expressions or the sequence expressions with the \texttt{\indexed{graphof}} function:
\begin{description}
...
In \emph{graph-oriented programming} the attributes but especially the neighboring elements are known to the containing graph, the connecting topology is open for \emph{pattern matching}.
