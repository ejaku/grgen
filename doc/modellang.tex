...
It allows specifying \emph{node} and \emph{edge classes} bearing typed \emph{attributes} that are used in building and wiring the graph. 
Additionally, \emph{enumeration types} can be declared, to be used as \emph{attribute types} (the other available attribute types are built-in and don't need to be declared, they can just be used).
A graph (instance) adheres to a graph (meta) model as defined in the following.
...
In Chapter~\ref{cha:performance}, you find more on \emph{indices}, which allow quickly accessing the elements of the graph that are of interest to you.
...
The direction for \texttt{AEdge} and its ancestors that do not inherit from \texttt{Edge} or \texttt{UEdge} is undefined or \newterm{arbitrary}.
...
As soon as you decided for directed or undirected edge classes within your type hierarchy, you are not able to let ancestor classes inherit from a contradicting base type, of course.
That is, no edge may be directed \emph{and} undirected.
This is an exception to the concept of multi-inheritance.
...
A \emph{\indexed{connection assertion}} allows specifying a \emph{constraint} on the topology.
It allows specifying the node types an edge may connect to, and the multiplicities with which such edges appear on the nodes.
...
Furthermore there is the strict-only-specified mode, which only does strict validation of the edges for which connection assertions are given.
...
The \texttt{?--?} arrow means an arbitrary edge, i.e. directed as well as undirected is possible (fixed by the concrete type inheriting from it); in case of a directed edge the connection pattern gets matched in both directions.
...
Furthermore, attributes may be of node or edge or graph type, so they may be directly pointing to a node or an edge (seldom useful and discouraged), or to a subgraph (containers may contain those types, too, this holds also for container-typed attributes).
...
The initialization values are evaluated in the order in which they appear in the graph model file.
...
An \texttt{enum} type is a collection of so called \emph{\indexed{enum item}s} (or members, or cases), which are used to define and name the values you want to distinguish (when using this type within your model). 
Those are internally associated with integral numbers, which may be specified explicitly.
Accordingly, a \GrG\ enum can be cast to an \texttt{int} (see Section~\ref{sec:builtintypes}).
...
\begin{note}
	An enum type and an {\tt int} are different things, but in expressions enum values are implicitly cast to {\tt int} values as needed (see Section~\ref{sec:builtintypes}). The other direction is not available at all, cf. the following Note.
\end{note}
...
\begin{note}
	The C-like semantics of enum item declarations implies that multiple items of one enum type can be associated with the same {\tt int} value, and esp. implies that such items/cases cannot be distinguished anymore in-memory after assignment to a variable.
	...
	The \GrG{}-\texttt{enum} is directly mapped to a C\#-\texttt{enum}.
\end{note}
...
\begin{note}
Don't materialize transitive relationships. 
...
It is worth your continued attention.
