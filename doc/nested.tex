...
The following patterns that allow matching a simplified abstract syntax tree give a rough picture of the language of nested and subpatterns:
...
From now on we will increase the expressiveness of the pattern language, and dependent on it the rewrite language, to describe much more finely and flexibly what patterns to accept.
This will be done by pattern specifications built up from multiple static pattern piece specifications, where the pieces may be matched dynamically zero, one, or multiple times, or are forbidden to exist for the entire pattern to be matched.
...
just think of NACs to simply specify a pattern which should not be in the graph, with the possibility of forcing elements to be the same as in the enclosing pattern by name equality.
  ...
  \begin{example}
    We specify a variable which is not badly typed, i.e. a node \texttt{x} of type \texttt{Variable} which must not be the target of an edge of type \texttt{type} with a source node of type \texttt{BadType}:
    \begin{grgen}
  x:Variable;
  negative {
    x <-:type- :BadType;
...
They are used when all matches of a pattern are wanted, and a part of that pattern is available in the graph multiple times, but should not cause combinatorially additional matches; then the \texttt{independent} can be used to check only for the existence of that part, limiting the all-matching to the core pattern (when only one match of a pattern is wanted, with a part being available multiple times in the graph, its not of importance which of the parts being available multiple times gets matched).
They are needed if subpatterns want to match elements which were already matched during the subpattern derivation (and are thus isomorphy-constraint-locked, which would prevent them from getting matched another time -- a situation which could easily occur because the full pattern is not statically visible).

An independent can be seen as a conjunction with an existence-quantified pattern, a negative as a conjunction with the negation of an existence-quantified pattern (a main pattern can be used as an existence-quantified or all-quantified pattern, which is decided by sequence all-bracketing, and it rewrites the matched spots, in contrast to the application condition patterns); the multiple/iterated patterns introduced below are also a kind of all-bracketing, but with isomorphy-constraint locking in between the matches.
...
The \indexmain{pattern cardinality} blocks allow specifying how often the nested pattern -- opening a scope -- is to be matched.
...
then being able to access their specific attributes and being able to give different additional rewrite parts.
	\begin{grgen}
test refineFeature
{
...
The image from above shows an example of how a structure is matched by several pieces.
...
As the already introduced syntax remains valid, in aggregate we extend the nested patterns with optional names to form \indexed{named nested pattern}.
