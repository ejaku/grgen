...
Nodes and edges from the pattern containing the nested pattern containing the nested rewrite are only available for deletion or retyping inside the nested rewrite if it can be statically determined that this is unambiguous, i.e. only happening once.
...
The subpattern rewriting specifications within the subpattern declaration look like a nested rewriting specification,
...
The \emph{SubpatternOccurrence} is explained in the next subsection \ref{sub:delpressub}.
...
In addition to the fine-grained dependent rewrite, subpatterns may get deleted or kept as a whole.

\begin{rail}  
  SubpatternOccurrence: 
    Ident ';' |
    'delete' '(' (Ident + ',') ')' ';';
\end{rail}\ixkeyw{SubpatternOccurrence}\ixkeyw{delete}
