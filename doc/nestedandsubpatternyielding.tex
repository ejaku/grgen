...
%in previous chapters: match object tree assembly, reflecting pattern nesting - TODO: example for match tree structure - syntax relationship - first only inherited attributes; rewriting pass alongside nesting; LHS,RHS,nesting 
%here extension by synthesized attributes - def elements and yields, extended eval, execs; flows
...
%nested and subpatterns (LHS only) --> rewriting of nested and subpatterns (extension to RHS, dependent replacement, 2nd IAG pass) --> def elements (extension of IAG by SAG, of both passes)
%2 LAG durchläufe, bei erstem kein einfluss auf L möglich, limitiert auf IAG+SAG, bei zweiten auch default IAG+SAG, aber Ordnung kann erzwungen werden, L definiert)
