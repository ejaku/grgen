...
which offer a rapid prototyping environment supporting graphical and stepwise debugging of rules that are controled by sequences.
...
\begin{figure}[htbp]
  ...
  \caption{\GrG\ system components and artefacts}
  \label{figsys}
\end{figure}
...
The central object is the \emph{graph}, it adhers to the specified graph model.
...
At the foundation you find the graph model (meta-model) language given with \texttt{class} definintions in the style of an object-oriented language, with some influences of data definition languages as offered by databases. 

It allows you to specify \emph{node} and \emph{edge types}, with \emph{multiple inheritance} on the types.
...
Moreover, you may configure indices: attribute indices for a quick lookup of a graph element based on the attribute value, or incidence count indices for a quick lookup based on the number of incident edges.
...
It supports pattern-based \emph{rules}, which are built by a \emph{pattern to be matched} and a \emph{rewrite} to be carried out; or put differently: a left-hand-side precondition pattern and a right-hand-side postcondition pattern.
A pattern is described by \emph{graphlets}, a specification of nodes connected by edges in an intuitive syntax.
Complex patterns are combined from elementary patterns with \emph{nested patterns} (defining alternative, optional, multiple and iterated structures, or negative and independent application conditions) and \emph{subpattern} definitions and usages.
Besides those parts for declarative processing of structural information are \emph{attribute conditions} and \emph{attribute evaluations} available, the former are expressions computing an output value by reading attributes of matched elements, the latter are statements changing attribute values with assignments. 

The attribute processing expressions and statements are in fact parts of a full-fledged \emph{imperative programming language} integrated with the declarative pattern-based rule language.
The \emph{expressions} can query the graph with a multitude of available functions in addition to reading the attributes of matched elements and carrying out computations with the attributes; they can be abstracted into \emph{functions}.
The \emph{statements} support direct graph changes and supply control flow statements and def variable declarations in addition to the attribute assignments; they can be abstracted into \emph{procedures}.
You especially find subgraph operations for the processing of nested graphs, with subgraph extraction, subgraph comparison, and subgraph insertion. 
...
The rewrite part supports two modes of specification: A rule can either express the changes to the match (\texttt{modify}-mode, requiring deletion to be specified explicitely) or specify a whole new subgraph (\texttt{replace}-mode).
...
Besides changing the graph can you \texttt{emit} user-defined text to \texttt{stdout} or files (supporting model-to-text transformations).

A special class of computations are available with the post-match filter functions, that allow to inspect and manipulate the set of matches found when applying a rule (with all bracketing). 
...
A graph rewrite system described in those languages can be employed via an \emph{API} by .NET-applications, typically written in C\#.
...
This strategy language allows to determine the rule that is called next by its control operators and constructs.
...
The sequences offer a \emph{computations} sublanguage that supports calling procedures and functions, basic graph querying and modification, visited flags management, and storages manipulation.
...
Most usages are related to \emph{storages} which are variables of container type storing graph elements, they esp. allow to build transformations following a wavefront running over the graph.
...
Advanced constructs are available with \emph{transactions} angles and \emph{backtracking} double angles capable of \emph{rolling back changes} carried out on the graph to get back to the state of the graph again from before their nested content was applied.
...
Those sequences are available in an integrated form in the rule language.
...
Regarding graph handling you find commands for graph creation and manipulation, graph export and import, and graph change recording plus replaying.
...
Changes that are occuring to a graph may be \texttt{record}ed to a \texttt{GRS} and later \texttt{replay}ed or simply \texttt{import}ed.
...
in detail mode even the match found and the rewrite carried out by the rule are highlighted in the graph.
...
Altogether, you can taylor the graph display to what you need, transforming an unreadable haystack which is what large graphs tend to become into a well-readable representation.
...
are nested and subpatterns available for combining them flexibly;
they allow to process substructures of arbitrary depth and breadth with a single rule application.
This surpassess the capabilities of the VIATRA2\cite{viatra2,recursiveviatra} and GROOVE \cite{Groove} tools, our strongest competitors regarding rule expressiveness.
...
For subtasks where the declarative pattern-based rules do not work well (which happens occasionally), are you free to revert to imperative and object-oriented programming as known from the C++, Java, C\# language family, utilizing the imperative parts of the \GrG-languages.
...
The multi-paradigm languages of \GrG\ offer you all the constructs you need for transforming graph-represenations.

\subsection*{General-Purpose Graph Rewriting}
in contrast to \emph{special-purpose} graph rewriting requires high programmability, genericity and customizability.
...
But state space enumeration requires a lot of memory due to state explosion phenomenon -- for practical applications you need control over which changes are to be seen as states and which not, and single rules often are too low-level regarding this classification, materializing many superfluous states.
...
Programmability always must come with means to abstract and encapsulate the programs into own units and parameterize them, so you are not exposed to details where you don't need them (but can access them when you need to do so, in contrast to fixed-function units).
...
It was always amongst the best rated tools, a record that is only achievable for a general-purpose tool with expressive and universably usable languages.
...
It is achieved by a model implementation that is optimized for efficient and scalable pattern matching (and rewriting),
while not growing memory needs out of proportion (as it happens for incremental approaches storing and maintaining the matches of all rules at any time).
...
When needed, attribute indices or incidence count indices can be applied, getting fast access to graph elements based on attribute values, or incidence counts.

The generative approach compiling the rules into executable code,
with nested loops for matching the pattern elements is helping tremendously regarding performance.
But it comes at a cost: the test-debug-cycle is slowed down, and you are less flexible regarding runtime changes.
A further help for performance are the types, which are speeding up the pattern matcher
-- besides being a help in modeling the domain and besides easening the life of the programmer by eliminating large classes of errors at compile time.
Performance is further fostered by explicit control and rule applications from preset parameters on, defining where to match the rules (giving rooted pattern matching), and which rule to match when (getting faster to a matching rule based on domain knowledge), in contrast to approaches based on implicit control, most notably the graph-grammar approach with its parameterless rules (only controled by an optional layering). 

Performance is gained by the host graph sensitive search plans.
...
Furthermore, there are several strength reduction and inlining optimizations employed by the compiler to eliminate the overhead of the higher level of abstraction where it is not needed;
in addition to the search state space stepping that typically yields matches faster,
an important factor for being at least one order of magnitude faster than any other tool known to us according to \indexed{Varr\'o's benchmark}\cite{varro_bench}.
Finally, search intensive tasks can be parallelized, reaping benefits from ourdays multicore machines; have a look at our solution for the Movie Database case of the TTC14\cite{MovieDatabase} to see what can be achieved this way.

\subsection*{Understandability and Learnability}
was taken care by evaluating for each language construct several options,
prefering constructs already known from imperative and object-oriented programming languages as well as parser generators ---
...
which may be pleasing to someone from the realm of formal specification, but which are not to the mind of a programmer.
...
Especially since the consequences of the development goals expressiveness and programmability are increasing the learning effort.
...
The costs of developing a domain-specific lanuguage, a model as a high-level representation of a domain, and a code generator emitting executable code from it (maybe with an intermediate platform dependent model in the chain) can only be amortized for large program families with a high degree of variation, for the bulk of software application the costs will never pay off.
...
Besides, we have serious doubts about the encompassing MDA vision (see paragraph above), yet think that a great graph-representation-processing language and supporting environemnt is of real practical interest for many tasks.
...
-- in addition to \emph{batch-wise offline} mapping of a graph representation to another one, which defines the common and only usage of model \emph{mapping} tools like ATL\cite{atl} or Epsilon\cite{epsilon}--
