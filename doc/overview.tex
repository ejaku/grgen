...
In contrast to systems like \indexed{Fujaba}~\cite{fujaba}, our pattern matching algorithm is fully automatic and does not need to be tuned nor to be partly implemented by hand.
...
\section{Goals of \GrG development}
TODO

algorithmic core
graph structured data
  semantic nets, computer linguistics, model driven architecture, program graphs - high/low level as compiler vs software engineering, social nets, chemical formulas
rapid prototyping with tools (maybe suffucient to you) then api level programming
use the result in your program, stemming from any .NET language, with the .NET framework availabe

special purpose software development tool, genereal purpose graph rewriting 

expressiveness, convenience, speed
automatic optimization, declarative languages
generative approach, host graph sensitive search plan bases graph pattern matching

Offering the highest combined speed of development and execution.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Besides SPO, three additional modes for the semantics of a rule application can be choosen: matching of exact patterns only, matching of induced subgraphs only, and DPO semantics
...
\GrShell\ is capable of creating, manipulating, and dumping graphs as well as performing graph rewriting with graphical debug support. For further information about the \GrShell\ language see chapter~\ref{chapgrshell}.
