...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Homomorphic matching for a selectable set of pattern element, so that they may match (non-injectively) the same graph element 
  ...
  \begin{itemize}
    ...
    \item A rewrite part for the nested patterns and subpatterns, so that complex structures can not only get matched (parsed), but also get rewritten (transduced)
