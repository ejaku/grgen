...
\GrG\ is a version of {\scshape GrGen} written in C\#.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Regular graph rewrite sequences that can contain optionally nested transactions\index{transaction, nested} (yet to be implemented)
  \end{itemize}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
      \begin{scope}[shape=rectangle,minimum size=0.75cm,text width=3cm,text centered]
          ...
          \node (grs)      at (6   ,5.5)  {Graph Rrewrite Script\indexmain{graph rewrite script}\indexmainsee{GrShell script}{graph rewrite script}\indexmainsee{script}{graph rewrite script} (*\indexed{.grs})};
      \end{scope}
...
For most issues---in particular or experimental purposes---you might rather want to work with the \GrShell\ because of its more convenient interface.
However, \GrShell\ does not provide the full power of the \LibGr; see also note \ref{note:indeterminism}.
...
We start using a special case to construct our host graph: an \indexed{empty pattern} always produces exactly one\footnote{Because of the uniqueness of the total and totally undefined morphism.} match (independent of the host graph). So starting with an empty host graph we construct an apple by applying
\[
  p_0:  
  \begin{array}[c]{c} 
    \emptyset
...
The \GrShell\index{GrShell} is a shell application of the \LibGr. \GrShell\ is capable of creating, manipulating and dumping graphs as well as performing graph rewriting and debugging graph rewriting. For further information about the \GrShell\ language see chapter \ref{chapgrshell}.
...
\label{tools:ycomp}
\yComp\index{yComp} is a graph visualization tool based on \yFiles. It is well integrated in \GrG, but it's not a part of \GrG. \yComp\ implements several graph layout algorithms and has file format support for \indexed{VCG}, GML and YGF among others. Notice that \yComp\ is not free software. However, it's free for use in academic and non-commercial areas.\\
