...
A graph rewriting system\footnote{System, in this context, is not a CHO-like grammar rewriting system, but rather a set of interacting software components.} is defined by a rule set file (*.grg) and zero or more graph model description files (*.gm). 
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
      \begin{scope}[shape=rectangle,minimum size=0.75cm,text width=3cm,text centered]
          ...
          \node (act)      at (13.5,-0.5) {rules1\_action.dll};
          \node (backend)  at (13.5,1)    {backend.dll};
          \node (mod)      at (13.5,2.5)  {rules1\_model.dll};
      \end{scope}
...
In the simplest case such a graph rewrite rules consists of a tuple $L \leftarrow R$, whereas $L$ the left hand side of the rule is called \emph{pattern graph} and $R$ the right hand side of the rule is the \emph{replacement graph}.
...
Together with a rule name $p$ we have $p : L \leftarrow R$.
...
\]
applied to $H$. 
We'll get the apple as new host graph $H'$. 
...
\]
What happened? 
\GrG\ has randomly chosen a match, and $e3$ matches as well as $e1$. 
...
\[
  ...
  \begin{array}[c]{c} 
    \begin{tikzpicture}
      \tikzstyle{every node}=[circle]
      \node[draw] (n1) at (2.5,5.5) {};
      \node[draw] (n2) at (2,4)   {};
      \node[draw] (n3) at (0,2)   {};
      \node[draw] (n4) at (2,0)   {};
      \node[draw] (n5) at (4,2)   {};
      \node[draw] (n6) at (1.7,5.0)   {};
    	
    	\draw[-latex] (n2) --                                  (n1) node[right,midway]  {$e_1$};
    	\draw[-latex] (n2) .. controls +(-1,1) and +(0,1) ..   (n3) node[left,midway]  {$e_2$};
      \draw[-latex] (n3) .. controls +(0,-1) and +(-1,0) ..  (n4) node[left,midway]  {$e_3$};
    	\draw[-latex] (n4) .. controls +(1,0)  and +(0,-1) ..  (n5) node[right,midway] {$e_4$};
      \draw[-latex] (n5) .. controls +(0,1)  and +(1,1) ..   (n2) node[right,midway] {$e_5$};
    	\draw[-latex] (n2) .. controls +(-0.2,+0.4) and +(-0.1,-0.4) .. (n6) node[left,midway]   {};
    	\draw[-latex] (n2) .. controls +(+0.1,+0.4) and +(+0.2,-0.4) .. (n6) node[right,midway]  {};
    \end{tikzpicture}
  \end{array}
\]
