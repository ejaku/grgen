...
(In general you have to distinguish between a graph model on the meta level, a host graph created as an instance of the graph model, and a statically specified pattern graph of a rule that matches a portion of the host graph at runtime).
It may be modified directly by \GrShell, or by a user application.
But it is typically changed by the application of one of the generated \emph{actions}, most often from within a \emph{sequence}, which is parsed into an operator tree and then interpreted, by the graph \emph{processing environment}.
...
you may be interested in additionally using the \GrShell\ because of its debugging abilities.
...
Here we give a short introduction to the tools.
You find a more detailed explanation in \ref{cha:toolsandcomponents}, also explaining the components/assemblies.

The programs and libraries of \GrG\ are open source licensed under \indexed{LGPL}\cite{LGPL3}.
The graph viewer \yComp\ is closed source and ships with its own license granted by \yFiles\ for academic use.
The \MSAGL{} based graph viewer comes with a way more permissive MIT license.
The persistent graph is implemented utilizing the SQLite database, which is in the public domain.

Executing a generated graph rewrite system requires .NET Framework\cite{NET} version 4.7.2 or mono\cite{MONO} version 5.10, or later (maybe complete packaging); compiling (and debugging with \yComp) a graph rewrite system in addition requires JAVA\cite{JAVAweb} version 1.8 or later. 
...
It is typically very helpful during development, to use the \texttt{-keep} and \texttt{-debug} compiler options, you find more on them in Chapter~\ref{cha:toolsandcomponents}.

\begin{note}
If you run into \texttt{Unable to process specification: The system cannot find the file specified} errors, 
GrGen is typically not able to execute the JAVA frontend (\texttt{grgen.jar}).
...
%-----------------------------------------------------------------------------------------------
\subsection{\texttt{GrShell.exe} and \texttt{GGrShell.exe}}

\parpic[l] {
\includegraphics[width=48pt]{fig/grshell-256.png}
}
\noindent The \texttt{GrShell.exe}\indexmain{GrShell} and the \texttt{GGrShell.exe} version are shell applications on top of the \LibGr.
They are capable of creating, manipulating, and dumping graphs as well as performing graph rewriting with graphical debug support.
For further information about the \GrShell\ language see Chapter~\ref{chapgrshell}.

The \GrShell\ is a console application, while the \GGrShell\ is a WindowsForms application, using a rich text box console control.

%-----------------------------------------------------------------------------------------------
\subsection{\texttt{yComp.jar} and \texttt{MSAGL}}
...
You may use the internal \MSAGL{} based graph viewer instead, which comes as a .NET component.
It is a more natural fit for a .NET application, and can be freely included in your own .NET applications (to visualize the graph(s) for the user).
For more on \yComp, see Section~\ref{tools:ycomp}, for more on MSAGL, see Section~\ref{sec:MSAGL}.

%-----------------------------------------------------------------------------------------------
\subsection{Several Dynamic Link Libraries that can be used at API level}

Several DLLs are used in implementing the functionality of the system, that is offered by the \GrShell\ or the \GGrShell\ in the form of an application.
The base system functionality by \texttt{libGr.dll}, \texttt{lgspBackend.dll}, and \texttt{libConsoleAndOS.dll}, plus \texttt{libConsoleAndOSWindowsForms.dll} for the GUI extension (defining the base runtime environment).
Persistence support is realized by a persistence provider \texttt{libGrPersistenceProviderSQLite.dll}.
Graph display and sequence debugging are implemented by the \texttt{graphViewerAndSequenceDebugger.dll}, and its \texttt{graphViewerAndSequenceDebuggerWindowsForms.dll} extension.
The shell functionality itself is contained in the \texttt{libGrShell.dll}.
These assemblies are also available on API level, they can be or have to be included in order to use their corresponding functionalities -- besides the model and actions DLLs generated by the compiler.
...
At the foundation, you find the graph model (meta-model) language, consisting of a series of \texttt{class} definitions in the style of an object-oriented programming language, which shows some influence of data definition languages as offered by databases. 
...
On top of the graph model language and at the heart of \GrG, you find the rule and computations language.
...
Besides those parts for \emph{declarative} processing of structural information, \emph{attribute conditions} (see Chapter \ref{cha:typeexpr}) and \emph{attribute evaluations} are available, the former are expressions computing an output value by reading attributes of matched elements, the latter are statements changing attribute values with assignments. 
...
Besides changing the graph, you can \texttt{emit} user-defined text to \texttt{stdout} or files (supporting model-to-text transformations; see Chapter \ref{cha:imperativeandstate}).

A special class of computations is available with the post-match filter functions (see Chapter \ref{sub:filters}), that allow to inspect and manipulate the array of matches found when applying a rule (with all bracketing). 
...
This limits the changes needed during development and maintenance, and saves you together with the concise and easily editable patterns from many large-scale tedious modifications that are normally needed in those cases.
...
This way common parts can be reused, but it is especially possible to program a recursive strategy.
...
For graph creation 3 simple \texttt{new} commands are available, one for creating an empty graph, one for creating a node, and one for creating an edge (between two nodes).
