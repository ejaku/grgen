...
In contrast to systems like Fujaba \cite{fujaba} our pattern matching algorithm is fully automatic and does not need be tuned or partly be implemented by hand.


\section{Features of \GrG}

The process of graph rewriting can be divided into four step:
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Creation of new nodes/edges with static as well as dynamically computed types (some kind of generic templates)
    \item A rule can alternatively express changes to be made to the match or replace the whole match (the semantics is always mapped to SPO)
...
A graph rewriting system\footnote{System, in this context, is not a CH0-like grammar rewriting system, but rather a set of interacting software components.} is defined by a rule set file (*.grg) and zero or more graph model description files (*.gm). 
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
      \begin{scope}[shape=rectangle,minimum size=0.75cm,text width=3cm,text centered]
          ...
          \node (act)      at (13.5,1) {rules1\_action.dll};
...
In general you have to distinguish carefully between a graph model (meta level), a host graph, a pattern graph and a rewriting rule.
In \GrG\ pattern graphs are implicitly defined by rules, i.e.\ each rule defines its pattern.
On the technical side, specification documents for a graph rewriting system can be available as source documents for graph models and rule sets (plain text *.gm and *.grg files) or as their translated .NET modules, either C\# source files or their compiled assemblies (*.dll).
...
In the simplest case such a graph rewrite rules consists of a tuple $L \rightarrow R$, whereas $L$ the left hand side of the rule is called \emph{pattern graph} and $R$ the right hand side of the rule is the \emph{replacement graph}.
...
Moreover we need to identify graph elements (node or edges) of $L$ and $R$ for preserving them during rewrite. 
...
To perform a rule application, we have to find a suitable location in the host graph with respect to the pattern graph.
Mathematically speaking, such a \emph{match} is a isomorphic subgraph of the $L$ in $H$.
This morphism may not be unique, meaning there may be several matches.
Afterwards we change the matched spot of the host graph, such that it becomes a isomorphic subgraph of the replacement graph $R$.
Elements of $L$ not mapped by $r$ are deleted from $m(L)$ during rewrite.
Elements of $R$ not in the image of $r$ are inserted to $H$, all others (elements that are mapped by $r$) are retained.
The outcome of this steps is the resulting graph $H'$.


\section{An Example}

We'll have a look at a small example. 
First we use a special case to construct our host graph: an empty pattern always produces exactly one match (independently of the host graph). So starting with an empty host graph $H$ we construct an apple using
\[
  p_0:  
  \begin{array}[c]{c} 
    \emptyset
...
\]
applied to $H$. 
We'll get the apple as new host graph $H_1$. 
...
\]
What happened? 
\GrG\ has arbitrary chosen one match among the set of possible matches, because $x$ matches edge $e3$ as well as $e1$.
...
TODO Command line parameters, etc. \GrG, \GrShell, \LibGr, Ycomp,...
