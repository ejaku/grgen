...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
      \begin{scope}[shape=rectangle,minimum size=0.75cm,text width=3cm,text centered]
          ...
          \node (grs)      at (6   ,5.5)  {Graph Rrewrite Script\indexmain{graph rewrite script} (*\indexed{.grs})};
      \end{scope}
...
The transformation is done by \newterm{application} of a rule to a \newterm{host graph} $H$.
...
\begin{description}
  \item[Usage] \texttt{[mono] grShell.exe [-c "<commands>" | <grshell-script>]}\\
     Opens the interactive shell. The \GrShell\ will execute the commands in \emph{grshell-script} (usually a \texttt{*\indexed{.grs}} file) immediately.  
  \item[Options] \mbox{} 
    \begin{tabularx}{\linewidth}{lX}
...
The \LibGr\index{libGr} is a .NET assembly implementing \GrG's \indexed{API}. See the extracted HTML documentation for interface descriptions. 
