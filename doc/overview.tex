...
\GrG\ rules are \emph{modular}: in contrast to traditional pointer structure traversal where you attach the functionality to visiting passes comprising a lot of boilerplate code, do you note down just the functionality.
This frees you from the smearing of graph traversal and graph matching/rewriting functionality, and in consequence from many tedious modifications during development stemming from the continuous change of traversal code due to the steady evolvement of functionality.
You are still free to build local (or even global) passes with rules and tests (tests are rules with only a LHS pattern) by using \emph{parameters}: input and output parameters are supported by rules and tests, the subpatterns, and functions and procedures.
...
Those features and their optimized implementation crown \GrG\ the king of graph pattern matching and rewriting, and allow you to process graph representations at their natural level of abstraction, with a flexibility and performance coming near to what you can achieve by programming everything by hand.
