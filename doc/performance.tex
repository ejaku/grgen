...
For a single pattern normally no user specifies something like this, and if, it is pretty easy to spot.
If specified for a good reasons, normally only one match of such a pattern is sought, aborting the search as soon as found.
...
\begin{rail}
  IndexAccess:
    lbrace IndexName '==' Expression rbrace |
		lbrace ('ascending'|'descending') '(' (IndexBound (',' IndexBound)?)? ')';
...
In case you want to execute a rule on all matches that are available at a point in time, and not on other matches that were not available originally at the first rule application, this is exactly what you need anyway.
...
Especially as rules that search for all matches can be executed quite efficiently in parallel (see parallelization above).
Please note that applying a rule on all matches found is only possible if the parts of the matches which are to be modified, e.g. retyped, are disjoint.
