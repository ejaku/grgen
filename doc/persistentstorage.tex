...
The support for persistent storage can be distinguished into
\begin{itemize}
	\item the import/export functionality, that allows you to serialize/deserialize an in-memory graph at a dedicated point in time
...
With \texttt{libGrPersistenceProviderSQLite.dll}, the database (management system) SQLite\cite{SQLite} is used; this dll is as of now the only offered persistence provider.
...
At the beginning of the next session, the state at the end of the previous session is still available.
...
A mark and sweep like garbage collector is employed directly after the initial graph reading, it finds out which of the known objects and graphs are still reachable from the host graph, and which of the zombie nodes/edges are still reachable from the host graph.
...
The update is also aborted when types have changed their kind and the aforementioned conditions apply, e.g. a \texttt{node class Foo} was switched to an \texttt{edge class Foo} (and the database contains instances of its type), or an enum \texttt{Bar} was switched to an object \texttt{class Bar} (and the database contains values of its type, determined by attributes of its type), they are modeled as a type deletion in a first step, followed by a type introduction in a second step (types are not only defined by their name, but also their kind (but not their content, content changes are handled as single attribute changes)).
...
This holds also for attributes that changed their type, e.g. an attribute \texttt{bar:int} being turned into a \texttt{bar:string}, in the same \texttt{class Foo}, they are modeled as an attribute removal in a first step, followed by an attribute introduction in a second step.
...
\begin{warning}
The persistent graph supports references to nodes/edges that are not contained in the graph of the node/edge that stores them as attributes (so e.g. \texttt{graphof(n.a) != graphof(n)} is allowed for \texttt{n} and \texttt{n.a} being (references to) nodes).
This holds neither for the import/export commands, nor for the debugger or the emitting functionality (as of now).
\end{warning}
...
If a model override of the form \texttt{Filename.grg} is specified, the model of the given rule file will be used instead of the model in the GXL file.
...
\begin{itemize}
	\item for one for post-problem debugging, you can execute your transformation, and after a problem occurred inspect the changes that were leading to it
	\item for the other for ensuring persistence of changes as they happen, in case you are using \GrG\ as an application-embedded in-memory graph-database
\end{itemize}
...
You may supply skip-like \texttt{update} parameters to the persistent graph at database opening, cf.~\ref{persistentgraphparameters}, which auto-confirm the changes, saving you/the user from the necessity to confirm them
...
If you forgot to delete the instances / attributes of a type before deleting the type itself (or instances of enum cases before the enum cases), you have to revert the model or temporarily insert the type (the enum cases) again manually, to successfully load the export/database with the model in order to carry out the required deletions (the partial model written by the database could be of help regarding this).
