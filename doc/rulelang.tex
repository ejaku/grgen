...
\begin{note}
  The \GrG\ rule set language does not distinguish between \indexed{declaration}s and \indexed{definition}s. More precisely, every declaration is also a definition. That means, that every element of a LHS (left hand side, see section \ref{patternpart}) is actually mapped by a match. 
\end{note}
\mbox{ }\\
\emph{ModelIdent}, \emph{TypeIdent}, \emph{NodeType}, \emph{EdgeType}\\
These are (semantic) specializations of \emph{Ident}. \emph{TypeIdent} matches every type identifier, i.e.\ a node type, an edge type, an enum type or a primitive type. All the type identifiers are actually type \emph{expressions}\indexmain{type expression}. See \ref{typeexpressions} for the use of type expressions.\\
...
In contrast to pure syntatic scoping, the replace/modify part is a direct inner scope of the pattern part.
...
\end{rail}
Specifies an edge\indexmain{edge (graphlet)}. Anonymous edges are specified by \texttt{-->} or \texttt{<--}. For a more detailed specification you can use an inplace notated edge refinement clause. Type constraints\indexmain{type constraint} are allowed in the pattern part only (see section \ref{typeexpressions}, \emph{TypeConstraint}). The \texttt{<>} operator retypes\indexmain{retyping} an edge. Retyping is allowed in the replace/modify part only (see section \ref{replacepart}, \emph{Retyping}).\\
\begin{center}
  \begin{tabular}[c]{ll}
    \textbf{Graphlet} & \textbf{Meaning}\\ \hline
...
\begin{figure}[htbp]
\begin{note}
The \indexed{application} of a rule is not deterministic\indexmain{indeterminism} (remember the introducing example in \ref{ov:example}); in particular there may be more than one subgraph that matches the pattern. 
...
Also notice that the \indexed{regular graph rewrite sequence}s introduces a further variant of indeterminism on rule application level: 
The \indexed{\texttt{\$<op>}} flag marks the operator \texttt{<op>} as commutative, i.e.\ the execution order of its operands (rules) is indeterministic. 
...
\paragraph{Implicit Definition of the Preservation Morphism\indexmain{preservation morphism} $r$}
...
\begin{figure}[htbp]
  ...
  \label{rule:figrule}
\end{figure}
In \GrG\ the preservation morphism $r$ is defined implicity by using of \indexed{name}s in pattern \indexed{graphlet}s and replace graphlets. (The modify part can be assumed to be a special case of a replace part).
...
So \indexed{anonymous} graph elements in a replace pattern will be created.
Multiple using of a name has the same effect as a single using occurrence.\\
In case of a conflict between \indexed{deletion} and \indexmain{preservation}, deletion is prioritized. 
...
\begin{description}
  ...
  \item[Attribute Evaluation.] \indexmain{attribute evaluation}\indexmainsee{evaluation}{attribute evaluation}\indexmainsee{re-evaluation}{attribute evaluation}If a rule is applied, then the attributes of matched and inserted graph elements will be recalculated.
  ...
  \item[Retyping.] \indexmain{retyping}Retyping enables us to keep all adjacent nodes and all attributes stemming from common super types of a graph element while changing its type. Retyping differs from a \indexedsee{type cast}{retyping}: During replacement both of the graph elements are alive. Specifically both of them are available for evaluation. Furthermore the source and destination types need not to be on a path in the directed \indexed{type hierarchy} graph, rather their relation can be arbitrary.\\
