...
\end{rail}\ixkeyw{test}\ixkeyw{rule}\ixkeyw{dpo}\ixnterm{TestDeclaration}\ixnterm{RuleDeclaration}
Declares a single \indexed{rewrite rule} such as \texttt{SomeRule}. It consists of a pattern part (see Section~\ref{patternpart}) in conjunction with its rewrite/modify part (see Section~\ref{replacepart}). The modifier \texttt{dpo} switches to dpo semantics. See section \ref{sct:patternmodifier} for an explanation of available rule and pattern modifiers. A \newterm{test} has no rewrite specification. It's intended to check whether (and maybe how many times) a pattern occurs.
\begin{example}
...
\begin{rail}  
  PatternStatement: 
    ...
    'return' '(' (Ident+',') ')' ';' |
    ('exact' | 'induced') '(' (Ident+',') ')';
\end{rail}\ixkeyw{induced}\ixkeyw{exact}\ixkeyw{hom}\ixkeyw{negative}\ixkeyw{if}\ixkeyw{return}\ixnterm{PatternStatement}
...
\begin{rail}
  Replace: ('replace' | 'modify') lbrace (()+ReplaceStatement) (()+ExecStatement) \\ 
  	(() | ReturnStatement) rbrace ;
...
\begin{rail}  
  ExecStatement: 'emit' '(' StringExpr ')' ';' |
    'exec' lbrace (RewriteSequence+';') rbrace
...
That means, these statements are executed in the order as they appear within the rule.
...
\begin{description}
  \item[Text Output] The \texttt{emit} statement prints a string to \texttt{stdout}. See section \ref{expressions} for a description of string expressions.
...
\indexmain{return value}Graph elements of the replace/modify part can be returned according to the return types in the signature (see Section~\ref{ruledecls}, \texttt{ActionSignature}). 
The graph element names have to be in the same order as the corresponding return types in the signature. 
The named elements must be compatible to the declared type.
...
Graph rewrite sequences are an imperative enhancement to the rule set language.
Graph rewrite seuences are syntactically expressed as expressions similar to boolean expressions.
Composing single graph rewrite rules to an expression not only yields more complex host graph operations but also determines sort of a control flow by means of evaluation order of the operands.
Graph rewrite sequences have a boolean return value; 
For a single rule, \texttt{true} means the rule could successfully applied to the host graph.
A \texttt{false} return value means that the pattern was not found in the host graph.
Graph rewrite sequences are transactional.
If the return value is \texttt{false}, all the related operations on the host graph will be rolled back.

In order to pass return values of rules to other rules, \emph{parameters} can be defined.
A parameter is an arbitrary identifier which is defined by assigning it a graph element or another parameter.
A parameter is alive from its first assignment until the end of the enclosing \texttt{exec} statement.

\makeatletter
\begin{rail}
  RewriteSequence: ((() | '!') RewriteTerm (() | ('*' | lbrace Number rbrace))) + ((() | dollar) ('|' | ampersand | '||' | doubleampersand));
  RewriteTerm: (() | '(' (Parameter+',') ')' '=') \\ (() | '\%') (() | '?') ('[' Rule ']' | Rule) |
    'def' '(' (Parameter+',') ')' |
    'true' |
    'false' |
    Parameter '=' (NodeOrEdge | Parameter) |
    '(' RewriteSequence ')' ;
  Rule: RuleIdent (() | '(' ((NodeOrEdge | Parameter)+',') ')');
\end{rail}\ixnterm{RewriteSequence}\ixnterm{RewriteTerm}\ixnterm{Rule}\ixnterm{Parameter}
\makeatother
\mbox{\quad}\\
Table~\ref{ruletab} lists graph rewrite expressions at a glance. Unless you use the \texttt{\$} modifier, operands will evaluated from left to right. 
...
\begin{tabularx}{\linewidth}{|lX|}
\hline
\texttt{s | t}	& First execute \texttt{s} afterwards execute \texttt{t}. The sequence \texttt{s | t} is successfully executed iff \texttt{s} or \texttt{t} is successfully executed.\\
\texttt{s || t}		& The same as \texttt{s | t} but with lazy evaluation, i.e.\ if \texttt{s} is successful, \texttt{t} will not be executed,\\
\texttt{s \& t}	& First execute \texttt{s}, afterwards execute \texttt{t}. The sequence \texttt{s \& t} is successfully executed iff \texttt{s} and \texttt{t} is successful.\\
...
\texttt{\$<op>}	& Flag the operator \texttt{<op>} as commutative. Usually operands are executed/evaluated from left to right with respect to bracketing (left-\indexed{associative}). In contrast the sequences \texttt{s}, \texttt{t}, \texttt{u} in \texttt{s \$<op> t \$<op> u} are executed/evaluated in arbitrary order. \\
...
\texttt{?\emph{Rule}} & Switches \emph{Rule} to a \texttt{test}, i.e.\ does not perform the rewrite part of the rule but only tests if a match exists. \\
\texttt{\%\emph{Rule}} & This is a multi-purpose flag. In the \GrShell\ (see chapter \ref{chapgrshell}) it dumps the matched graph elements to \texttt{stdout}; in \texttt{debug}-mode (see section \ref{sct:debugger}) it acts as a \indexed{break point}; you are also able to use this flag for your own purposes, when using \GrG\ via its API interface (see section \ref{sct:API}).\\
\texttt{\emph{Rule}} & Rewrite the first found pattern match produced by the action \emph{Rule}.\\
\texttt{[\emph{Rule}]} & Rewrite every pattern match produced by the action \emph{Rule}. \textbf{Note:} Its semantics is not equal to \texttt{Rule*}. Instead this operator collects all the matches first before starting rewritings. In particular the semantics is unsafe, i.e.\ one needs to avoid deleting a graph element that is bound by another match. If \emph{Rule} returns values, the values of \emph{one} of the executed rules will be returned.\\
