...
\makeatletter
\begin{rail}
  RewriteSequence: ((() | '!') RewriteTerm (() | ('*' | lbrace Number rbrace))) + ((() | dollar) ('|' | ampersand | '||' | doubleampersand | xorhat));
...
\texttt{s \^ t}	& First execute \texttt{s} afterwards execute \texttt{t}. The sequence \texttt{s \^ t} is successfully executed iff \texttt{s} is successful or \texttt{t} successfully but not both; so this an XOR sequence.\\
