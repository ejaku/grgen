...
\begin{rail}  
  ReplaceStatement:
    Graphlet ';' |
    'delete' '(' (Ident + ',') ')' ';' |
    'eval' lbrace ((Assignment|SetMapStateChange) ';' +) rbrace |
...
\begin{rail}    
  ...
  SetMapStateChange:
	...
	;
\end{rail}\indexmain{attribute evaluation}\ixnterm{Assignment}\ixnterm{SetMapStateChange}\ixkeyw{visited}
...
In addition, there are by-ref parameters available which can be operated upon by the set/map state change methods,
which allow to only partially change the set/map by adding and removing elements resp. pairs of elements;
they are especially useful for storages, i.e. sets or maps containing nodes or edges.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Type Handling and Storage Access}

In the following section we'll have a look at language constructs to constrain the types to match by excluding forbidden types or by requiring an element to be typed the same as another element. Furthermore we learn about the storage access constructs.
...
%-----------------------------------------------------------------------------------------------
\subsection{Storage Access} \label{sub:storageaccess}\indexmain{storage access}
Storages are sets or maps (cf. \ref{sec:builtintypes}) storing nodes or edges. They can be used in the rule control language as defined here \ref{sec:storages}, they can get filled or emptied in the rules as defined here \ref{replstmt}, a discussion about their usage is given here \ref{sub:copymergesplit}.
You may ask for an element to get bound to an element from a storage; this is syntactically specified by giving the storage enclosed in left and right angles.
...
\indexmain{copy}The copy operator allows to create a node or edge of the type of another node/edge, bearing the same attributes as that other node. It can be seen as an extended version of the \texttt{typeof} construct not only copying the exact dynamic type but also the attributes of the matched graph element. Together with the \texttt{iterated} construct it allows to simulate node replacement grammars or to copy entire structures, see \ref{sub:copymergesplit}.
