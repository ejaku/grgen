...
The \indexed{application} of a rule is not deterministic\indexmain{non-determinism}\indexmainsee{determinism}{non-determinism} (remember the introductionary example in \ref{ov:example}); in particular there may be more than one subgraph that matches the pattern. 
...
\begin{figure}[htbp]
  ...
  \label{rule:figrule}
\end{figure}
In \GrG\ the preservation morphism $r$ is defined implicitly by using \indexed{name}s in pattern \indexed{graphlet}s and replace graphlets. (We don't need to care about the the differences of the replace part and the modify part at the moment.)
...
\begin{description}
  \item[Replace mode.] \indexmain{replace mode}The semantics of this mode is to delete every graph element of the pattern that is not used (denoted) in the replace part, keep every graph element that is used, and create every additionally defined graph elements. ``Using'' means denoting a graph element in a graphlet. Attribute calculations are no using occurrences.\\
