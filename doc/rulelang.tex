...
The braces (\texttt{\{\}}) let execute a sequence repeatedly as long as its execution does not fail but \emph{Number} times at most.
...
\texttt{s \^\ t}	& First execute \texttt{s} afterwards execute \texttt{t}. The sequence \texttt{s \^\ t} is successfully executed iff \texttt{s} is successful or \texttt{t} successfully but not both; i.e.\ this an XOR sequence.\\
