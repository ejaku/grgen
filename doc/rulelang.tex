...
\GrG\ provides graphlets as a descriptive notation to define both, patterns to search for as well as the subgraphs that replace or modify matched spots in a host graph. 
...
All the graph elements of a graphlet have \emph{name}s.
...
For example the graphlet \texttt{n1 --> n2} contains a anonymous edge; thus can be understood as \texttt{n1 -\$1:Edge-> n2}.
Names must not be redefined; once defined, a name is \emph{bound} to a graph element. 
...
\begin{center}
  ...
  \end{tabular}
\end{center} 
As the above table shows, edges can be defined and used separately, i.e.\ without their incident nodes. Beware of accidentally ``redirecting''\footnote{You cannot directly express the redirection of edges. This a direct consquence of the SPO approach. Redirection of edges can be ``simulated'' by either deleting and re-inserting an edge or more indirect by re-typing of nodes.} an edge: 
The graphlets
\begin{grgenlet}
-e:Edge-> .;
x:Node -e-> y:Node;
