...
The advanced \emph{modifiers} introduced in the following section allow annotating patterns or actions with keywords which restrict what graph patterns are accepted as matches (some of them independent of the rewrite part, some of them depending on the rewrite specification).
...
they allow requesting more fine-grained or more dynamically what types to match, as well as specifying a match from a storage.
...
\indexmain{copy}\indexmain{clone}The copy and clone operators allow creating a node or edge of the type of another node/edge, bearing the same attributes like that other node/edge.
