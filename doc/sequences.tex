...
\begin{rail} 
  RewriteFactor: 
	...
	;
\end{rail}\ixnterm{RewriteFactor}\ixkeyw{emit}
...
Further on, there's the \texttt{emit} sequence: it writes a double quoted string or the value of a variable to the emit target (stdout as default, or a file specified with the shell command \texttt{redirect emit}).
...
In particular the semantics is unsafe, i.e.\ one needs to avoid deleting or retyping a graph element that is bound by another match.
...
\begin{rail} 
  ...
  SequencesList:
	...
	;
\end{rail}\ixnterm{ExtendedControl}\ixnterm{SequencesList}\ixkeyw{if}\indexmain{\texttt{<>}}

The extended control constructs offer further rule application control in the form of \indexed{conditional sequence}s, \indexed{transaction}s, parenthesis, \indexed{sequence constant}s, and \indexed{indeterministic choice} from a sets of rules or sequences.
The condition execution statement \texttt{if} executes the condition sequence, and if it yielded true executes the true case xgrs, otherwise the false case xgrs.
...
\texttt{\$\{r1,[r2],\$[r3]\}}	& Tries to match all contained rules, then rewrites one of the rules which matched. True if at least one matched.\\
