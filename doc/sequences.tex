...
The random all of operators (which are currently only available in the interpreted sequences) given in function call notation with the operator symbol as name have the following semantics:
...
A \texttt{yield} term yields the variable values to the containing \texttt{exec} which can include the yielded values into the RHS pattern; but as of now they are only accesible from the rule \texttt{return} statement.
Yielding is only applicable to compiled sequences. 
