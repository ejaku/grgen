...
The at operator allows to access a graph element by its \indexed{persistent name} (this is not supported in the compiled sequences because names are not available on that level).
...
\begin{rail}
  SpecialSequenceExpression:
    ...
    MethodCall;
  ;
\end{rail}\ixnterm{SpecialSequenceExpression}\ixkeyw{in}\ixkeyw{visited}
...
\end{minipage}\\
\\ 
{\small Let \texttt{u}, \texttt{v}, \texttt{w} be variable identifiers, \texttt{e} and \texttt{f} be expressions, \texttt{t} be an assignment target, \texttt{<op>} $\in \{\texttt{|}, \texttt{\textasciicircum}, \texttt{\&}, \texttt{||}, \texttt{\&\&}\}$ }%and \texttt{n}, \texttt{m} $\in \N_0$.}
