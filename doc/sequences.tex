\chapter{Extended Graph Rewrite Sequences (XGRS)}\indexmain{rule application language}
\label{chapruleapplang}

Graph rewrite sequences\indexmain{graph rewrite sequence} are a domain specific GrGen.NET language
...
Allow assignments of the success of a sequence to a variable
    (e.g. \begin{grgenlet}res:boolean=(r1 || r2[5])\end{grgenlet})
  - Allow boolean variables as sequences (only works in execs, yet)
%    (e.g. \begin{grgenlet}res && r1\end{grgenlet})
%  - Added random-match-selector
%    (e.g. \begin{grgenlet}$7[r1]\end{grgenlet} searches for all matches and then randomly selects 7 of them to be rewritten. 
%\begin{grgenlet}$[r1]\end{grgenlet} is equivalent to \begin{grgenlet}$1[r1]\end{grgenlet})
%- Allow to write \begin{grgenlet}r1*\end{grgenlet} instead of \begin{grgenlet}r1[*]\end{grgenlet} in execs
