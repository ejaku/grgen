...
a = ((b,c)=R(x,y,z))
	\end{grgen}
assign the two returned graph elements from rule \texttt{R} to variables \texttt{b} and \texttt{c} and the information whether \texttt{R} matched or not to variable \texttt{a}. The first version is recommended.
\end{example}
\end{figure}
...
Here we start with the most important one, applying rules.
...
Furthermore, a boolean constant may be used as a predicate. 
These \indexed{sequence constant}s being one of the boolean literals \texttt{true} or \texttt{false} come in handy if a sequence is to be evaluated but its result must be a predefined value; furthermore a \indexed{break point} may be attached to them.
