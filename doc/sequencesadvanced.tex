...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Multi-Rule Transactions}\label{sec:multitransaction}

\begin{rail}
  MultiBacktracking: '<<' MultiRuleExecution ';;' RewriteSequence '>>';
\end{rail}\ixnterm{MultiBacktracking}

The \emph{MultiBacktracking} clause searches for all matches of all contained rules and then executes the sequence for each found match, rolling back the changes in case the sequence fails, continuing with the next match,
or committing the changes in case the sequence succeeds, ending the execution of the sequence (as success).

Filters may be applied to the rules in the construct (e.g. \verb#r\f#), but especially: match class filters may be applied to the multi rule execution part \verb#([[...]\f])# -- they allow to bring the matches of the different rules into a global order, so only the most promising ones are followed, while the other ones are purged.

\begin{rail}
  MultiRulePrefixedSequenceBacktracking: '<<' MultiRulePrefixedSequence '>>';
\end{rail}\ixnterm{MultiRulePrefixedSequenceBacktracking}

The \emph{MultiRulePrefixedSequenceBacktracking} construct searches for all matches of all contained rules, and then, for each found match (one-by-one): starts a transaction, rewrites the match, and then executes the corresponding sequence, if the result is false, the effects on the graph are rolled back and the next match is tried, otherwise the changes are committed and the overall sequences succeeds (without processing further matches; if all matches lead to a failing sequence, the overall sequence fails)
...
(note that as for the non-backtracking multi rule prefixed sequence, the rewrites are performed in order of the globally filtered matches list, in contrast to the syntactic sequence of rule prefixed sequences within the construct).
