...
If one \texttt{s} succeeded, the overall sequence succeeds, otherwise it fails.
...
The sequence \verb#<<r;;t>># allows us directly to do so, it fails iff none of the possible applications of \texttt{r} can make a following \texttt{t} succeed, it succeeds with the first application of \texttt{r} for which \texttt{t} succeeds. 
\end{example}
...
If one sequence succeeded, the overall sequence succeeds, otherwise it fails.
...
if the left is lower than or equal to the right the variable is incremented from left on in steps of one until right is reached (inclusive),
...
Note that rollback only occurs when the sequence crashes, otherwise at the closing angle, the database transaction will be committed.

Also note that the persistence provider transactions aka database transactions are unrelated to the application logic layer transactions that are processed in-memory (an undo log is recorded, that is executed upon rollback).
...
Of course it makes sense to couple the persistence provider transactions / commit-and-restart command with the application logic transactions introduced above at commit/rollback (or backtracking of the backtracking double angles) -- you have to do so explicitly. Transactional behavior and change rollback in \GrG{} is an application layer construct, with persistence provider transactions you only define the granularity of durability/performance of the changes from the layer above.
