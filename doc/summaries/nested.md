...
- Elements matched separately from main pattern (no contribution to match combinations and no rewrite possible)
...
- Filters can be appended (`FiltersDecl`); see `filters.md`
- Warning: enumeration blocker in all-mode -- yields only one match per instance even when used from an all-bracketed rule `[r]`
...
- `alternative { Case1 { ... } Case2 { ... } }` -- at least one case must match (otherwise fails); exactly one case is matched (other possible case matches are ignored)
...
- `negative name { ... }`, `iterated name { ... }`, etc. -- required for filtering (iterated), rewrite order specifications (alternative and iterated), and yield
