...
In this chapter we'll have a look at transformation techniques to solve common graph rewriting tasks utilizing the constructs introduced so far.
...
With \emph{embedded sequences} you are able to call rules from within a rule application, after the rule proper was executed (deferred execution), with direct access to the elements of that rule.
Seen from the outside, they allow you to build a complex rule and thus allow for rule \emph{internal composition}, again in an \emph{imperative} way.
...
Avoid rules carrying out a tiny amount of work with a lot of input and output and heavy external orchestration (graph-rewriting assembler-level style, to be considered an anti-pattern), do as much as you can per rule call (but note that when you press too hard regarding this you can get over the top regarding understandability, so maybe a medium amount of work per rule call is the right solution).
\end{note}

\begin{note}
An alternative to storage containment are visited flags, they allow to mark interesting elements directly.
The storage sets are more efficient compared to the visited flags in case the count of elements of interest is a good deal smaller than the number of elements in the graph; they are looked up in the set, in contrast to the visited flags, which are used by enumerating all available graph elements, filtering according to the visited state.
...
A step towards mapping is now available with the mapping clause, cf. Section~\ref{sec:mappingclause}, which applies a rule (a single one, in a single step, in contrast to typical mapping tool usages, which apply multiple rules to reach a complete mapping to the result graph).
It yields several result graphs, as many as there were matches, created by cloning the original host graph, then applying the rewrite part of the rule.
...
The graph model might be a union of multiple graph models, and the graph may consist of multiple unconnected components, so you don't lose anything regarding expressiveness here.

What you lose is the automatic construction of the \indexed{traceability} links which allow to retrieve the source node from a mapped node or the mapped node from a source node, which is normally carried out by a mapping tool in the background.
...
This comes at the price of always having to specify the meta-model, similar to how you specify the model in a mapping tool.
...
So while you can write mapping (\emph{out-of-place}) transformations leaving the source untouched, we recommend rewriting (\emph{in-place}) transformations (unless you have a strong real need for the former).
...
In the example we match the entire subgraph from a root node on, if one wants to copy a more constrained subgraph one can simply constrain the types, directions, and structures in the iterated subpattern covering the nodes.
...
You might be interested in the transparencies under \url{http://www.imm.dtu.dk/~hrni/PPA/slides2.pdf} for some reading on this topic, but as of 2026, they seem to have gone for good, you should find the same knowledge in \cite{dataflowanalysis};
especially as this is a general method to compute non-local information over graphs not limited to compiler construction.
...
instead of computing the iterated path with a recursive pattern each and every time you must check for it,
...
rule addToWorklist(p:N, ref todo:set<N>, last:N) : (N)
{
  ...
  \begin{example}
An example rule for worklist handling, removing the by-then processed node \texttt{pos} from the worklist.
    \begin{grgen}
...
\subsubsection*{State Space Enumeration by Mapping}

Since the introduction of the mapping clause (cf. Section~\ref{sec:mappingclause}, and \ref{sec:transfo} above), another approach is possible.
You don't partition the host graph into subgraphs anymore, and don't carry out changes in order to roll them back, but map an original graph to result graphs (with the state graphs being contained in some data structure, for better visualization still in a graph, the statespace graph).
You find an implementation of that approach in the \texttt{tests/statespaceMapping} folder, the parallelized versions offer a \emph{breadth-first} statespace enumeration and a statespace graph (see Section~\ref{sec:sequenceparallelization} for more on sequence parallelization), the non-parallelized versions offer a \emph{depth-first} statespace enumeration, and state graph storage in object structures (utilizing the object types from Section~\ref{sec:objecttypes}).
Both versions (sequential and parallelized enumeration) are available without and with isomorphic graphs pruning (the former yielding a tree, the latter a graph).
