...
These functionalities are also available on API level, you can add graph visualization, stepwise sequence/rule debugging, or even shell scripting to your end user app.
...
You find the binaries (tool applications and components) in the \texttt{bin} subdirectory of your \GrG\ installation.
%tools: to be distinguished from the tools of engine-net-2
...
\begin{itemize}
  ...
  \item \emph{Name}\texttt{Model.cs}---the C\# source file(s) of the \emph{rule-set}\texttt{Model.dll} assembly.
...
\begin{tabularx}{\linewidth}{lX}
    ...
    \end{tabularx}
  \item[Requires] .NET Framework 4.0 (or above) or Mono 3.0 (or above).
\end{description}
...
The grgen compiler consists of a frontend \texttt{FrontendGrGen.exe} used by the C\# backend and driver \texttt{GrGen.exe}.
The frontend can be executed directly to get a visualization of the model and the rewrite rules,
in the form of a dump of the compiler IR as a .vcg file:\\
\texttt{FrontendGrGen.exe -i yourfile.grg}
\end{note}

\begin{note}
If you run into \texttt{Unable to process specification: The system cannot find the file specified} errors, 
\texttt{GrGen.exe} is typically not able to execute the frontend \texttt{FrontendGrGen.exe}.
Likely the system is not referenced in the search path, the error message should give further hints.
\end{note}
...
The WindowsForms extension of the \texttt{graphViewerAndSequenceDebugger.dll} contains code that allows to use the MSAGL graph viewer, and debug sequences with a graphical debugger.
...
The \texttt{libGrPersistenceProviderSQLite.dll} contains code that allows for persistent storage of the host graph to an SQLite database file (including writing of ongoing changes, so they are available at the next program start even in case of a crash).
...
\label{sct:API}
The \LibGr\indexmain{libGr} (graph rewrite library) is a .NET assembly defining \GrG's \indexed{API}.
...
Besides, it implements some of the functionality (generically, independent of backends or the concrete models).
...
The base library \texttt{libConsoleAndOS.dll} offers interfaces for text consoles and a basic implementation, as well as some OS abstractions (you have to include it in your own application if you want to use \GrG, also the libGr requires it).

%-----------------------------------------------------------------------------------------------
\section{\texttt{libConsoleAndOSWindowsForms.dll}}
The WindowsForms extension of the \texttt{libConsole\-And\-OS.dll} contains a GUI console implementation for WindowsForms apps (that is used by the debugger from the \texttt{graph\-Viewer\-And\-Sequence\-Debugger\-Windows\-Forms.dll} and the \GGrShell, you have to include it in your own WindowsForms application if you want to use \GrG).
...
In order to use the persistent graph, or better the SQLite persistence provider on API level, you have to ensure that the \texttt{System.Data.SQLite.dll} is available in your search path, besides the \texttt{libGrPersistenceProviderSQLite.dll}, and the \texttt{e\_sqlite3.dll} in case of Windows or the \texttt{libe\_sqlite3.so} in case of LINUX (note that the latter two libraries are native x64-builds of SQLite, and that the release of \GrG{} is a 64bit-only version by now, you may build the x86-version(s) on your own, see Section~\ref{sub:building}).
...
\begin{note}
If yComp doesn't start: check whether your path contains a zombie java variable from an old installation pointing to nowhere; remove it in this case.
Normally just installing a JRE is sufficient, but it may help to install a JDK to a non system path and adding the bin folder of the JDK to the path variable.
It could be necessary to utilize the OpenJDK in an old version (e.g. \texttt{1.8}) to get a running yComp.jar because of incompatibilities introduced later on (or may be necessary to use JAVA from Oracle, which is normally the better choice regarding compatibility anyway).
\end{note}

%-----------------------------------------------------------------------------------------------
\section{Birds eye view}\label{birdseyeview}

Figure~\ref{figcomponentsapps} gives an overview of the \GrG\ system components and applications (with their dependencies).
...
Figure~\ref{figtla} depicts how an application you build with \GrG\ (at API level) would look like 
when fit into a traditional three layer architecture (TLA), composed of a presentation layer facing the user, the application logic layer (with the algorithmic core), and the persistence layer.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{fig/ThreeLayerArchitecture}
  \caption{\GrG\ three layer architecture (draft)}
  \label{figtla}
\end{figure}
