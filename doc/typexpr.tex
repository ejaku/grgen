...
\begin{rail} 
  PrimaryExpr: CastExpr | MemberAccess | VarIdent | 'visited' '(' NodeOrEdge (',' FlagNumber)? ')' | 'nameof' '(' NodeOrEdge? ')' | '(' Expression ')';
...
The visited flags are written in the assignments of the eval statements (see \ref{Assignment}).
