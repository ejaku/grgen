...
A cast to a string value of an enum value will return the declared name of the enum value.
\begin{table}[htbp]
  \centering
...
\texttt{String} types, \texttt{boolean} types, and \textttt{object} types support only the \texttt{==} and the \texttt{!=} operators.
