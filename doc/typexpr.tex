...
In the following sections \emph{Ident} refers to an identifier of the graph model language (see \ref{modelbb}) or the rule set language (see \ref{rulebb}). \emph{TypeIdent} is an identifier of a type.
...
\label{builtintypes}
\end{table}
Table \ref{tabcasts} lists \GrG's implicit type casts and the allowed explicit type casts. Do not express an implicit type cast by an explicit type cast or ``cast'' an enum type to itself.
\begin{example}
...
\begin{table}[htbp]
  ...
  \begin{tabular}[c]{|c|cccccc|} \hline
    ...
    \texttt{int} & \texttt{(int)} & & $=$/\texttt{(int)} & \texttt{(int)} & \texttt{(int)} & \\
    \texttt{float} & \texttt{(float)} & & implicit & $=$/\texttt{(float)} & \texttt{(float)} & \\
    \texttt{double} &  \texttt{(double)} & & implicit & implicit & $=$/\texttt{(double)} & \\
    \texttt{string} & \texttt{(string)} & \texttt{(string)} & \texttt{(string)} & \texttt{(string)} & \texttt{(string)} & $=$/\texttt{(string)} \\ \hline
  \end{tabular}
...
\end{rail}
The $\sim$ operator is the bitwise complement. That means every bit of an integer value will be flipped. The \texttt{?} operator is a simple if-then-else: if the \emph{BoolExpr} is evaluated to \texttt{true}, the operator returns the first \emph{IntExpr}, otherwise it returns the second \emph{IntExpr}.\\ 
