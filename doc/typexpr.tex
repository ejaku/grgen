...
The \texttt{double} is the default floating point type, in the table it stands for all the floating-point types.
The integer types are implicitly cast upwards from smaller to larger types (\texttt{byte < short < int < long}), whereas a downcast requires an explicit cast.
The \texttt{byte} and \texttt{short} types are not used in computations, they are cast up to \texttt{int} (or \texttt{long} if required by the context.)
The floating point types are implicitly cast upwards (\texttt{float < double}), and require an explicit cast downwards, too.
As specified by the table, integer numbers are automatically cast to floating point numbers, and castable with an explicit cast vice versa.
...
Everything is implicitly cast to \texttt{string} to enable concise text output without the need for boilerplate casting.
...
They bind more weakly than the relational expressions which bind more weakly than the other expressions.
...
They are an intermediary that allows you to obtain the boolean type required by the decisions from the other types.
...
the semantics is given by Table~\ref{compandarithmetic} (by implicit casting they can also be used with all enum types).
...
Furthermore, it also allows to compare class objects and transient class objects for deep attribute value equality (preliminary) instead of only reference identity, see Subsection \ref{subsec:objectequality}, and is available as an extension in this rôle for graph element and container types (as well as external object types).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{String Expressions}\label{sec:stringexprs}
...
\begin{rail}
  ...
  MethodSelector: '.' FunctionIdent '(' (Expression*',') ')' (MethodSelector)?;
...
The methods given with the \emph{MethodSelector} may be chained (the function methods -- all string methods are function methods, see~\ref{sub:funcmethcall} for more on this topic we preview here, but the container types introduced in Chapter~\ref{sec:builtingenerictypes} also offer procedure methods (cf.~\ref{sub:procmethcall}), those are statements and not expressions and cannot be chained).
...
\item[\texttt{.asArray(separator)}] returns the original string exploded to an array of substrings (\texttt{array<string>}), at each occurrence of the \texttt{separator:string}; the separators are not included in the array. If separator is empty, you get the original string exploded to an array of single-character strings (an \texttt{array<string>} contains an \texttt{asString} method for reversal). 
\end{description}
...
\end{rail}
A function call employs an external (attribute evaluation) function (cf. \ref{sub:extfct}), or an (internal) user-defined function (see \ref{sub:functions}, there you find the syntax definition we replicate here as a preview), or a built-in function (you find more on the \emph{IndexFunctionCall} in \ref{sec:attributeindices} and on the \emph{ScanTryscanCall} in \ref{sec:scantryscan}).
...
The cast expression returns a value cast to a new type, it is specified by a type in parentheses before the expression computing the original value.
Another primary expression is an arbitrary expression enclosed in parentheses.
