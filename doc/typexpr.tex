\chapter{Basic Types and Expressions}
\label{cha:typeexpr}
...
A function call employs an external (attribute evaluation) function (cf. \ref{sub:extfct}), or a (internal) user-defined function (see \ref{sub:functions}), or one of the following built-in functions:
...
\item[\texttt{abs(.)}] returns the absolute value of the argument, which must be of numeric type (i.e. \texttt{byte} or \texttt{short} or \texttt{int} or \texttt{long} or \texttt{float} or \texttt{double})
\item[\texttt{pow(.,.)}] returns the first argument value to the power of the second value; both must be of type \texttt{double}
\item[\texttt{pow(.)}] returns $e$ to the power of the argument value, which must be of type \texttt{double}
\item[\texttt{log(.,.)}] returns the logarithm of the first argument value regarding the base given by the second value; both must be of type \texttt{double}
\item[\texttt{log(.,.)}] returns the logarithm of the argument value that must be of type \texttt{double} regarding the base $e$
\item[\texttt{sin(.)}] returns the sine of the argument, which must be of type \texttt{double}
\item[\texttt{cos(.)}] returns the cosine of the argument, which must be of type \texttt{double}
\item[\texttt{tan(.)}] returns the tangent of the argument, which must be of type \texttt{double}
\item[\texttt{arcsin(.)}] returns the inverse of the sine for the argument, which must be of type \texttt{double}
\item[\texttt{arccos(.)}] returns the inverse of the cosine for the argument, which must be of type \texttt{double}
\item[\texttt{arctan(.)}] returns the inverse of the tangent for the argument, which must be of type \texttt{double}
\end{description}
