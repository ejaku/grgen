...
\begin{rail}
  IteratedQuery: '[''?' IteratedIdent ']'
\end{rail}\ixnterm{IteratedQuery}

The iterated query expression \texttt{[?it]} returns an \texttt{array} of \texttt{match<r.it>} 
...
You may apply the \texttt{extract<member>} method and then the array accumulation methods on it to get a value for further processing (cf. \ref{sec:arrayexpr}).
