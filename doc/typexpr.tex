...
\begin{center}
\begin{tabular}[c]{lll}
\begin{tabular}{|l|}\hline\texttt{x:T \char"5C\ (T1 + $\cdots$ + T$n$);}\\\hline\end{tabular} && 
  \begin{tabular}{|l|}\hline\texttt{x:T;} \\ \texttt{if \{!(\emph{typeof}(x) <= T1) \&\& $\cdots$} \\ \texttt{\ \ \ \ \&\& !(\emph{typeof}(x) <= T$n$)\}}\\\hline\end{tabular}
\end{tabular}
\end{center}
\begin{example}
