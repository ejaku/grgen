...
\section{Expressions}\indexmain{expression}
...
\begin{rail} 
  PrimaryExpr: CastExpr | MemberAccess | VarIdent | NodeOrEdge '.' 'visited' ('[' FlagNumber ']')? | 'nameof' '(' NodeOrEdge? ')' | 'random' '(' ( IntExpr )? ')' | '(' Expression ')';
...
\end{rail}\ixnterm{CastExpr}\ixnterm{MemberAccess}
