...
Of course you are free to express an implicit type cast by an explicit type cast as well as ``cast'' a type to itself, except for 
...
\begin{example}
  \texttt{x.myfloat = x.myint; x.mydouble = (float) x.myint; x.mystring = (string) x.mybool;} is allowed,
  \texttt{x.myfloat = x.mydouble;} and \texttt{x.myint = (int) x.mybool;} are forbidden.
  
  Now assume that {\tt myenum1} and {\tt myenum2} have different enum types.
  Then \texttt{myenum1 = myenum2;} as well as
  \texttt{myenum1 = (myenum1\_type) int;} and \texttt{myenum2 = (myenum2\_type) myenum1;} are also forbidden.
\end{example}
\begin{note}
