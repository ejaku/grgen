...
\texttt{String} types, \texttt{boolean} types, \texttt{object} types, and \textt{graph} types support only the \texttt{==} and the \texttt{!=} operators;
for strings they denote whether the strings are the same or not,
on boolean values they denote equivalence and antivalence,
on object types the tell whether the references are the same, thus the objects identical,
and on (sub)graph types they tell whether the (sub)graphs are isomorphic to each other (\indexed{isomorphy checking}/\indexed{graph isomorphy checking}).
