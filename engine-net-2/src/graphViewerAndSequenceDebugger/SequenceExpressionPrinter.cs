...
{
    public class SequenceExpressionPrinter
    {
        readonly IDebuggerEnvironment env;
        readonly SequencePrinter seqPrinter;
        ...
        {
            if(seqPrinter.Context.sequenceIdToBreakpointPosMap != null)
            {
                seqPrinter.PrintBreak((ISequenceSpecial)seqExprRuleQuery);
            }

            HighlightingMode highlightingModeLocal = highlightingMode;
            if(seqExprRuleQuery == seqPrinter.Context.highlightSeq)
                highlightingModeLocal = seqPrinter.Context.success ? HighlightingMode.FocusSucces : HighlightingMode.Focus;

            seqPrinter.PrintSequence(seqExprRuleQuery.RuleCall, seqExprRuleQuery, highlightingModeLocal); // rule all call with test flag, thus [?r]
        }
        ...
        {
            HighlightingMode highlightingModeLocal = highlightingMode;
            if(seqExprMultiRuleQuery == seqPrinter.Context.highlightSeq)
                highlightingModeLocal = seqPrinter.Context.success ? HighlightingMode.FocusSucces : HighlightingMode.Focus;
            ...
            foreach(SequenceRuleCall rule in seqExprMultiRuleQuery.MultiRuleCall.Sequences)
            {
                ...
                seqPrinter.PrintReturnAssignments(rule.ReturnVars, parent, highlightingMode);
                env.PrintHighlighted(rule.DebugPrefix, highlightingMode);
                seqPrinter.PrintRuleCallString(rule, parent, highlightingMode);
            }
            env.PrintHighlighted("]", highlightingModeLocal);
            foreach(SequenceFilterCallBase filterCall in seqExprMultiRuleQuery.MultiRuleCall.Filters)
            {
                seqPrinter.PrintSequenceFilterCall(filterCall, seqExprMultiRuleQuery.MultiRuleCall, highlightingModeLocal);
            }
        ...
        {
            HighlightingMode highlightingModeLocal = highlightingMode;
            if(seqExprMappingClause == seqPrinter.Context.highlightSeq)
                highlightingModeLocal = seqPrinter.Context.success ? HighlightingMode.FocusSucces : HighlightingMode.Focus;
            ...
            foreach(SequenceRulePrefixedSequence seqRulePrefixedSequence in seqExprMappingClause.MultiRulePrefixedSequence.RulePrefixedSequences)
            {
                ...
                if(seqRulePrefixedSequence == seqPrinter.Context.highlightSeq)
                    highlightingModeRulePrefixedSequence = seqPrinter.Context.success ? HighlightingMode.FocusSucces : HighlightingMode.Focus;

                env.PrintHighlighted("for{", highlightingModeRulePrefixedSequence);
                seqPrinter.PrintSequence(seqRulePrefixedSequence.Rule, seqRulePrefixedSequence, highlightingModeRulePrefixedSequence);
                env.PrintHighlighted(";", highlightingModeRulePrefixedSequence);
                seqPrinter.PrintSequence(seqRulePrefixedSequence.Sequence, seqRulePrefixedSequence, highlightingMode);
                env.PrintHighlighted("}", highlightingModeRulePrefixedSequence);
            }

            foreach(SequenceFilterCallBase filterCall in seqExprMappingClause.MultiRulePrefixedSequence.Filters)
            {
                seqPrinter.PrintSequenceFilterCall(filterCall, seqExprMappingClause.MultiRulePrefixedSequence, highlightingModeLocal);
            }
            env.PrintHighlighted(":]", highlightingModeLocal);
        }
