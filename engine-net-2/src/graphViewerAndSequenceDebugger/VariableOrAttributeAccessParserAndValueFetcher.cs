...
namespace de.unika.ipd.grGen.graphViewerAndSequenceDebugger
{
    class VariableOrAttributeAccessParserAndValueFetcher
    {
        ...
        public bool FetchObjectToBeShownAsGraph(SequenceBase seq, out object toBeShownAsGraph, out AttributeType attrType)
        {
            do
            {
                env.outWriter.WriteLine("Enter name of variable or attribute access to show as graph (just enter for abort): ");
                env.outWriter.WriteLine("Examples: \"v\", \"v.a\", \"@(\"$0\").a\" ");
                ...
                if(str.StartsWith("@"))
                {
                    ...
                    if(elem == null)
                    {
                        env.outWriter.WriteLine("Can't parse graph access / unknown graph element: " + str);
                        continue;
                    }
                    if(attributeName == null)
                    {
                        env.outWriter.WriteLine("The result of a graph access is a node or edge, you must access an attribute: " + str);
                        continue;
                    }
                    attrType = elem.Type.GetAttributeType(attributeName);
                    if(attrType == null)
                    {
                        env.outWriter.WriteLine("Unknown attribute: " + attributeName);
                        continue;
                    }
                    object attribute = elem.GetAttribute(attributeName);
                    if(attribute == null)
                    {
                        env.outWriter.WriteLine("Null-valued attribute: " + attributeName);
                        continue;
                    }
                ...
                else
                {
                    ...
                    if(value == null)
                    {
                        env.outWriter.WriteLine("Can't parse variable / unknown variable / null-valued variable: " + str);
                        continue;
                    }
                    ...
                    if(attributeName != null)
                    {
                        ...
                        {
                            env.outWriter.WriteLine("Can't access attribute, the variable value is not a graph element: " + str);
                            continue;
                        }
