    ...
    public class SequenceComputationGenerator
    {
            ...
            for(int i = 0; i < seqEmit.Expressions.Count; ++i)
            {
                if(!(seqEmit.Expressions[i] is SequenceExpressionConstant))
                {
                        ...
                        || seqEmit.Expressions[i].Type(env).StartsWith("array<") || seqEmit.Expressions[i].Type(env).StartsWith("deque<"))
                    {
                        ...
                        source.AppendFrontIndentedFormat("procEnv.{0}.Write(GRGEN_LIBGR.EmitHelper.ToString((IDictionary){1}, graph, false, null, null, null));\n",
                            emitWriter, emitVal);
                        source.AppendFrontFormat("else if({0} is IList)\n", emitVal);
                        source.AppendFrontIndentedFormat("procEnv.{0}.Write(GRGEN_LIBGR.EmitHelper.ToString((IList){1}, graph, false, null, null, null));\n",
                            emitWriter, emitVal);
                        source.AppendFrontFormat("else if({0} is GRGEN_LIBGR.IDeque)\n", emitVal);
                        source.AppendFrontIndentedFormat("procEnv.{0}.Write(GRGEN_LIBGR.EmitHelper.ToString((GRGEN_LIBGR.IDeque){1}, graph, false, null, null, null));\n",
                            emitWriter, emitVal);
                        source.AppendFront("else\n");
                        source.AppendFrontIndentedFormat("procEnv.{0}.Write(GRGEN_LIBGR.EmitHelper.ToString({1}, graph, false, null, null, null));\n",
                            emitWriter, emitVal);
                    }
                    else
                    {
                        source.AppendFrontFormat("procEnv.{0}.Write(GRGEN_LIBGR.EmitHelper.ToString({1}, graph, false, null, null, null));\n",
                            emitWriter, emitVal);
                    }
                }
                else
        ...
        public void EmitSequenceComputationRecord(SequenceComputationRecord seqRec, SourceBuilder source)
        {
            if(!(seqRec.Expression is SequenceExpressionConstant))
            {
                ...
                {
                    source.AppendFrontFormat("if({0} is IDictionary)\n", recVal);
                    source.AppendFrontIndentedFormat("procEnv.Recorder.Write(GRGEN_LIBGR.EmitHelper.ToString((IDictionary){0}, graph, false, null, null, null));\n",
                        recVal);
                    source.AppendFrontFormat("else if({0} is IList)\n", recVal);
                    source.AppendFrontIndentedFormat("procEnv.Recorder.Write(GRGEN_LIBGR.EmitHelper.ToString((IList){0}, graph, false, null, null, null));\n",
                        recVal);
                    source.AppendFrontFormat("else if({0} is GRGEN_LIBGR.IDeque)\n", recVal);
                    source.AppendFrontIndentedFormat("procEnv.Recorder.Write(GRGEN_LIBGR.EmitHelper.ToString((GRGEN_LIBGR.IDeque){0}, graph, false, null, null, null));\n",
                        recVal);
                    source.AppendFront("else\n");
                    source.AppendFrontIndentedFormat("procEnv.Recorder.Write(GRGEN_LIBGR.EmitHelper.ToString({0}, graph, false, null, null, null));\n",
                        recVal);
                }
                else
                {
                    source.AppendFrontFormat("procEnv.Recorder.Write(GRGEN_LIBGR.EmitHelper.ToString({0}, graph, false, null, null, null));\n",
                        recVal);
                }
            }
            else
