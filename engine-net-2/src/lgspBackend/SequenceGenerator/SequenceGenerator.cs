...
namespace de.unika.ipd.grGen.lgsp
{
    ...
    public class SequenceGenerator
    {
        ...
        private void EmitSequenceMultiRuleAllCall(SequenceMultiRuleAllCall seqMulti, SourceBuilder source)
        {
            ...
            {
                EmitMatchClassFilterCall(source, (SequenceFilterCallCompiled)sequenceFilterCall, ((SequenceFilterCallCompiled)sequenceFilterCall).MatchClassName, matchListName);
            }
        ...
        internal void EmitFilterCall(SourceBuilder source, SequenceFilterCallCompiled sequenceFilterCall, string patternName, string matchesName)
        {
            ...
            {
                switch(sequenceFilterCall.Name)
                {
                case "keepFirst":
                    source.AppendFrontFormat("{0}.FilterKeepFirst((int)({1}));\n",
                        matchesName, exprGen.GetSequenceExpression(sequenceFilterCall.ArgumentExpressions[0], source));
                    break;
                case "keepLast":
            ...
            {
                if(sequenceFilterCall.IsAutoGenerated)
                {
                    source.AppendFrontFormat("GRGEN_ACTIONS.{0}MatchFilters.Filter_{1}_{2}(procEnv, {3});\n",
                        TypesHelper.GetPackagePrefixDot(sequenceFilterCall.ImplementationPackageAutoGenerated), patternName, sequenceFilterCall.FullNameUnderscore, matchesName);
                }
                else
                {
            ...
            if(sequenceFilterCall.IsAutoSupplied)
            {
                ...
                case "keepFirst":
                    source.AppendFrontFormat("GRGEN_LIBGR.MatchListHelper.FilterKeepFirst({0}, (int)({1}));\n",
                        matchListName, exprGen.GetSequenceExpression(sequenceFilterCall.ArgumentExpressions[0], source));
                    break;
                case "keepLast":
            ...
            else
            {
                if(sequenceFilterCall.IsAutoGenerated)
                {
                    source.AppendFrontFormat("GRGEN_ACTIONS.{0}MatchClassFilters.Filter_{1}_{2}(procEnv, {3});\n",
                        TypesHelper.GetPackagePrefixDot(sequenceFilterCall.ImplementationPackageAutoGenerated), matchClassName, sequenceFilterCall.FullNameUnderscore, matchListName);
                }
                else
                {
