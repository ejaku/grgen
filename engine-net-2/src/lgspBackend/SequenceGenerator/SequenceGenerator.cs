...
namespace de.unika.ipd.grGen.lgsp
{
    ...
    public class SequenceGenerator
    {
        ...
        {
            if(sequenceFilterCall.IsAutoSupplied)
            {
                IFilter filter = seqHelper.actionsTypeInformation.GetFilterOfRule(packagePrefixedRuleName, sequenceFilterCall.Name);
                IFilterAutoSupplied filterAutoSupplied = (IFilterAutoSupplied)filter;
                source.AppendFrontFormat("{0}.Filter_{1}(",
                    matchesName, sequenceFilterCall.Name);
                bool first = true;
                for(int i = 0; i < sequenceFilterCall.ArgumentExpressions.Length; ++i)
                {
                    ...
                    source.AppendFormat("({0})({1})",
                        ...
                        exprGen.GetSequenceExpression(sequenceFilterCall.ArgumentExpressions[i], source));
                }
                source.Append(");\n");
            }
            else if(sequenceFilterCall.IsAutoGenerated)
            {
                IFilter filter = seqHelper.actionsTypeInformation.GetFilterOfRule(packagePrefixedRuleName, sequenceFilterCall.FullName);
                ...
                source.AppendFrontFormat("GRGEN_ACTIONS.{0}MatchFilters.Filter_{1}_{2}(procEnv, {3});\n",
                    ...
                    patternName, sequenceFilterCall.FullNameUnderscore, matchesName);
            }
            else
            {
                source.AppendFrontFormat("GRGEN_ACTIONS.{0}MatchFilters.Filter_{1}(procEnv, {2}",
                    TypesHelper.GetPackagePrefixDot(sequenceFilterCall.Package), sequenceFilterCall.Name, matchesName);
                List<String> inputTypes = seqHelper.actionsTypeInformation.filterFunctionsToInputTypes[sequenceFilterCall.PackagePrefixedName];
                for(int i = 0; i < sequenceFilterCall.ArgumentExpressions.Length; ++i)
                {
                    source.AppendFormat(", ({0})({1})",
        ...
        internal void EmitMatchClassFilterCall(SourceBuilder source, SequenceFilterCallCompiled sequenceFilterCall, string matchListName)
        {
            if(sequenceFilterCall.IsAutoSupplied)
            {
                IFilter filter = seqHelper.actionsTypeInformation.GetFilterOfMatchClass(sequenceFilterCall.MatchClassPackagePrefixedName, sequenceFilterCall.Name);
                IFilterAutoSupplied filterAutoSupplied = (IFilterAutoSupplied)filter;
                source.AppendFrontFormat("GRGEN_LIBGR.MatchListHelper.Filter_{0}({1}",
                    sequenceFilterCall.Name, matchListName);
                for(int i = 0; i < sequenceFilterCall.ArgumentExpressions.Length; ++i)
                {
                    source.AppendFormat(", ({0})({1})",
                        TypesHelper.TypeName(filterAutoSupplied.Inputs[i]),
            ...
            {
                IFilter filter = seqHelper.actionsTypeInformation.GetFilterOfMatchClass(sequenceFilterCall.MatchClassPackagePrefixedName, sequenceFilterCall.FullName);
                IFilterAutoGenerated filterAutoGenerated = (IFilterAutoGenerated)filter;
                source.AppendFrontFormat("GRGEN_ACTIONS.{0}MatchClassFilters.Filter_{1}_{2}(procEnv, {3});\n",
                    TypesHelper.GetPackagePrefixDot(filterAutoGenerated.PackageOfApplyee), 
                    sequenceFilterCall.MatchClassName, sequenceFilterCall.FullNameUnderscore, matchListName);
            }
            else
            {
                source.AppendFrontFormat("GRGEN_ACTIONS.{0}MatchClassFilters.Filter_{1}(procEnv, {2}",
                    TypesHelper.GetPackagePrefixDot(sequenceFilterCall.Package), sequenceFilterCall.Name, matchListName);
                List<String> inputTypes = seqHelper.actionsTypeInformation.filterFunctionsToInputTypes[sequenceFilterCall.PackagePrefixedName];
                for(int i = 0; i < sequenceFilterCall.ArgumentExpressions.Length; ++i)
                {
                    source.AppendFormat(", ({0})({1})",
