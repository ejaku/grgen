        ...
        void GenerateActionImplementation(SourceBuilder sb, LGSPRulePattern matchingPattern)
        {
            ...
            foreach(IFilter filter in matchingPattern.Filters)
            {
                if(filter is IFilterAutoGenerated)
                {
                    ...
                    if(((IFilterAutoGenerated)filter).Entities.Count >= 1)
                    {
                        sb.AppendFrontFormat("case \"{0}{1}\": ", filter.Name, filterAutoGenerated.EntitySuffix);
                        sb.AppendFrontFormat("GRGEN_ACTIONS.{0}MatchFilters.", TypesHelper.GetPackagePrefixDot(filter.Package));
                        sb.AppendFrontFormat("Filter_{0}_{1}_{2}((GRGEN_LGSP.LGSPGraphProcessingEnvironment)actionEnv, ({3})matches); break;\n",
                            patternName, filter.Name, filterAutoGenerated.EntitySuffixForName, matchesType);
                        if(filter.Package != null)
                        {
                            sb.AppendFrontFormat("case \"{0}{1}<{2}>\": ",
                                filter.Package + "::", filter.Name, filterAutoGenerated.EntitySuffix);
                            sb.AppendFrontFormat("GRGEN_ACTIONS.{0}MatchFilters.", filter.Package + ".");
                            sb.AppendFrontFormat("Filter_{0}_{1}_{2}((GRGEN_LGSP.LGSPGraphProcessingEnvironment)actionEnv, ({3})matches); break;\n",
                                patternName, filter.Name, filterAutoGenerated.EntitySuffixForName, matchesType);
                        }
                    }
                    ...
                    {
                        sb.AppendFrontFormat("case \"{0}\": ", filter.Name);
                        sb.AppendFrontFormat("GRGEN_ACTIONS.{0}MatchFilters.", TypesHelper.GetPackagePrefixDot(filter.Package));
                        sb.AppendFrontFormat("Filter_{0}_{1}((GRGEN_LGSP.LGSPGraphProcessingEnvironment)actionEnv, ({2})matches); break;\n",
                            patternName, filter.Name, matchesType);
                        if(filter.Package != null)
                        {
                            sb.AppendFrontFormat("case \"{0}{1}\": ", filter.Package + "::", filter.Name);
                            sb.AppendFrontFormat("GRGEN_ACTIONS.{0}MatchFilters.", filter.Package + ".");
                            sb.AppendFrontFormat("Filter_{0}_{1}((GRGEN_LGSP.LGSPGraphProcessingEnvironment)actionEnv, ({2})matches); break;\n",
                                patternName, filter.Name, matchesType);
                        }
                ...
                else
                {
                    ...
                    if(filter.Package != null)
                    {
                        ...
                        sb.AppendFrontFormat("GRGEN_ACTIONS.{0}MatchFilters.Filter_{1}((GRGEN_LGSP.LGSPGraphProcessingEnvironment)actionEnv, ({2})matches",
