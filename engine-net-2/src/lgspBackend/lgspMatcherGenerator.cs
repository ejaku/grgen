...
/*            StreamWriter sw = new StreamWriter(dumpname + "-scheduledsp.vcg", false);
            ...
            PatternGraph patternGraph, bool isNegativeOrIndependent)
        {
            ...
            // then schedule all pick from storage/map with storage elements not depending on other elements
            foreach(SearchPlanEdge edge in spGraph.Root.OutgoingEdges)
            {
            ...
            for(int i = 0; i < spGraph.Nodes.Length - spGraph.NumPresetElements - spGraph.NumIndependentStorageElements; ++i)
            {
                foreach(SearchPlanEdge edge in lastNode.OutgoingEdges)
                {
                    if(edge.Target.IsPreset)
                        continue;
                    if(edge.Target.PatternElement.Storage!=null && edge.Target.PatternElement.GetPatternElementTheStorageDependsOn()==null)
                        continue;
            ...
            foreach(IFilter filter in matchingPattern.Filters)
            {
                if(filter is IFilterAutoGenerated)
                {
                    if(((IFilterAutoGenerated)filter).Entity != null)
                        sb.AppendFrontFormat("case \"{1}<{2}>\": MatchFilters.Filter_{0}_{1}_{2}((GRGEN_LGSP.LGSPGraphProcessingEnvironment)actionEnv, ({3})matches); break;\n", patternName, filter.Name, ((IFilterAutoGenerated)filter).Entity, matchesType);
                    else // auto
