...
namespace de.unika.ipd.grGen.lgsp
{
    ...
    public class LGSPSequenceGenerator
    {
        ...
        void EmitRuleOrRuleAllCall(SequenceRuleCall seqRule, SourceBuilder source)
        {
            ...
            {
                if(seqRule.Filters[i].IsContainedIn(rulesToFilters[patternName]))
                {
                    if(seqRule.Filters[i].IsAutoGenerated && seqRule.Filters[i].Name == "auto")
                        source.AppendFrontFormat("MatchFilters.Filter_{0}_{1}(procEnv, {2});\n", patternName, seqRule.Filters[i].Name, matchesName);
                    else if(seqRule.Filters[i].IsAutoGenerated)
                        source.AppendFrontFormat("MatchFilters.Filter_{0}_{1}_{2}(procEnv, {3});\n", patternName, seqRule.Filters[i].Name, seqRule.Filters[i].Entity, matchesName);
                    else
                        source.AppendFrontFormat("MatchFilters.Filter_{0}(procEnv, {1});\n", seqRule.Filters[i], matchesName);
                }
                else
                {
                    if(seqRule.Filters[i].Argument is double)
                        source.AppendFrontFormat("{0}.FilterFirstLast(new GRGEN_LIBGR.FilterCall(\"{1}\", {2}));\n", matchesName, seqRule.Filters[i].Name, ((double)seqRule.Filters[i].Argument).ToString(System.Globalization.CultureInfo.InvariantCulture));
                    else
                        source.AppendFrontFormat("{0}.FilterFirstLast(new GRGEN_LIBGR.FilterCall(\"{1}\", {2}));\n", matchesName, seqRule.Filters[i].Name, seqRule.Filters[i].Argument);
                }
            }
		...
		void EmitSequence(Sequence seq, SourceBuilder source)
		{
			switch(seq.SequenceType)
			{
                ...
                case SequenceType.ForMatch:
                {
                    ...
                    {
                        if(seqFor.Rule.Filters[i].IsContainedIn(rulesToFilters[patternName]))
                        {
                            if(seqFor.Rule.Filters[i].IsAutoGenerated && seqFor.Rule.Filters[i].Name == "auto")
                                source.AppendFrontFormat("MatchFilters.Filter_{0}_{1}(procEnv, {2});\n", patternName, seqFor.Rule.Filters[i].Name, matchesName);
                            else if(seqFor.Rule.Filters[i].IsAutoGenerated)
                                source.AppendFrontFormat("MatchFilters.Filter_{0}_{1}_{2}(procEnv, {3});\n", patternName, seqFor.Rule.Filters[i].Name, seqFor.Rule.Filters[i].Entity, matchesName);
                            else
                                source.AppendFrontFormat("MatchFilters.Filter_{0}(procEnv, {1});\n", seqFor.Rule.Filters[i], matchesName);
                        }
                        else
                        {
                            if(seqFor.Rule.Filters[i].Argument is double)
                                source.AppendFrontFormat("{0}.FilterFirstLast(new GRGEN_LIBGR.FilterCall(\"{1}\", {2}));\n", matchesName, seqFor.Rule.Filters[i].Name, ((double)seqFor.Rule.Filters[i].Argument).ToString(System.Globalization.CultureInfo.InvariantCulture));
                            else
                                source.AppendFrontFormat("{0}.FilterFirstLast(new GRGEN_LIBGR.FilterCall(\"{1}\", {2}));\n", matchesName, seqFor.Rule.Filters[i].Name, seqFor.Rule.Filters[i].Argument);
                        }
                    }
        ...
        public void EmitSequenceBacktrack(SequenceBacktrack seq, SourceBuilder source)
        {
            ...
            {
                if(seq.Rule.Filters[i].IsContainedIn(rulesToFilters[patternName]))
                {
                    if(seq.Rule.Filters[i].IsAutoGenerated && seq.Rule.Filters[i].Name == "auto")
                        source.AppendFrontFormat("MatchFilters.Filter_{0}_{1}(procEnv, {2});\n", patternName, seq.Rule.Filters[i].Name, matchesName);
                    else if(seq.Rule.Filters[i].IsAutoGenerated)
                        source.AppendFrontFormat("MatchFilters.Filter_{0}_{1}_{2}(procEnv, {3});\n", patternName, seq.Rule.Filters[i].Name, seq.Rule.Filters[i].Entity, matchesName);
                    else
                        source.AppendFrontFormat("MatchFilters.Filter_{0}(procEnv, {1});\n", seq.Rule.Filters[i], matchesName);
                }
                else
                {
                    if(seq.Rule.Filters[i].Argument is double)
                        source.AppendFrontFormat("{0}.FilterFirstLast(new GRGEN_LIBGR.FilterCall(\"{1}\", {2}));\n", matchesName, seq.Rule.Filters[i].Name, ((double)seq.Rule.Filters[i].Argument).ToString(System.Globalization.CultureInfo.InvariantCulture));
                    else
                        source.AppendFrontFormat("{0}.FilterFirstLast(new GRGEN_LIBGR.FilterCall(\"{1}\", {2}));\n", matchesName, seq.Rule.Filters[i].Name, seq.Rule.Filters[i].Argument);
                }
            }
        ...
        void EmitSequenceSome(SequenceSomeFromSet seqSome, SourceBuilder source)
        {
            ...
            for (int i = 0; i < seqSome.Sequences.Count; ++i)
            {
                ...
                {
                    if(seqRule.Filters[j].IsContainedIn(rulesToFilters[patternName]))
                    {
                        if(seqRule.Filters[j].IsAutoGenerated && seqRule.Filters[j].Name == "auto")
                            source.AppendFrontFormat("MatchFilters.Filter_{0}_{1}(procEnv, {2});\n", patternName, seqRule.Filters[j].Name, matchesName);
                        else if(seqRule.Filters[j].IsAutoGenerated)
                            source.AppendFrontFormat("MatchFilters.Filter_{0}_{1}_{2}(procEnv, {3});\n", patternName, seqRule.Filters[j].Name, seqRule.Filters[i].Entity, matchesName);
                        else
                            source.AppendFrontFormat("MatchFilters.Filter_{0}(procEnv, {1});\n", seqRule.Filters[j], matchesName);
                    }
                    else
                    {
                        if(seqRule.Filters[j].Argument is double)
                            source.AppendFrontFormat("{0}.FilterFirstLast(new GRGEN_LIBGR.FilterCall(\"{1}\", {2}));\n", matchesName, seqRule.Filters[j].Name, ((double)seqRule.Filters[j].Argument).ToString(System.Globalization.CultureInfo.InvariantCulture));
                        else
                            source.AppendFrontFormat("{0}.FilterFirstLast(new GRGEN_LIBGR.FilterCall(\"{1}\", {2}));\n", matchesName, seqRule.Filters[j].Name, seqRule.Filters[j].Argument);
                    }
                }
                if (gen.UsePerfInfo) source.AppendFront("if(procEnv.PerformanceInfo!=null) procEnv.PerformanceInfo.MatchesFound += " + matchesName + ".Count;\n");
                source.AppendFront("if(" + matchesName + ".Count!=0) {\n");
                ...
                case SequenceComputationType.Emit:
                {
                    ...
                    if(!(seqEmit.Expression is SequenceExpressionConstant))
                    {
                        ...
                        if(constant.Constant is string)
                        {
                            ...
                            source.AppendFront("procEnv.EmitWriter.Write(GRGEN_LIBGR.EmitHelper.ToString(" + GetSequenceExpression(seqEmit.Expression, source) + ", graph));\n");
                    }
