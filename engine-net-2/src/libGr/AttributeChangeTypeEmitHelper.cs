    ...
    public static partial class EmitHelper
    {
            ...
            AttributeType attrType, IGraph graph)
        {
            if(changeType == AttributeChangeType.PutElement)
            {
                ...
                {
                    ToString(setmap, out type, out content, attrType, graph, null, null, null);
                    content += "|" + ToString(newValue, attrType.ValueType, graph, null, null, null);
                }
                else
                {
                    ToString(setmap, out type, out content, attrType, graph, null, null, null);
                    content += "|" + ToString(keyValue, attrType.KeyType, graph, null, null, null) 
                        + "->" + ToString(newValue, attrType.ValueType, graph, null, null, null);
                }
            }
            ...
            {
                ToString((IDictionary)newValue, out type, out content, attrType, graph, null, null, null);
            }
        }
            ...
            AttributeType attrType, IGraph graph)
        {
            if(changeType == AttributeChangeType.PutElement)
            {
                ...
                ToString(array, out type, out content, attrType, graph, null, null, null);
                content += ".add(" + ToString(newValue, attrType.ValueType, graph, null, null, null);
                if(keyValue != null)
            ...
            else if(changeType == AttributeChangeType.AssignElement)
            {
                ...
                ContainerHelper.GetListType(array, out valueType);
                ToString(array, out type, out content, attrType, graph, null, null, null);
                content += "[" + keyValue.ToString() + "] = " + ToString(newValue, attrType.ValueType, graph, null, null, null);
            }
            else // changeType==AttributeChangeType.Assign
                ToString((IList)newValue, out type, out content, attrType, graph, null, null, null);
        }
            ...
            AttributeType attrType, IGraph graph)
        {
            if(changeType == AttributeChangeType.PutElement)
            {
                ...
                ToString(deque, out type, out content, attrType, graph, null, null, null);
                content += ".add(" + ToString(newValue, attrType.ValueType, graph, null, null, null) + ")";
            }
            else if(changeType == AttributeChangeType.RemoveElement)
            {
                ...
                ToString((IDeque)newValue, out type, out content, attrType, graph, null, null, null);
        }
    }
}
