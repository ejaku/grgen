    ...
    public static partial class EmitHelper
    {
            ...
            ObjectNamerAndIndexer objectNamerAndIndexer, IGraphProcessingEnvironment procEnv)
        {
            ...
            if(setmap != null)
            {
                if(valueType == typeof(SetValueType))
                {
                    ...
                    AppendSet(sb, setmap, attrValueType, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv);
                }
                else
                {
                    type = "map<" + keyType.Name + "," + valueType.Name + ">";
                    AppendMap(sb, setmap, attrKeyType, attrValueType, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv);
                }
            }
            else
                type = "<INVALID>";
            ...
            bool firstLevelObjectEmitted, ObjectNamerAndIndexer objectNamerAndIndexer, IGraphProcessingEnvironment procEnv)
        {
            ...
            foreach(DictionaryEntry entry in set)
            {
                ...
                if(attrValueType != null)
                    sb.Append(ToString(entry.Key, attrValueType, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv));
                else
                    sb.Append(ToStringAutomatic(entry.Key, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv));
            }
        }
            ...
            bool firstLevelObjectEmitted, ObjectNamerAndIndexer objectNamerAndIndexer, IGraphProcessingEnvironment procEnv)
        {
            ...
            foreach(DictionaryEntry entry in setmap)
            {
                ...
                if(attrKeyType != null)
                    sb.Append(ToString(entry.Key, attrKeyType, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv));
                else
                    sb.Append(ToStringAutomatic(entry.Key, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv));
                sb.Append("->");
                if(attrValueType != null)
                    sb.Append(ToString(entry.Value, attrValueType, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv));
                else
                    sb.Append(ToStringAutomatic(entry.Value, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv));
            }
        }
            ...
            ObjectNamerAndIndexer objectNamerAndIndexer, IGraphProcessingEnvironment procEnv)
        {
            ...
            if(array != null)
            {
                ...
                AppendArray(sb, array, attrValueType, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv);
            }
            else
                type = "<INVALID>";
            ...
            bool firstLevelObjectEmitted, ObjectNamerAndIndexer objectNamerAndIndexer, IGraphProcessingEnvironment procEnv)
        {
            ...
            foreach(object entry in array)
            {
                if(first)
                    ...
                    sb.Append(",");
                if(attrValueType != null)
                    sb.Append(ToString(entry, attrValueType, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv));
                else
                    sb.Append(ToStringAutomatic(entry, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv));
            }
        }
            ...
            ObjectNamerAndIndexer objectNamerAndIndexer, IGraphProcessingEnvironment procEnv)
        {
            ...
            if(deque != null)
            {
                ...
                AppendDeque(sb, deque, attrValueType, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv);
            }
            else
                type = "<INVALID>";
            ...
            ObjectNamerAndIndexer objectNamerAndIndexer, IGraphProcessingEnvironment procEnv)
        {
            if(attrType == null)
            {
                ToString(value, out type, out content, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv);
                return;
            }
            ...
            if(type == "object")
                ...
                content = ToString(value, attrType, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv);
        }
            ...
            bool firstLevelObjectEmitted, ObjectNamerAndIndexer objectNamerAndIndexer, IGraphProcessingEnvironment procEnv)
        {
            ...
            {
                type = "<INVALID>";
                content = ToString(value, null, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv);
                return;
            }
            ...
            {
                type = "IMatch";
                content = ToString((IMatch)value, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv);
                return;
            }
            ...
            if(value is IObject)
            {
                ...
                content = ToString((IObject)value, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv);
                return;
            }
            ...
            if(value is ITransientObject)
            {
                ...
                content = ToString((ITransientObject)value, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv);
                return;
            }
        ...
        {
            if(value is IMatch)
                return ToString((IMatch)value, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv);
            if(value is IObject)
                return ToString((IObject)value, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv);
            if(value is ITransientObject)
                return ToString((ITransientObject)value, graph, firstLevelObjectEmitted, objectNamerAndIndexer, procEnv);
