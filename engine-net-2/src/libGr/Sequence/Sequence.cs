...
namespace de.unika.ipd.grGen.libGr
{
    ...
    public class SequenceFilterCall
    {
        public SequenceFilterCall(SequenceExpression[] argExprs)
        {
    ...
    public class SequenceFilterCallInterpreted : SequenceFilterCall
    {
        ...
        {
            Filter = filter;
            IFilterAutoGenerated filterAutoGenerated = filter as IFilterAutoGenerated;
            if(filterAutoGenerated != null)
                FilterCallBase = new FilterCallBase(filterAutoGenerated.NameWithSuffix, argExprs.Length);
            else
        ...
        public override string ToString()
        {
            ...
            sb.Append(Filter.PackagePrefixedName);
            if(Filter is IFilterAutoGenerated)
                sb.Append(((IFilterAutoGenerated)Filter).NameWithSuffix);
            sb.Append("(");
            for(int i = 0; i < ArgumentExpressions.Length; ++i)
            {
    ...
    {
        public SequenceFilterCallCompiled(String filterBase, List<String> entities,
            String packagePrefix, String packageContext, String rulePackage,
            bool isAutoSupplied, bool isAutoGenerated,
            bool unprefixedFilterNameExists, bool filterNameInPackageContextExists,
            SequenceExpression[] argExprs)
            : base(argExprs)
        {
            if(isAutoSupplied)
            {
                FilterCall = new FilterCall(packagePrefix, filterBase, null, null, packageContext);
            ...
            if(isAutoGenerated)
            {
                FilterCall = new FilterCall(packagePrefix, filterBase, null, null, entities.ToArray(), packageContext, true);

                FilterCall.Package = null; // implementation only for auto-generated filters, does not appear in calls
                FilterCall.PackagePrefixedName = FilterCall.Name;
                FilterCall.ImplementationPackageAutoGenerated = rulePackage;
                return;
            }

            FilterCall = new FilterCall(packagePrefix, filterBase, null, null, argExprs.Length, packageContext);
            if(FilterCall.PrePackage != null)
            {
                FilterCall.Package = FilterCall.PrePackage;
                FilterCall.PackagePrefixedName = FilterCall.Package + "::" + FilterCall.Name;
                return;
            }
            if(unprefixedFilterNameExists)
            {
                FilterCall.Package = null;
                FilterCall.PackagePrefixedName = FilterCall.Name;
                return;
            }
            if(filterNameInPackageContextExists)
            {
            ...
            if(rulePackage != null)
            {
                FilterCall.Package = rulePackage;
                FilterCall.PackagePrefixedName = FilterCall.Package + "::" + FilterCall.Name;
                return;
            }
        ...
        public SequenceFilterCallCompiled(String filterBase, List<String> entities,
            String packagePrefix, String packageContext,
            String matchClassPackage, String matchClassName,
            bool isAutoSupplied, bool isAutoGenerated,
            bool unprefixedFilterNameExists, bool filterNameInPackageContextExists,
            SequenceExpression[] argExprs)
            : base(argExprs)
        {
            if(isAutoSupplied)
            {
                FilterCall = new FilterCall(packagePrefix, filterBase, matchClassPackage, matchClassName, packageContext);

                FilterCall.MatchClassPackage = matchClassPackage;
                FilterCall.MatchClassPackagePrefixedName = matchClassPackage != null ? matchClassPackage + "::" + matchClassName : matchClassName;
            ...
            if(isAutoGenerated)
            {
                FilterCall = new FilterCall(packagePrefix, filterBase, matchClassPackage, matchClassName, entities.ToArray(), packageContext, true);
                ...
                FilterCall.Package = null; // implementation only for auto-generated filters, does not appear in calls
                FilterCall.PackagePrefixedName = FilterCall.Name;
                FilterCall.ImplementationPackageAutoGenerated = FilterCall.MatchClassPackage;
                return;
            }

            FilterCall = new FilterCall(packagePrefix, filterBase, matchClassPackage, matchClassName, argExprs.Length, packageContext);
            ...
            if(matchClassPackage != null)
            {
                FilterCall.Package = matchClassPackage;
                FilterCall.PackagePrefixedName = FilterCall.Package + "::" + FilterCall.Name;
                return;
            }
        ...
        public override string ToString()
        {
            ...
            if(FilterCall.MatchClassName != null)
                sb.Append(FilterCall.MatchClassPackagePrefixedName + ".");
            sb.Append(FilterCall.PackagePrefixedName);
            if(FilterCall.Entities != null)
                sb.Append(FilterCall.EntitySuffix);
            sb.Append("(");
            for(int i = 0; i < ArgumentExpressions.Length; ++i)
            {
                sb.Append(ArgumentExpressions[i].Symbol);
        ...
        public readonly FilterCall FilterCall;
    }
    ...
    public class SequenceRuleCallCompiled : SequenceRuleCall
    {
            ...
            : base(argExprs, returnVars, subgraph, special, test, isRuleForMultiRuleAllCallReturningArrays)
        {
            ...
            ResolvePackage(Name, PrePackage, PrePackageContext, unprefixedRuleNameExists,
    ...
    public class SequenceRuleAllCallInterpreted : SequenceRuleAllCall
    {
        /// <summary>
        /// The IAction instance to be used
        /// </summary>
    ...
    public class SequenceRuleAllCallCompiled : SequenceRuleAllCall
    {
        /// <summary>
        /// The name of the rule.
        /// </summary>
    ...
    public class SequenceBacktrack : Sequence
    {
        ...
        public override void Check(SequenceCheckingEnvironment env)
        {
            ...
            if(Rule.RuleInvocation.Subgraph != null)
    ...
    public class SequenceForMatch : SequenceUnary
    {
        ...
        public override void Check(SequenceCheckingEnvironment env)
        {
            ...
            if(!TypesHelper.IsSameOrSubtype(Var.Type, "match<"+Rule.RuleInvocation.Name+">", env.Model))
                throw new SequenceParserException(Symbol, "match<" + Rule.RuleInvocation.Name + ">", Var.Type);
            if(Rule.RuleInvocation.Subgraph != null)
    ...
    public class SequenceSequenceCallCompiled : SequenceSequenceCall
    {
        ...
        public SequenceSequenceCallCompiled(String Name, String PrePackage, String PrePackageContext, bool unprefixedSequenceNameExists,
            List<SequenceExpression> argExprs, List<SequenceVariable> returnVars, SequenceVariable subgraph,
            bool special)
            : base(argExprs, returnVars, subgraph, special)
        {
            ...
            ResolvePackage(Name, PrePackage, PrePackageContext, unprefixedSequenceNameExists,
                        out Package, out PackagePrefixedName);
        }
