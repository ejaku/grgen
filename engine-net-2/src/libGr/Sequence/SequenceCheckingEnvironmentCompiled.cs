...
namespace de.unika.ipd.grGen.libGr
{
    ...
    public class SequenceCheckingEnvironmentCompiled : SequenceCheckingEnvironment
    {
        ...
        protected override bool IsFilterExisting(FilterCall filterCall, SequenceRuleCall seq)
        {
            ...
            if(actionsTypeInformation.RuleContainsFilter(seq.RuleInvocation.PackagePrefixedName, filterCall.Name))
            {
                filterCall.Package = null;
                filterCall.PackagePrefixedName = filterCall.Name;
                return true;
            }
            if(filterCall.PrePackageContext != null && actionsTypeInformation.RuleContainsFilter(ruleName, filterCall.PrePackageContext + "::" + filterCall.Name))
            {
                filterCall.Package = filterCall.PrePackageContext;
                filterCall.PackagePrefixedName = filterCall.Package + "::" + filterCall.Name;
                return true;
            }
            if(seq.RuleInvocation.Package != null && actionsTypeInformation.RuleContainsFilter(ruleName, seq.RuleInvocation.Package + "::" + filterCall.Name))
            {
        ...
        protected override bool IsFilterExisting(FilterCall filterCall, SequenceMultiRuleAllCall seq)
        {
            ...
            if(actionsTypeInformation.MatchClassContainsFilter(matchClassName, filterCall.Name))
            {
                filterCall.Package = null;
                filterCall.PackagePrefixedName = filterCall.Name;
                return true;
            }
            if(filterCall.PrePackageContext != null && actionsTypeInformation.MatchClassContainsFilter(matchClassName, filterCall.PrePackageContext + "::" + filterCall.Name))
            {
                filterCall.Package = filterCall.PrePackageContext;
                filterCall.PackagePrefixedName = filterCall.Package + "::" + filterCall.Name;
                return true;
            }
            if(filterCall.MatchClassPackage != null && actionsTypeInformation.MatchClassContainsFilter(matchClassName, filterCall.MatchClassPackage + "::" + filterCall.Name))
            {
