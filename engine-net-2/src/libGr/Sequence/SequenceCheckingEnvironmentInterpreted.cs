...
namespace de.unika.ipd.grGen.libGr
{
    ...
    public class SequenceCheckingEnvironmentInterpreted : SequenceCheckingEnvironment
    {
        ...
        protected override bool IsFilterExisting(FilterCall filterCall, SequenceRuleCall seq)
        {
            ...
            IRulePattern rulePattern = ((RuleInvocationInterpreted)seq.RuleInvocation).Action.RulePattern;
            ...
            if(filterCall.IsAutoGenerated)
            {
                ...
                filterCall.PackagePrefixedName = filterCall.Name;
                filterCall.ImplementationPackageAutoGenerated = seq.RuleInvocation.Package;
                return rulePattern.ContainsFilter(filterCall.FullName);
            }
            ...
            if(filterCall.PrePackage != null)
            {
                ...
                return rulePattern.ContainsFilter(filterCall.PackagePrefixedName);
            }
            if(rulePattern.ContainsFilter(filterCall.Name))
            {
                filterCall.Package = null;
                filterCall.PackagePrefixedName = filterCall.Name;
                return true;
            }
            if(filterCall.PrePackageContext != null && rulePattern.ContainsFilter(filterCall.PrePackageContext + "::" + filterCall.Name))
            {
                filterCall.Package = filterCall.PrePackageContext;
                filterCall.PackagePrefixedName = filterCall.Package + "::" + filterCall.Name;
                return true;
            }
            if(rulePattern.PatternGraph.Package != null && rulePattern.ContainsFilter(rulePattern.PatternGraph.Package + "::" + filterCall.Name))
            {
                filterCall.Package = rulePattern.PatternGraph.Package;
                filterCall.PackagePrefixedName = filterCall.Package + "::" + filterCall.Name;
                return true;
            }
        ...
        protected override bool IsMatchClassExisting(FilterCall filterCall)
        {
            ...
            else if(actions.GetMatchClass(filterCall.MatchClassName) != null)
            {
                filterCall.MatchClassPackage = null;
                filterCall.MatchClassPackagePrefixedName = filterCall.MatchClassName;
            }
            else if(filterCall.PrePackageContext != null)
            {
                filterCall.MatchClassPackage = filterCall.PrePackageContext;
                filterCall.MatchClassPackagePrefixedName = filterCall.PrePackageContext + "::" + filterCall.MatchClassName;
            }
            else
            {
                filterCall.MatchClassPackage = null;
        ...
        protected override bool IsFilterExisting(FilterCall filterCall, SequenceMultiRuleAllCall seq)
        {
            ...
            MatchClassInfo matchClass = actions.GetMatchClass(filterCall.MatchClassPackagePrefixedName).info;
            ...
            if(filterCall.IsAutoGenerated)
            {
                ...
                return matchClass.ContainsFilter(filterCall.FullName);
            }
            ...
            if(filterCall.PrePackage != null)
            {
                filterCall.Package = filterCall.PrePackage;
                filterCall.PackagePrefixedName = filterCall.Package + "::" + filterCall.Name;
                return matchClass.ContainsFilter(filterCall.PackagePrefixedName);
            }
            if(matchClass.ContainsFilter(filterCall.Name))
            {
                filterCall.Package = null;
                filterCall.PackagePrefixedName = filterCall.Name;
                return true;
            }
            if(filterCall.PrePackageContext != null && matchClass.ContainsFilter(filterCall.PrePackageContext + "::" + filterCall.Name))
            {
                filterCall.Package = filterCall.PrePackageContext;
                filterCall.PackagePrefixedName = filterCall.Package + "::" + filterCall.Name;
                return true;
            }
            if(filterCall.MatchClassPackage != null && matchClass.ContainsFilter(filterCall.MatchClassPackage + "::" + filterCall.Name))
            {
