...
namespace de.unika.ipd.grGen.libGr
{
    ...
    public enum SequenceExpressionType
    {
        ...
        Constant, Variable, This, New, IndexUse,
        ...
        ArrayOrSetMin, ArrayOrSetMax,
        ArraySum, ArrayProd, ArrayAvg, ArrayMed, ArrayMedUnordered, ArrayVar, ArrayDev, ArrayAnd, ArrayOr,
    ...
    public class SequenceExpressionArrayOrSetMin : SequenceExpressionContainer
    {
        public SequenceExpressionArrayOrSetMin(SequenceExpression containerExpr)
            : base(SequenceExpressionType.ArrayOrSetMin, containerExpr)
        {
        }

        protected SequenceExpressionArrayOrSetMin(SequenceExpressionArrayOrSetMin that, Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
           : base(that, originalToCopy, procEnv)
        {
        }
        ...
        internal override SequenceExpression CopyExpression(Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
        {
            return new SequenceExpressionArrayOrSetMin(this, originalToCopy, procEnv);
        }
        ...
        public override void Check(SequenceCheckingEnvironment env)
        {
            ...
            if(containerType.StartsWith("map<") || containerType.StartsWith("deque<"))
                throw new SequenceParserExceptionTypeMismatch(Symbol, "array<T> type or set<T> type", containerType);

            if(containerType.StartsWith("array<") || containerType.StartsWith("set<"))
            {
                if(!IsNumericType(TypesHelper.ExtractSrc(containerType)))
                {
                    throw new SequenceParserExceptionTypeMismatch(Symbol, "array<T> type or set<T> type where T=number", containerType);
                }
            }
        }
        ...
        public override object ExecuteImpl(IGraphProcessingEnvironment procEnv)
        {
            object containerValue = ContainerValue(procEnv);
    ...
    public class SequenceExpressionArrayOrSetMax : SequenceExpressionContainer
    {
        public SequenceExpressionArrayOrSetMax(SequenceExpression containerExpr)
            : base(SequenceExpressionType.ArrayOrSetMax, containerExpr)
        {
        }

        protected SequenceExpressionArrayOrSetMax(SequenceExpressionArrayOrSetMax that, Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
           : base(that, originalToCopy, procEnv)
        {
        }
        ...
        internal override SequenceExpression CopyExpression(Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
        {
            return new SequenceExpressionArrayOrSetMax(this, originalToCopy, procEnv);
        }
    ...
    public class SequenceExpressionIndexUse : SequenceExpression
    {
        public string IndexName;

        public SequenceExpressionIndexUse(string indexName)
            : base(SequenceExpressionType.IndexUse)
        {
            IndexName = indexName;
        }

        protected SequenceExpressionIndexUse(SequenceExpressionIndexUse that, Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
           : base(that)
        {
        ...
        internal override SequenceExpression CopyExpression(Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
        {
            return new SequenceExpressionIndexUse(this, originalToCopy, procEnv);
        }
        ...
        public override void Check(SequenceCheckingEnvironment env)
        {
            ...
            if(GetIndexDescription(env.Model) == null)
                throw new SequenceParserExceptionIndexUnknown(Symbol);
        }
        ...
        public override object ExecuteImpl(IGraphProcessingEnvironment procEnv)
        {
            return procEnv.Graph.Indices.GetIndex(IndexName);
        }

        public IndexDescription GetIndexDescription(IGraphModel model)
        {
            foreach(IndexDescription indexDescription in model.IndexDescriptions)
            {
                if(indexDescription.Name == IndexName)
    ...
    public abstract class SequenceExpressionFromIndex : SequenceExpression
    {
        public readonly SequenceExpression Index;
        public readonly int ArgumentShift;
        public bool EmitProfiling;

        public SequenceExpressionFromIndex(SequenceExpression index, int argumentShift, SequenceExpressionType type)
            : base(type)
        {
            Index = index;
            ArgumentShift = argumentShift;
        }

        protected SequenceExpressionFromIndex(SequenceExpressionFromIndex that, Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
          : base(that)
        {
            Index = that.Index.CopyExpression(originalToCopy, procEnv);
            ArgumentShift = that.ArgumentShift;
            EmitProfiling = that.EmitProfiling;
        }
        ...
        public override void Check(SequenceCheckingEnvironment env)
        {
            ...
            catch(SequenceParserExceptionIndexUnknown)
            {
                ...
                throw new SequenceParserExceptionIndexUnknown(Index.Symbol, FunctionSymbol + String.Format(", {0}. argument", 1 + ArgumentShift));
            }

            IndexDescription indexDescription = GetIndexDescription(env.Model);
            if(!TypesHelper.IsSameOrSubtype(TypesHelper.DotNetTypeToXgrsType(indexDescription.IndexedType), RootType, env.Model))
                throw new SequenceParserExceptionTypeMismatch(FunctionSymbol + String.Format(", {0}. argument - indexed type", 1 + ArgumentShift), RootType, TypesHelper.DotNetTypeToXgrsType(indexDescription.IndexedType));
        }
        ...
        protected IndexDescription GetIndexDescription(IGraphModel model)
        {
    ...
    public abstract class SequenceExpressionFromIndexSame : SequenceExpressionFromIndex
    {
        public readonly SequenceExpression Value;

        public SequenceExpressionFromIndexSame(SequenceExpression index, SequenceExpression value, int argumentShift, SequenceExpressionType type)
            : base(index, argumentShift, type)
        {
            Value = value;
        }

        protected SequenceExpressionFromIndexSame(SequenceExpressionFromIndexSame that, Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
          : base(that, originalToCopy, procEnv)
        {
            Value = that.Value.CopyExpression(originalToCopy, procEnv);
        }
        ...
        public override void Check(SequenceCheckingEnvironment env)
        {
            ...
            IndexDescription indexDescription = GetIndexDescription(env.Model);
            if(Value != null && Value.Type(env) != "") // we can't gain access to an attribute type if the variable is untyped, only runtime-check possible
            {
                if(!TypesHelper.IsSameOrSubtype(Value.Type(env), TypesHelper.AttributeTypeToXgrsType(indexDescription.IndexAccessType), env.Model))
                    throw new SequenceParserExceptionTypeMismatch(FunctionSymbol + String.Format(", {0}. argument", 2 + ArgumentShift), TypesHelper.AttributeTypeToXgrsType(indexDescription.IndexAccessType), Value.Type(env));
            }
        }
    ...
    public abstract class SequenceExpressionFromIndexFromTo : SequenceExpressionFromIndex
    {
        public readonly SequenceExpression From;
        public readonly bool IncludingFrom;
        public readonly SequenceExpression To;
        public readonly bool IncludingTo;

        public SequenceExpressionFromIndexFromTo(SequenceExpression index, SequenceExpression from, bool includingFrom, SequenceExpression to, bool includingTo, int argumentShift, SequenceExpressionType type)
            : base(index, argumentShift, type)
        {
            From = from;
        ...
        protected SequenceExpressionFromIndexFromTo(SequenceExpressionFromIndexFromTo that, Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
          : base(that, originalToCopy, procEnv)
        {
            if(that.From != null)
                From = that.From.CopyExpression(originalToCopy, procEnv);
            IncludingFrom = that.IncludingFrom;
            if(that.To != null)
                To = that.To.CopyExpression(originalToCopy, procEnv);
            IncludingTo = that.IncludingTo;
        }
        ...
        public override void Check(SequenceCheckingEnvironment env)
        {
            ...
            IndexDescription indexDescription = GetIndexDescription(env.Model);
            if(From != null && From.Type(env) != "") // we can't gain access to an attribute type if the variable is untyped, only runtime-check possible
            {
                if(!TypesHelper.IsSameOrSubtype(From.Type(env), TypesHelper.AttributeTypeToXgrsType(indexDescription.IndexAccessType), env.Model))
                    throw new SequenceParserExceptionTypeMismatch(FunctionSymbol + String.Format(", {0}. argument", 2 + ArgumentShift), TypesHelper.AttributeTypeToXgrsType(indexDescription.IndexAccessType), From.Type(env));
            }
            if(To != null && To.Type(env) != "") // we can't gain access to an attribute type if the variable is untyped, only runtime-check possible
            {
                if(!TypesHelper.IsSameOrSubtype(To.Type(env), TypesHelper.AttributeTypeToXgrsType(indexDescription.IndexAccessType), env.Model))
                    throw new SequenceParserExceptionTypeMismatch(FunctionSymbol + String.Format(", {0}. argument", 3 + ArgumentShift), TypesHelper.AttributeTypeToXgrsType(indexDescription.IndexAccessType), To.Type(env));
            }
        }
            ...
            List<SequenceExpressionConstructor> constructors)
        {
            Index.GetLocalVariables(variables, constructors);
            if(From != null)
                From.GetLocalVariables(variables, constructors);
            if(To != null)
                To.GetLocalVariables(variables, constructors);
        }
        ...
        public override string Symbol
        {
            get { return FunctionSymbol + "(" + Index.Symbol + (From != null ? "," + From.Symbol : "") + (To != null ? "," + To.Symbol : "") + ")"; }
        }
    }
    ...
    public abstract class SequenceExpressionIsInIndexFromTo : SequenceExpressionFromIndexFromTo
    {
        ...
        public SequenceExpressionIsInIndexFromTo(SequenceExpression candidate, SequenceExpression index, SequenceExpression from, bool includingFrom, SequenceExpression to, bool includingTo, SequenceExpressionType type)
            : base(index, from, includingFrom, to, includingTo, 1, type)
        {
            Candidate = candidate;
        }

        protected SequenceExpressionIsInIndexFromTo(SequenceExpressionIsInIndexFromTo that, Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
          : base(that, originalToCopy, procEnv)
        {
            Candidate = that.Candidate.CopyExpression(originalToCopy, procEnv);
        }
        ...
        public override string Symbol
        {
            get { return FunctionSymbol + "(" + Candidate.Symbol + "," + Index.Symbol + (From != null ? "," + From.Symbol : "") + (To != null ? "," + To.Symbol : "") + ")"; }
        }
    }
    ...
    public class SequenceExpressionNodesFromIndexSame : SequenceExpressionFromIndexSame
    {
        ...
        protected SequenceExpressionNodesFromIndexSame(SequenceExpressionNodesFromIndexSame that, Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
          : base(that, originalToCopy, procEnv)
        {
        }
        ...
        public override object ExecuteImpl(IGraphProcessingEnvironment procEnv)
        {
            ...
            if(EmitProfiling)
                return IndexHelper.NodesFromIndexSame(index, value, procEnv);
            else
                return IndexHelper.NodesFromIndexSame(index, value);
        }
    ...
    public class SequenceExpressionEdgesFromIndexSame : SequenceExpressionFromIndexSame
    {
        public SequenceExpressionEdgesFromIndexSame(SequenceExpression index, SequenceExpression value, SequenceExpressionType type)
            : base(index, value, 0, type)
        {
        }

        protected SequenceExpressionEdgesFromIndexSame(SequenceExpressionEdgesFromIndexSame that, Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
          : base(that, originalToCopy, procEnv)
        {
        }
        ...
        public override object ExecuteImpl(IGraphProcessingEnvironment procEnv)
        {
            ...
            if(EmitProfiling)
                return IndexHelper.EdgesFromIndexSame(index, value, procEnv);
            else
                return IndexHelper.EdgesFromIndexSame(index, value);
        }
    ...
    public class SequenceExpressionNodesFromIndexFromTo : SequenceExpressionFromIndexFromTo
    {
        public SequenceExpressionNodesFromIndexFromTo(SequenceExpression index, SequenceExpression from, bool includingFrom, SequenceExpression to, bool includingTo, SequenceExpressionType type)
            : base(index, from, includingFrom, to, includingTo, 0, type)
        {
        }

        protected SequenceExpressionNodesFromIndexFromTo(SequenceExpressionNodesFromIndexFromTo that, Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
          : base(that, originalToCopy, procEnv)
        {
        }
        ...
        public override object ExecuteImpl(IGraphProcessingEnvironment procEnv)
        {
            ...
            if(EmitProfiling)
                return IndexHelper.NodesFromIndexFromTo(index, from, IncludingFrom, to, IncludingTo, procEnv);
            else
                return IndexHelper.NodesFromIndexFromTo(index, from, IncludingFrom, to, IncludingTo);
        }
    ...
    public class SequenceExpressionEdgesFromIndexFromTo : SequenceExpressionFromIndexFromTo
    {
        public SequenceExpressionEdgesFromIndexFromTo(SequenceExpression index, SequenceExpression from, bool includingFrom, SequenceExpression to, bool includingTo, SequenceExpressionType type)
            : base(index, from, includingFrom, to, includingTo, 0, type)
        {
        }

        protected SequenceExpressionEdgesFromIndexFromTo(SequenceExpressionEdgesFromIndexFromTo that, Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
          : base(that, originalToCopy, procEnv)
        {
        }
        ...
        public override object ExecuteImpl(IGraphProcessingEnvironment procEnv)
        {
            ...
            if(EmitProfiling)
                return IndexHelper.EdgesFromIndexFromTo(index, from, IncludingFrom, to, IncludingTo, procEnv);
            else
                return IndexHelper.EdgesFromIndexFromTo(index, from, IncludingFrom, to, IncludingTo);
        }
    ...
    public class SequenceExpressionIsInNodesFromIndexSame : SequenceExpressionIsInIndexSame
    {
        public SequenceExpressionIsInNodesFromIndexSame(SequenceExpression candidate, SequenceExpression index, SequenceExpression value, SequenceExpressionType type)
            : base(candidate, index, value, type)
        {
        }

        protected SequenceExpressionIsInNodesFromIndexSame(SequenceExpressionIsInNodesFromIndexSame that, Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
          : base(that, originalToCopy, procEnv)
        {
        }
        ...
        public override object ExecuteImpl(IGraphProcessingEnvironment procEnv)
        {
            INode candidate = (INode)Candidate.Evaluate(procEnv);
            IAttributeIndex index = (IAttributeIndex)Index.Evaluate(procEnv);
            object value = Value.Evaluate(procEnv);

            if(EmitProfiling)
                return IndexHelper.IsInNodesFromIndexSame(candidate, index, value, procEnv);
            else
                return IndexHelper.IsInNodesFromIndexSame(candidate, index, value);
        }
    ...
    public class SequenceExpressionIsInEdgesFromIndexSame : SequenceExpressionIsInIndexSame
    {
        public SequenceExpressionIsInEdgesFromIndexSame(SequenceExpression candidate, SequenceExpression index, SequenceExpression value, SequenceExpressionType type)
            : base(candidate, index, value, type)
        {
        }

        protected SequenceExpressionIsInEdgesFromIndexSame(SequenceExpressionIsInEdgesFromIndexSame that, Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
          : base(that, originalToCopy, procEnv)
        {
        }
        ...
        public override object ExecuteImpl(IGraphProcessingEnvironment procEnv)
        {
            IEdge candidate = (IEdge)Candidate.Evaluate(procEnv);
            IAttributeIndex index = (IAttributeIndex)Index.Evaluate(procEnv);
            object value = Value.Evaluate(procEnv);

            if(EmitProfiling)
                return IndexHelper.IsInEdgesFromIndexSame(candidate, index, value, procEnv);
            else
                return IndexHelper.IsInEdgesFromIndexSame(candidate, index, value);
        }
    ...
    public class SequenceExpressionIsInNodesFromIndexFromTo : SequenceExpressionIsInIndexFromTo
    {
        public SequenceExpressionIsInNodesFromIndexFromTo(SequenceExpression candidate, SequenceExpression index, SequenceExpression from, bool includingFrom, SequenceExpression to, bool includingTo, SequenceExpressionType type)
            : base(candidate, index, from, includingFrom, to, includingTo, type)
        {
        }

        protected SequenceExpressionIsInNodesFromIndexFromTo(SequenceExpressionIsInNodesFromIndexFromTo that, Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
          : base(that, originalToCopy, procEnv)
        {
        }
        ...
        public override object ExecuteImpl(IGraphProcessingEnvironment procEnv)
        {
            ...
            if(EmitProfiling)
                return IndexHelper.IsInNodesFromIndexFromTo(candidate, index, from, IncludingFrom, to, IncludingTo, procEnv);
            else
                return IndexHelper.IsInNodesFromIndexFromTo(candidate, index, from, IncludingFrom, to, IncludingTo);
        }
        ...
        public override string FunctionSymbol
        {
            ...
            {
                if(From == null && To == null)
                    return "isInNodesFromIndex";
                else if(To == null)
                    return IncludingFrom ? "isInNodesFromIndexFrom" : "isInNodesFromIndexFromExclusive";
                else if(From == null)
                    return IncludingTo ? "isInNodesFromIndexTo" : "isInNodesFromIndexToExclusive";
                else
                {
                    if(IncludingFrom && IncludingTo)
                        return "isInNodesFromIndexFromTo";
                    else if(IncludingFrom)
                        return "isInNodesFromIndexFromToExclusive";
                    else if(IncludingTo)
                        return "isInNodesFromIndexFromExclusiveTo";
                    else
                        return "isInNodesFromIndexFromExclusiveToExclusive";
                }
            }
        }
    }

    public class SequenceExpressionIsInEdgesFromIndexFromTo : SequenceExpressionIsInIndexFromTo
    {
        public SequenceExpressionIsInEdgesFromIndexFromTo(SequenceExpression candidate, SequenceExpression index, SequenceExpression from, bool includingFrom, SequenceExpression to, bool includingTo, SequenceExpressionType type)
            : base(candidate, index, from, includingFrom, to, includingTo, type)
        {
        }

        protected SequenceExpressionIsInEdgesFromIndexFromTo(SequenceExpressionIsInEdgesFromIndexFromTo that, Dictionary<SequenceVariable, SequenceVariable> originalToCopy, IGraphProcessingEnvironment procEnv)
          : base(that, originalToCopy, procEnv)
        {
        }
        ...
        public override object ExecuteImpl(IGraphProcessingEnvironment procEnv)
        {
            ...
            if(EmitProfiling)
                return IndexHelper.IsInEdgesFromIndexFromTo(candidate, index, from, IncludingFrom, to, IncludingTo, procEnv);
            else
                return IndexHelper.IsInEdgesFromIndexFromTo(candidate, index, from, IncludingFrom, to, IncludingTo);
        }
        ...
        public override string FunctionSymbol
        {
            ...
            {
                if(From == null && To == null)
                    return "isInEdgesFromIndex";
                else if(To == null)
                    return IncludingFrom ? "isInEdgesFromIndexFrom" : "isInEdgesFromIndexFromExclusive";
                else if(From == null)
                    return IncludingTo ? "isInEdgesFromIndexTo" : "isInEdgesFromIndexToExclusive";
                else
                {
                    if(IncludingFrom && IncludingTo)
                        return "isInEdgesFromIndexFromTo";
                    else if(IncludingFrom)
                        return "isInEdgesFromIndexFromToExclusive";
                    else if(IncludingTo)
    ...
    public class SequenceExpressionFunctionMethodCall : SequenceExpressionFunctionCall, MethodFunctionInvocation
    {
        ...
        public override object ExecuteImpl(IGraphProcessingEnvironment procEnv)
        {
            ...
            if(owner == null)
                throw new Exception("Function method call " + Symbol + " expects a node or edge or class object as target (but is given " + ownerCandidate.GetType() + ") (maybe a procedure call is intended?)!");
            FillArgumentsFromArgumentExpressions(ArgumentExpressions, Arguments, procEnv);
