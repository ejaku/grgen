    ...
    public class SequenceParserEnvironmentCompiled : SequenceParserEnvironment
    {
            ...
            String packagePrefix, String filterBase, List<String> entities, List<SequenceExpression> argExprs)
        {
            ...
            SequenceFilterCall seqFilterCall = new SequenceFilterCallCompiled(filterBase, entities,
                packagePrefix, packageContext, rulePackage,
                IsAutoSuppliedFilterName(filterBase), IsAutoGeneratedFilter(filterBase, entities),
                actionNames.RuleContainsFilter(packagePrefixedRuleName, filterName),
                packageContext != null && actionNames.RuleContainsFilter(packagePrefixedRuleName, packageContext + "::" + filterName),
                argExprs.ToArray());
            ...
            String packagePrefix, String filterBase, List<String> entities, List<SequenceExpression> argExprs)
        {
            ...
            SequenceBase.ResolvePackage(matchClassName, matchClassPackage, packageContext, unprefixedMatchClassNameExists,
                out resolvedMatchClassPackage, out packagePrefixedMatchClassName);

            String filterName = GetFilterName(filterBase, entities);

            SequenceFilterCall seqFilterCall = new SequenceFilterCallCompiled(filterBase, entities,
                packagePrefix, packageContext,
                resolvedMatchClassPackage, matchClassName,
                IsAutoSuppliedFilterName(filterBase), IsAutoGeneratedFilter(filterBase, entities),
                actionNames.MatchClassContainsFilter(packagePrefixedMatchClassName, filterName),
                packageContext != null && actionNames.MatchClassContainsFilter(packagePrefixedMatchClassName, packageContext + "::" + filterName),
                argExprs.ToArray());
