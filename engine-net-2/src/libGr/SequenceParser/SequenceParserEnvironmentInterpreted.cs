...
namespace de.unika.ipd.grGen.libGr.sequenceParser
{
    ...
    public class SequenceParserEnvironmentInterpreted : SequenceParserEnvironment
    {
            ...
            String packagePrefix, String filterBase, List<String> entities, List<SequenceExpression> argExprs)
        {
            ...
            {
                filter = action.RulePattern.GetFilter(filterBase);
                if(filter == null)
                    throw new SequenceParserException(action.PackagePrefixedName, filterBase, SequenceParserError.FilterError);
            ...
            String packagePrefix, String filterBase, List<String> entities, List<SequenceExpression> argExprs)
        {
            ...
            if(IsAutoSuppliedFilterName(filterBase))
            {
                filter = matchClass.info.GetFilter(filterBase);
                if(filter == null)
                    throw new SequenceParserException(matchClass.info.PackagePrefixedName, filterBase, SequenceParserError.FilterError);
            ...
            if(IsAutoGeneratedFilter(filterBase, entities))
            {
                ...
                if(filter == null)
                    throw new SequenceParserException(matchClass.info.PackagePrefixedName, filterName, SequenceParserError.FilterError);
