...
namespace de.unika.ipd.grGen.libGr.sequenceParser
{
    ...
    public class SequenceParserEnvironmentInterpreted : SequenceParserEnvironment
    {
        ...
        {
            IAction action; // no resolving of rules in interpreted sequences cause there is not package context existing
            if(rulePackage != null)
                action = actions.GetAction(rulePackage + "::" + ruleName);
            else
                action = actions.GetAction(ruleName);

            if(IsAutoSuppliedFilterName(filterBase))
            {
                // todo: input names and input types missing - have to come from the rule, description and calling interface for auto-supplied filters must be same as for auto-generated filters
                FilterAutoSupplied filterAutoSupplied = new FilterAutoSupplied(filterBase, null, filterBase, null, null);
                return new SequenceFilterCallInterpreted(/*action, */filterAutoSupplied, argExprs.ToArray());
            }

            IFilter filter = null;
            String filterName = GetFilterName(filterBase, entities);
            if(IsAutoGeneratedFilter(filterBase, entities))
            {
                filter = action.RulePattern.GetFilter(filterName);
                if(filter == null)
                    throw new SequenceParserException(action.PackagePrefixedName, filterName, SequenceParserError.FilterError);
            ...
            if(packagePrefix != null)
            {
                filter = action.RulePattern.GetFilter(packagePrefix + "::" + filterName);
                if(filter == null)
                    throw new SequenceParserException(action.PackagePrefixedName, packagePrefix + "::" + filterName, SequenceParserError.FilterError);
            }
            else
            {
                filter = action.RulePattern.GetFilter(filterName);
                if(filter == null)
                {
                    filter = action.RulePattern.GetFilter(rulePackage + "::" + filterName);
                    if(filter == null)
                        throw new SequenceParserException(action.PackagePrefixedName, filterName, SequenceParserError.FilterError);
                }
            }
        ...
        public override SequenceFilterCall CreateSequenceMatchClassFilterCall(String matchClassName, String matchClassPackage,
            String packagePrefix, String filterBase, List<String> entities, List<SequenceExpression> argExprs)
        {
            MatchClassFilterer matchClass = null; // no resolving of match classes in interpreted sequences cause there is not package context existing
            if(matchClassPackage != null)
                matchClass = actions.GetMatchClass(matchClassPackage + "::" + matchClassName);
            else
                matchClass = actions.GetMatchClass(matchClassName);

            if(IsAutoSuppliedFilterName(filterBase))
            {
                // todo: input names and input types missing - have to come from the match class, description and calling interface for auto-supplied filters must be same as for auto-generated filters
                FilterAutoSupplied filterAutoSupplied = new FilterAutoSupplied(filterBase, null, filterBase, null, null);
                return new SequenceFilterCallInterpreted(matchClass, filterAutoSupplied, argExprs.ToArray());
            }
            ...
            if(IsAutoGeneratedFilter(filterBase, entities))
            {
                filter = matchClass.info.GetFilter(filterName);
                return new SequenceFilterCallInterpreted(matchClass, filter, argExprs.ToArray());
            }

            if(packagePrefix != null)
            {
                filter = matchClass.info.GetFilter(packagePrefix + "::" + filterName);
                if(filter == null)
                    throw new Exception("Unknown filter: " + packagePrefix + "::" + filterName);
            }
            else
            {
                filter = matchClass.info.GetFilter(filterName);
                if(filter == null)
                {
                    filter = matchClass.info.GetFilter(matchClassPackage + "::" + filterName);
                    if(filter == null)
