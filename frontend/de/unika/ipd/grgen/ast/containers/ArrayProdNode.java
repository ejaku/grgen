...
import de.unika.ipd.grgen.ir.containers.ArrayProdExpr;
...
public class ArrayProdNode extends ArrayAccumulationMethodNode
{
	...
	protected boolean checkLocal() {
		...
		if(!(targetType instanceof ArrayTypeNode)) {
			targetExpr.reportError("This argument to array prod method must be of type array<T>");
			return false;
		}
		ArrayTypeNode arrayType = (ArrayTypeNode)targetExpr.getType();
		if(!arrayType.valueType.isAccumulatableType()) {
			targetExpr.reportError("The array value type of the array prod method must be one of: " + TypeNode.getAccumulatableTypesAsString());
			return false;
		}
		return true;
	}
