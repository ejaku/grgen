...
public abstract class MatcherDeclNode extends DeclNode
{
		...
		for(FilterAutoDeclNode fa : filters) {
			if(fa instanceof FilterAutoGeneratedDeclNode) {
				...
				if(alreadySeenFilters.contains(filterNameWithEntitySuffix)) {
					reportError(filterNameWithEntitySuffix + " was already declared for " + pattern.nameOfGraph + ", only one declaration admissible.");
		...
		case "keepSameAsLast":
		{
			...
			if(filter.entities.size() != 1) {
				reportError(filterNameWithEntitySuffix
						...
						+ " variables (at " + pattern.nameOfGraph + ").");
				allFilterEntitiesExistAndAreOfAdmissibleType = false;
			}
			return allFilterEntitiesExistAndAreOfAdmissibleType;
		}
			...
			if(filter.entities.size() != 3) {
				...
				if(filter.entities.get(0).equals(filter.entities.get(1))) {
					getIdentNode().reportError("The accumulation variable " + filter.entities.get(1)
							+ " must be different from the variable (in " + filterNameWithEntitySuffix + ")"
							+ " (at " + pattern.nameOfGraph + ").");
					return false;
				}
				...
				if(accumulationMethod == null) {
					getIdentNode().reportError("The array accumulation method "
							+ filter.entities.get(2) + " is not known (in " + filterNameWithEntitySuffix + ")"
							+ " (at " + pattern.nameOfGraph + ").");
					return false;
				}
			...
			ConnectionNode iConnection, ConnectionNode jConnection)
	{
		...
		if(!( (iSrc instanceof NodeDeclNode) && ((NodeDeclNode)iSrc).isDummy() )
			...
			iConnection.reportError("Reused edge " + iConnection.getEdge() + " does not connect the same nodes"
					+ " (" + iSrc + " differs from " + jSrc + ").");
			edgeReuse = false;
		}
		if(!( (iTgt instanceof NodeDeclNode) && ((NodeDeclNode)iTgt).isDummy() )
			...
			alreadyReported.add(iConnection.getEdge());
			iConnection.reportError("Reused edge " + iConnection.getEdge() + " does not connect the same nodes"
					+ " (" + iTgt + " differs from " + jTgt + ").");
			edgeReuse = false;
		}

		if(iConnection.getConnectionKind() != jConnection.getConnectionKind()) {
			alreadyReported.add(iConnection.getEdge());
			iConnection.reportError("Reused edge " + iConnection.getEdge() + " does not have the same connection kind"
					+ " (" + ConnectionNode.toString(iConnection.getConnectionKind()) + " differs from " + ConnectionNode.toString(jConnection.getConnectionKind())+ ").");
			edgeReuse = false;
		}
			...
			for(ConnectionCharacter leftConnectionCharacter : pattern.getConnections()) {
				...
				if(leftConnection.getConnectionKind() != rightConnection.getConnectionKind()) {
					...
					rightConnection.reportError("Reused edge " + rightEdge + " does not have the same connection kind"
							+ " (in pattern: " + ConnectionNode.toString(leftConnection.getConnectionKind()) + ", but in rewrite part: " + ConnectionNode.toString(rightConnection.getConnectionKind())+ ").");
			...
			NodeDeclNode rSrc, HashSet<BaseNode> rhsNodes)
	{
		...
		if(!lSrc.isDummy()) {
			if(rSrc.isDummy()) {
				if(rhsNodes.contains(lSrc)) {
					...
					res = false;
					rightConnection.reportError("The source node (" + lSrc + ") of reused/referenced edge " + le + " must be reused/referenced, too.");
				...
				res = false;
				rightConnection.reportError("Reused/referenced edge " + le
		...
		if((rightConnection.getRedirectionKind() & ConnectionNode.REDIRECT_SOURCE) == ConnectionNode.REDIRECT_SOURCE) {
			if(rSrc.isDummy()) {
				res = false;
				rightConnection.reportError("A source redirection is specified for edge " + le + ", but no source node is given.");
			}

			if(lSrc.equals(rSrc)) {
				rightConnection.reportWarning("Redirecting edge " + le + " to same source again (" + rSrc + ").");
			}

			if(redirectedFrom.containsKey(le)) {
				res = false;
				NodeDeclNode rSrc2 = redirectedFrom.get(le);
				rightConnection.reportError("The source of edge " + le + " is redirected more than once"
						+ " (" + rSrc + " and " + rSrc2 + ").");
			}
		...
		if(!alreadyReported.contains(re)) {
			if(lSrc.isDummy() && !rSrc.isDummy()
					&& (rightConnection.getRedirectionKind() & ConnectionNode.REDIRECT_SOURCE) != ConnectionNode.REDIRECT_SOURCE) {
				res = false;
				rightConnection.reportError("Reused edge " + le + " dangles on LHS, but has a source node on RHS (" + rSrc + ").");
				alreadyReported.add(re);
			}
		}
			...
			NodeDeclNode rTgt, HashSet<BaseNode> rhsNodes)
	{
		...
		if(!lTgt.isDummy()) {
			if(rTgt.isDummy()) {
				if(rhsNodes.contains(lTgt)) {
					...
					res = false;
					rightConnection.reportError("The target node (" + lTgt + ") of reused/referenced edge " + le + " must be reused/referenced, too.");
					alreadyReported.add(re);
				}
			} else if(lTgt != rTgt
		...
		if((rightConnection.getRedirectionKind() & ConnectionNode.REDIRECT_TARGET) == ConnectionNode.REDIRECT_TARGET) {
			if(rTgt.isDummy()) {
				res = false;
				rightConnection.reportError("A target redirection is specified for edge " + le + ", but no target node is given.");
			}

			if(lTgt.equals(rTgt)) {
				rightConnection.reportWarning("Redirecting edge " + le + " to same target again (" + rTgt + ").");
			}

			if(redirectedTo.containsKey(le)) {
				res = false;
				NodeDeclNode rTgt2 = redirectedTo.get(le);
				rightConnection.reportError("The target of edge " + le + " is redirected more than once"
						+ " (" + rTgt + " and " + rTgt2 + ").");
			}
		...
		if(!alreadyReported.contains(re)) {
			if(lTgt.isDummy() && !rTgt.isDummy()
					&& (rightConnection.getRedirectionKind() & ConnectionNode.REDIRECT_TARGET) != ConnectionNode.REDIRECT_TARGET) {
				res = false;
				rightConnection.reportError("Reused edge " + le + " dangles on LHS, but has a target node on RHS (" + rTgt + ").");
				alreadyReported.add(re);
			}
		}
