...
public class MatchNodeByIndexAccessOrderingDeclNode : MatchNodeByIndexDeclNode
{
	static MatchNodeByIndexAccessOrderingDeclNode()
	{
		SetClassName(typeof(MatchNodeByIndexAccessOrderingDeclNode), "match node by index access ordering decl");
	}
		...
		if(comp == Operator.LT || comp == Operator.LE)
		{
			if(expr2 != null && (comp2 == Operator.LT || comp2 == Operator.LE))
			{
				ReportError("Two upper bounds are not supported"
						+ " (in match node" + EmptyWhenAnonymousPostfix(" ") + " by index access of " + index.Ident + ").");
				return false;
			}
		}
