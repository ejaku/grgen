...
public class ArrayMapNode : ArrayFunctionMethodInvocationBaseExprNode
{
	static ArrayMapNode()
	{
		SetClassName(typeof(ArrayMapNode), "array map");
	}
	...
	protected internal override bool CheckLocal()
	{
		...
		if(!exprType.IsEqual(resultType))
		{
			...
			{
				ReportError("Type mismatch in the array map function method between the lambda expression value of " + exprType.Kind + " " + exprType.TypeName
				...
				|| targetType is EdgeTypeNode && elementVarType is NodeTypeNode)
		{
			ReportError("Cannot bind the element variable of " + elementVarType.Kind + " " + elementVarType.TypeName
					+ " to a value of " + targetType.Kind + " " + targetType.TypeName + " in the array map function method.");
			return false;
		}
