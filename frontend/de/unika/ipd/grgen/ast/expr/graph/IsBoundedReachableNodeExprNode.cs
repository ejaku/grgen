...
namespace de.unika.ipd.grgen.ast.expr.graph
{
	...
	/// </summary>
	public class IsBoundedReachableNodeExprNode : NeighborhoodQueryExprNode
	{
		static IsBoundedReachableNodeExprNode()
		{
			SetClassName(typeof(IsBoundedReachableNodeExprNode), "is bounded reachable node expr");
		}
		...
		protected internal override bool CheckLocal()
		{
			...
			if(!(endNodeExpr.Type is NodeTypeNode))
			{
				ReportError("The function " + ShortSignature() + " expects as 2. argument a value of type node"
						 + " (but is given a value of type " + endNodeExpr.Type.TypeName + ").");
				return false;
			}
		...
		protected internal override IR ConstructIR()
		{
			...
			return new IsBoundedReachableNodeExpr(startNodeExpr.CheckIR<Expression>(typeof(Expression)),
					endNodeExpr.CheckIR<Expression>(typeof(Expression)), depthExpr.CheckIR<Expression>(typeof(Expression)),
