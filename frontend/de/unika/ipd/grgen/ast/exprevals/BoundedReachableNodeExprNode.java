...
import de.unika.ipd.grgen.ir.exprevals.BoundedReachableNodeExpr;
...
public class BoundedReachableNodeExprNode extends ExprNode {
	...
	protected boolean checkLocal() {
		if(!(startNodeExpr.getType() instanceof NodeTypeNode)) {
			reportError("first argument of boundedReachableNodes(.,.,.,.) must be a node");
			return false;
		}
		if(!(depthExpr.getType() instanceof IntTypeNode)) {
			reportError("second argument of boundedReachableNodes(.,.,.,.) must be an int");
			return false;
		}
		if(!(incidentTypeExpr.getType() instanceof EdgeTypeNode)) {
			reportError("third argument of boundedReachableNodes(.,.,.,.) must be an edge type");
			return false;
		}
		if(!(adjacentTypeExpr.getType() instanceof NodeTypeNode)) {
			reportError("fourth argument of boundedReachableNodes(.,.,.,.) must be a node type");
			return false;
		}
		return true;
	}
