 ...
 */
public class ComputationSignature extends Base {
	...
	/**
	 * Make a new computation signature.
	 * @param resTypes The result types.
	 * @param opTypes The operand types.
	 */
	public ComputationSignature(TypeNode[] resTypes, TypeNode[] opTypes) {
		this.resTypes = resTypes;
		this.opTypes = opTypes;
	}

	/**
	 * Get the result types of this computation signature.
	...
	/**
	 * Get the operand types of this computation signature.
	 ...
	 * computation signature with the given operands.
	 * @param ops The operands
	 * @return The number of implicit type casts needed to apply the operands
	 * to this computation signature. <code>Integer.MAX_VALUE</code> is returned,
	 * if the operands cannot be applied to this computation signature.
	 */
	protected int getDistance(TypeNode[] ops) {
		...
		if(ops.length == opTypes.length) {
			...
			for(int i = 0; i < opTypes.length; i++) {
				 ...
				 * distance". Note that the below computation only test indirect
