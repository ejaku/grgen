...
import de.unika.ipd.grgen.ir.exprevals.CountReachableNodeExpr;
...
public class CountReachableNodeExprNode extends ExprNode {
	...
	protected boolean checkLocal() {
		if(!(startNodeExpr.getType() instanceof NodeTypeNode)) {
			reportError("first argument of countReachableNodes(.,.,.) must be a node");
			return false;
		}
		if(!(incidentTypeExpr.getType() instanceof EdgeTypeNode)) {
			reportError("second argument of countReachableNodes(.,.,.) must be an edge type");
			return false;
		}
		if(!(adjacentTypeExpr.getType() instanceof NodeTypeNode)) {
			reportError("third argument of countReachableNodes(.,.,.) must be a node type");
			return false;
		}
		return true;
	}
	...
	protected IR constructIR() {
		...
		return new CountReachableNodeExpr(startNodeExpr.checkIR(Expression.class), 
