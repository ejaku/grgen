...
/**
 * AST node class representing external computation declarations
 */
public class ExternalComputationDeclNode extends ComputationBase {
	static {
		setName(ExternalComputationDeclNode.class, "external computation declaration");
	}

	protected CollectNode<BaseNode> paramTypesUnresolved;
	protected CollectNode<TypeNode> paramTypes;

	private static final ExternalComputationTypeNode externalComputationType = 
		new ExternalComputationTypeNode();

	public ExternalComputationDeclNode(IdentNode id, CollectNode<BaseNode> paramTypesUnresolved, CollectNode<BaseNode> rets) {
		super(id, externalComputationType);
	...
	public TypeNode getDeclType() {
		...
		return externalComputationType;
	}
	...
	protected IR constructIR() {
		ExternalComputation externalComp = new ExternalComputation(getIdentNode().toString(),
				getIdentNode().getIdent());
		for(TypeNode retType : returnTypes.getChildren()) {
			externalComp.addReturnType(retType.checkIR(Type.class));
		}
		for(TypeNode param : paramTypes.getChildren()) {
			externalComp.addParameterType(param.checkIR(Type.class));
		}
		return externalComp;
	}
	
	public static String getKindStr() {
		return "external computation declaration";
	}
