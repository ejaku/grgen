...
import de.unika.ipd.grgen.ir.exprevals.GraphRetypeEdgeComp;
import de.unika.ipd.grgen.ir.exprevals.GraphRetypeNodeComp;
import de.unika.ipd.grgen.parser.Coords;

/**
 * A node for retyping a node or an edge to a new type.
 */
public class GraphRetypeCompNode extends ComputationInvocationBaseNode {
	static {
		setName(GraphRetypeCompNode.class, "retype computation");
	}
	...
	public GraphRetypeCompNode(Coords coords, ExprNode entity,
			ExprNode entityType) {
		super(coords);
		this.entity = entity;
	...
	protected IR constructIR() {
		if(entityType.getType() instanceof NodeTypeNode) {
			GraphRetypeNodeComp retypeNode = new GraphRetypeNodeComp(
			...
			GraphRetypeEdgeComp retypeEdge = new GraphRetypeEdgeComp(
													entity.checkIR(Expression.class), 
													entityType.checkIR(Expression.class));			
			for(TypeNode type : getType()) {
