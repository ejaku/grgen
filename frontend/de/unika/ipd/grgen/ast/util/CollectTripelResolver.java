 ...
 */
public class CollectTripelResolver<R extends BaseNode, S extends BaseNode, T extends BaseNode>
{
	private Resolver<? extends Tripel<R, S, T>> resolver;

	public CollectTripelResolver(Resolver<? extends Tripel<R, S, T>> resolver) {
	 ...
	 * the given resolver
	 */
	public Tripel<CollectNode<R>, CollectNode<S>, CollectNode<T>> resolve(CollectNode<?> collect) {
		CollectNode<R> first = null;
		CollectNode<S> second = null;
		CollectNode<T> third = null;

		for (BaseNode elem : collect.getChildren()) {
	        Tripel<R, S, T> tripel = resolver.resolve(elem, collect);
	        if (tripel == null) {
	        	return null;
	        }
		...
		Tripel<CollectNode<R>, CollectNode<S>, CollectNode<T>> res = new Tripel<CollectNode<R>, CollectNode<S>, CollectNode<T>>();
		res.first = first;
		res.second = second;
		res.third = third;
