...
seqFunctionCall [ ExecNode xg ] returns [ ExprNode res = ParserEnvironment.initExprNode() ]
	...
	: ( p=IDENT DOUBLECOLON { xg.append(p.getText()); xg.append("::"); packPrefix=true; } )?
	  ( i=IDENT | i=COPY | i=CLONE | i=NAMEOF | i=TYPEOF ) LPAREN { xg.append(i.getText()); xg.append("("); }
		...
		{
			if(i.getText().equals("now") && paramz.getChildrenExact().size() == 0 || ParserEnvironment.isGlobalFunction(null, i, paramz)) {
	...
	: (LPAREN { xg.append("("); } (seqRuleParams[xg, paramz])? RPAREN { xg.append(")"); })?
		{
			...
			if(ParserEnvironment.isAutoSuppliedFilterName(filterId.getText()))
			{
				if(paramz.size() != 1)
...
seqEntityDecl [ ExecNode xg ] returns [ ExecVarDeclNode res = null ]
	...
	|
		MINUS id=seqEntIdentDecl COLON type=seqTypeIdentUse RARROW // edge decl, interpreted grs don't use -:-> form
		{
			...
			xg.append(decl.getIdent() + ":" + decl.typeUnresolved);
			xg.addVarDecl(decl);
			res = decl;
		}
	;
