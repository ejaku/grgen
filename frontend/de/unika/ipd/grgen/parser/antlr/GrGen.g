...
@header {
	...
	import de.unika.ipd.grgen.ast.typedecl.*;
...
filterDeclList [ IdentNode actionOrIteratedIdent, ArrayList<FilterAutoNode> filters ]
	...
	: filterBase=IDENT { filterBaseText = filterBase.getText(); } (LT fvl=filterVariableList GT (filterBaseTextExt=filterExtension[filterBaseText, fvl] { filterBaseText = filterBaseTextExt; })?)? 
		{
			String fullName = filterBaseText + (fvl!=null ? "<" + String.join("_", fvl) + ">" : "");
			...
			if(fullName.equals("auto"))
				...
				filterAutoGenerated = new FilterAutoGeneratedNode(filterIdent, filterBaseText, fvl, actionOrIteratedIdent);
	...
	: BACKSLASH id=IDENT { idText = id.getText(); } ( LT fvl=filterVariableList GT (idTextExt=filterExtension[idText, fvl] { idText = idTextExt; })? )? 
			(LPAREN arguments[args, context] RPAREN)?
		{
			if(fvl != null)
			{
				...
				String fullName = idText + "<" + String.join("_", fvl) + ">";
				if(args.size()!=0)
