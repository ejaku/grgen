...
filterDecls [ IdentNode actionIdent, TestDeclNode actionDecl ]
	@init {
		ArrayList<FilterAutoNode> filters = new ArrayList<FilterAutoNode>();
	}
...
filterDeclsIterated [ IdentNode iteratedIdent, IteratedNode iterated ]
	@init {
		ArrayList<FilterAutoNode> filtersAutoGenerated = new ArrayList<FilterAutoNode>();
		ArrayList<FilterAutoNode> filtersAutoSupplied = env.getFiltersAutoSupplied(iterated);
	}
...
filterDeclList [ IdentNode actionOrIteratedIdent, ArrayList<FilterAutoNode> filters ]
	: filterBase=IDENT (LT fvl=filterVariableList GT)? 
		{
			String fullName = filterBase.getText() + (fvl!=null ? "<" + String.join("_", fvl) + ">" : "");
			IdentNode filterIdent = new IdentNode(env.define(ParserEnvironment.ACTIONS, fullName, getCoords(filterBase)));

			FilterAutoGeneratedNode filterAutoGenerated;
			if(fullName.equals("auto"))
				filterAutoGenerated = new FilterAutoGeneratedNode(filterIdent, "auto", null, actionOrIteratedIdent);
			else
				filterAutoGenerated = new FilterAutoGeneratedNode(filterIdent, filterBase.getText(), fvl, actionOrIteratedIdent);
...
filterDeclListContinuation [ IdentNode actionOrIteratedIdent, ArrayList<FilterAutoNode> filters ]
...
matchClassFilterDeclList [ IdentNode matchClassIdent, ArrayList<MatchClassFilterCharacter> matchClassFilters ]
	...
	: filterBase=IDENT { filterBaseText = filterBase.getText(); } LT fvl=filterVariableList GT (filterBaseTextExt=filterExtension[filterBaseText, fvl] { filterBaseText = filterBaseTextExt; })?
		{
			matchClassFilters.add(new MatchClassFilterAutoGeneratedNode(getCoords(filterBase), filterBaseText, fvl, matchClassIdent));
		}
	(
...
patternStmt [ CollectNode<BaseNode> conn, CollectNode<SubpatternUsageNode> subpatterns, CollectNode<SubpatternReplNode> subpatternRepls,
...
filterUse [ IdentNode iterated, int context ] returns [ FilterInvocationNode res = null ]
	...
	: BACKSLASH id=IDENT (LT fvl=filterVariableList GT)? (LPAREN arguments[args, context] RPAREN)?
		{
			if(fvl != null)
			{
				if(!env.isAutoGeneratedBaseFilterName(id.getText()))
					reportError(getCoords(id), "Unknown def-variable-based filter " + id.getText() + "! Available are: orderAscendingBy, orderDescendingBy, groupBy, keepSameAsFirst, keepSameAsLast, keepOneForEach.");
				String fullName = id.getText() + "<" + String.join("_", fvl) + ">";
				if(args.size()!=0)
			...
			else if(id.getText().equals("auto"))
			{
				if(args.size()!=0)
					reportError(getCoords(id), "The filter " + id.getText() + " expects 0 arguments.");
				reportError(getCoords(id), "The filter " + id.getText() + " is not supported for iterateds.");
				/*IdentNode filterAutoGen = new IdentNode(env.occurs(ParserEnvironment.ACTIONS, fullName, getCoords(id)));
					res = new FilterInvocationNode(iterated, filterAutoGen, args);*/
			}
			else if(env.isAutoSuppliedFilterName(id.getText()))
			{
				if(args.size()!=1)
					reportError(getCoords(id), "The filter " + id.getText() + " expects 1 arguments.");
				IdentNode filterAutoSup = new IdentNode(env.occurs(ParserEnvironment.ACTIONS, id.getText(), getCoords(id)));
				res = new FilterInvocationNode(iterated, filterAutoSup, args);
			}
			else
			{
				reportError(getCoords(id), "Unknown filter " + id.getText() + ". Available are the auto-supplied and auto-generated ones.");
			}
		}
	;
