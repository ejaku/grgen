				...
				{ env.popScope(); } { namer.undefExprBlock(); } RBRACE)?
		{
			if(fvl != null)
			{
				...
				if(idText.equals("assign"))
					res = new FilterInvocationLambdaExpressionNode(iterated, getCoords(id), idText, fvl.get(0), $lambdaExprVar.va,
						$lambdaExprVar.vi, $lambdaExprVar.vd, e);
				else
				{
			...
			{
				res = new FilterInvocationLambdaExpressionNode(iterated, getCoords(id), idText, null, $lambdaExprVar.va,
					$lambdaExprVar.vi, $lambdaExprVar.vd, e);
			}
...
lambdaExprVarDeclPrefix [ AnonymousScopeNamer namer, int context ]
		returns [ VarDeclNode va, VarDeclNode vi, VarDeclNode vd ]
	options { k = *; }
	...
	|
		lambdaExprVar=maybeIndexedLambdaExprVarDecl[namer, context]
			{ $va = null; $vi = $lambdaExprVar.vi; $vd = $lambdaExprVar.vd; }
	;
